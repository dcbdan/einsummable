...
#define NO_SAVE_TENSOR(t, x) [&] { return t; }()

model_args_t model_args_t::llama(int n, uint64_t batch_size)
{
    if (n == 1) {
        return llama_7B(batch_size);
    }
    if (n == 2) {
        return llama_13B(batch_size);
    }
    if (n == 4) {
        return llama_30B(batch_size);
    }
    if (n == 8) {
        return llama_65B(batch_size);
    }
...
static tuple<tensor_t, tensor_t>
lora_init(graph_writer_t* w, int rank, full_dim_t dj, full_dim_t dk)
{
...
static tensor_t lora_matmul(graph_writer_t*                     w,
                            tensor_t                            x,
                            tensor_t                            weight,
...
rms_norm_t::rms_norm_t(graph_writer_t* w, string name, full_dim_t dim, float eps, dtype_t dtype)
...
attention_t::attention_t(graph_writer_t* w,
                         string          name,
                         model_args_t    args,
                         uint64_t        start_pos,
      ...
      head_dim(args.head_dim())
{
    ...
    if (start_pos != 0) {
        ...
        prev_kv = tuple<tensor_t, tensor_t>{writer->input(prev_shape), writer->input(prev_shape)};
    }
}

tensor_t attention_t::apply_rotary_embedding(tensor_t x, tensor_t freqs_cis)
{
    return _apply_rotary_embedding(*writer, x, freqs_cis);
}

tensor_t
attention_t::_apply_rotary_embedding(graph_writer_t& writer, tensor_t x, tensor_t freqs_cis)
{
    ...
    x = x.to_complex();
    x = writer.ew("abcd,bd->abcd", scalarop_t::make_mul(dtype_t::c64), x, freqs_cis);
    return x.to_real();
}

map<string, tensor_t> attention_t::weight_map() const
{
    map<string, tensor_t> ret{{name + "wq.weight", wq},
                              {name + "wk.weight", wk},
                              {name + "wv.weight", wv},
                              {name + "wo.weight", wo}};
    if (lora_wq) {
...
tensor_t attention_t::forward(tensor_t x, tensor_t freqs_cis, optional<tensor_t> mask)
{
    ...
    vector<uint64_t> full_xshape = {batch_size, seqlen, n_heads, head_dim};
    ...
    if (mask) {
        scores = writer->ew(
            "abcd,cd->abcd", scalarop_t::make_add(scores.get_dtype()), scores, mask.value());
    }
    ...
    full_shape_t output_shape({full_dim_t::singleton(batch_size),
...
feedforward_t::feedforward_t(graph_writer_t* w, string name, full_dim_t dim, uint64_t hidden_dim)
    : writer(w), name(name)
{
    full_shape_t to_hidden({full_dim_t::singleton(hidden_dim), dim});
    full_shape_t to_dim({dim, full_dim_t::singleton(hidden_dim)});
...
transformer_block_t::transformer_block_t(graph_writer_t* w,
                                         int             layer_id,
                                         model_args_t    args,
                                         uint64_t        start_pos,
                                         optional<int>   lora_rank)
...
tensor_t transformer_block_t::forward(tensor_t x, tensor_t freqs_cis, optional<tensor_t> mask)
{
    mark1 = writer->get_graph().nodes.size();
    tensor_t h = writer->add(x, attention.forward(attention_norm.forward(x), freqs_cis, mask));
    mark2 = writer->get_graph().nodes.size();
    tensor_t out = writer->add(h, feedforward.forward(feedforward_norm.forward(h)));
    mark3 = writer->get_graph().nodes.size();
    return out;
}

transformer_t::transformer_t(graph_writer_t* w,
                             model_args_t    args,
                             uint64_t        start_pos,
                             optional<int>   lora_rank)
    : writer(w), args(args), expected_batch_size(args.batch_size), start_pos(start_pos)
{
    for (int layer_id = 0; layer_id != args.n_layers; ++layer_id) {
        layers.emplace_back(writer, layer_id, args, start_pos, lora_rank);
    }

    full_freqs_cis =
        writer->input({args.max_seq_len, uint64_div(args.head_dim(), 2)}, dtype_t::c64);

    norm = rms_norm_t(writer, "norm.", args.full_dim(), args.norm_eps);

    full_shape_t to_vocab({full_dim_t::singleton(args.vocab_size), args.full_dim()});
...
tensor_t transformer_t::forward(tensor_t x)
{
    ...
    if (seqlen > 1) {
        if (start_pos != 0) {
            throw std::runtime_error("Masks are only supported on the first iteration");
...
dbuffer_t transformer_t::form_full_freqs_cis(uint64_t args_dim,
...
dbuffer_t transformer_t::form_freqs_cis(uint64_t dim, uint64_t end, float theta)
{
...
dbuffer_t transformer_t::form_position_interpolation_full_freqs_cis(model_args_t const& args,
...
dbuffer_t transformer_t::form_position_interpolation_full_freqs_cis(uint64_t args_dim,
                                                                    uint64_t args_n_heads,
                                                                    uint64_t args_max_seq_len,
                                                                    uint64_t orig_seq_len)
{
...
{
    return fill_t(fill_t::lowertri_t{.lower = scalar_t::zero(dtype),
...
vector<tuple<tensor_t, tensor_t>> transformer_t::get_prev_kvs() const
{
...
vector<tuple<tensor_t, tensor_t>> transformer_t::get_new_kvs() const
{
    vector<tuple<tensor_t, tensor_t>> ret;
    for (auto const& layer : layers) {
