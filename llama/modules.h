...
tensor_t apply_rotary_embedding(graph_writer_t& writer, tensor_t x, tensor_t freqs_cis);
...
struct attention_t {
    ...
    static tensor_t _apply_rotary_embedding(graph_writer_t& writer, tensor_t x, tensor_t freqs_cis);

    tensor_t forward(tensor_t x, tensor_t freqs_cis, optional<tensor_t> mask);

    void set_next_keys_and_values(tensor_t k, tensor_t v);

    map<string, tensor_t> weight_map() const;

    tuple<tensor_t, tensor_t> get_prev_kv() const
    {
        return prev_kv.value();
    }

    tuple<tensor_t, tensor_t> get_new_kv() const
    {
        return next_kv.value();
    }
...
struct feedforward_t {
    ...
    feedforward_t(graph_writer_t* w,
...
struct transformer_block_t {
    transformer_block_t(graph_writer_t* w,
                        int             layer_id,
                        model_args_t    args,
                        uint64_t        start_pos,
                        optional<int>   lora_rank);
    ...
    tuple<tensor_t, tensor_t> get_prev_kv() const
    {
        return attention.get_prev_kv();
    }

    tuple<tensor_t, tensor_t> get_new_kv() const
    {
        return attention.get_new_kv();
    }
...
struct transformer_t {
    transformer_t(graph_writer_t* w,
                  model_args_t    args,
                  uint64_t        start_pos,
    ...
    static dbuffer_t
    form_full_freqs_cis(uint64_t args_dim, uint64_t args_n_heads, uint64_t args_max_seq_len);
    static dbuffer_t form_freqs_cis(uint64_t dim, uint64_t end, float theta = 10000.0);

    static dbuffer_t form_position_interpolation_full_freqs_cis(model_args_t const& args,
                                                                uint64_t            orig_seq_len);
    static dbuffer_t form_position_interpolation_full_freqs_cis(uint64_t args_dim,
