...
int main(int argc, char** argv)
{
    ...
    if (which_server == "mg") {
        server = std::unique_ptr<server_base_t>(
            new cpu_mg_server_t(communicator, mem_size, num_threads, num_channels_per_move));
                    ...
                    int                             world_size)
{
    ...
    if (actually_run) {
        ...
        // and creating a batchsize by embed size matrix
        {
            ...
            server->insert_tensor(embeddings.get_id(),
                                  embeddings.get_shape().full(),
                                  make_embedding(tokenizer.vocab_size(), embedding_matrix, tokens));
        }
