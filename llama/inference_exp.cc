...
void do_a_matmul()
{
    ...
    matrix_multiply(dtype_t::f32, 1000, 1000, 1000, false, false, z.raw(), x.raw(), y.raw());
}
//
...
struct recorder_t {
    ...
    void insert_freqs_cis(int      gid,
                          uint64_t margs_dim,
                          uint64_t margs_n_heads,
                          uint64_t margs_max_seq_len);

    void execute_graph(graph_t const&             graph,
                       vector<placement_t> const& pls,
                       string                     message,
                       set<int>                   random_gids = {});

    void build_next_relation(int                        scores_gids,
                             builder_t const&           next_builder,
                             vector<placement_t> const& next_pls);

    void close_readers()
    {
        events.emplace_back(event_t::close_readers_t{});
    }

    vector<event_t> const& get_events() const
    {
        return events;
    }
...
private:
    ...
    {
        return relations.at(gid);
    }

    void remap(remap_relations_t const& remap_relations, string message);

    void
    execute_taskgraph(taskgraph_t const& taskgraph, string message, vector<int> const& random_tids);
...
struct executor_t {
    executor_t(communicator_t& c, int n, exec_state_t::priority_t p)
        : comm(c),
          threadpool(std::max(1, int(std::thread::hardware_concurrency()))),
          num_channels_per_move(n),
          priority_type(p)
    {
...
private:
    ...
    void read_weight_chunk_server(int chunk, string name, memloc_t const& memloc);
    void read_weight_chunk(int chunk, string name, mem_t const& mem);

    void fill_embedding_matrix_server();

    map<int, buffer_t> fill_embedding_matrix(remap_relations_t const& remap);

    dbuffer_t get_tensor_server(partition_t const& src_part, vector<memloc_t> const& src_mems);

    void
    push_tensor_server(dbuffer_t buffer, partition_t dst_part, vector<memloc_t> const& dst_mems);

    map<int, buffer_t> get_data(partition_t const&      src_part,
                                vector<memloc_t> const& src_mems,
                                relation_t const&       dst_rel);

    void push_data(map<int, buffer_t>      data,
...
private:
    ...
    {
        static vector<string> ret{"execute",
                                  "read_weight_chunk",
                                  "read_embedding",
                                  "get_data",
                                  "push_data",
...
enum class build_type_t { init_next = 0, just_init, just_next };
...
void main_build(int argc, char** argv)
{
    ...
    event_t::init_t event_init{.world_size = world_size,
                               .mem_size = mem_size,
    ...
    if (build_type != build_type_t::just_next) {
        recorder.read_data_matrix(
            builder.embeddings, batch_size, seq_len, margs.n_heads, margs.head_dim());
    }
...

    {
        ...
        recorder.insert_freqs_cis(gid, margs.dim, margs.n_heads, margs.max_seq_len);
    }
...
void main_execute(int argc, char** argv)
{
    ...
    exec_state_t::priority_t priority_type = parse_priority_type(args.get<string>("priority_type"));
...
void recorder_t::read_data_matrix(int gid, uint64_t bsz, uint64_t seq_len, uint64_t d0, uint64_t d1)
{
    auto data_locs = insert_relation(
        gid, relation_t::make_singleton(dtype_t::f32, {bsz, seq_len, d0, d1}, _max_tid + 1));

    events.emplace_back(event_t::load_data_matrix_t{bsz, seq_len, d0 * d1, data_locs[0].as_mem()});
}
...
void recorder_t::read_weight(int gid, string name, vector<uint64_t> const& shape)
{
    ...
    events.emplace_back(event_t::load_weight_t{name, data_locs});
}

void recorder_t::insert_mask(int gid, uint64_t seq_len)
{
    auto data_locs = insert_relation(
        gid, relation_t::make_singleton(dtype_t::f32, {seq_len, seq_len}, _max_tid + 1));
    events.emplace_back(event_t::load_mask_t{seq_len, data_locs[0].as_mem()});
}

void recorder_t::insert_freqs_cis(int      gid,
                                  uint64_t args_dim,
                                  uint64_t args_n_heads,
                                  uint64_t args_max_seq_len)
{
    ...
    auto data_locs =
        insert_relation(gid, relation_t::make_singleton(dtype_t::c64, {end, hdim}, _max_tid + 1));
    events.emplace_back(
        event_t::load_freqs_cis_t{args_dim, args_n_heads, args_max_seq_len, data_locs[0].as_mem()});
}

void recorder_t::execute_graph(graph_t const&             graph,
                               ...
                               set<int>                   random_gids)
{
    ...
    for (auto const& [gid, tids] : out_g_to_t) {
        ...
        _max_tid = std::max(*std::max_element(tids.get().begin(), tids.get().end()), _max_tid);
    }
}

void recorder_t::build_next_relation(int                        scores_gid,
                                     builder_t const&           next_builder,
                                     vector<placement_t> const& next_pls)
{
    ...
    auto embedding_data_locs = insert_relation(embedding_gid, dtype_t::f32, embedding_pl);

    events.emplace_back(
        event_t::build_next_data_matrix_t{.src_partition = scores_rel.placement.partition,
...
void recorder_t::remap(remap_relations_t const& remap_relations, string message)
{
    ...
    auto [gid_to_inn, gid_to_out, taskgraph] = taskgraph_t::make(g.graph, g.get_placements());

    // before: full_data_locs is with respect to the remap inn tids
    _update_map_with_new_tg_inns(tensors_, remap_gid, gid_to_inn, remap_relations, std::nullopt);
    // after: full_data_locs is with respect to the tasgkraph inns

    auto [inn_tg_to_loc, out_tg_to_loc, memgraph] = memgraph_t::make(
        taskgraph, {}, vector<uint64_t>(world_size, mem_size), tensors_, alloc_settings, false);

    tensors_.clear();

    events.emplace_back(
        event_t::execute_t{.message = message + ":remap", .memgraph = std::move(memgraph)});
...
void recorder_t::execute_taskgraph(taskgraph_t const& taskgraph,
                                   string             message,
                                   vector<int> const& random_tids)
{
    ...
    if (random_tids.size() > 0) {
        ...
        events.emplace_back(event_t::random_t{.mems = random_locs});
    }

    events.emplace_back(
        event_t::execute_t{.message = message + ":inputs", .memgraph = inputs_mg.value()});
    events.emplace_back(
        event_t::execute_t{.message = message + ":core", .memgraph = std::move(core_mg)});
...
vector<memloc_t> recorder_t::insert_relation(int gid, dtype_t dtype, placement_t const& pl)
{
    relation_t   rel{.dtype = dtype, .placement = pl, .tids = vtensor_t<int>(pl.block_shape())};
...
vector<memloc_t> recorder_t::insert_relation(int gid, relation_t const& relation)
{
...
memloc_t recorder_t::insert_data(int tid, int loc, uint64_t size)
{
    ...
    if (maybe) {
        ...
        memloc_t        memloc{.offset = offset, .size = size, .loc = loc};
...
vector<memloc_t> recorder_t::get_data_locs(vector<int> const& tids)
{
    vector<memloc_t> ret;
...
void executor_t::operator()(event_t const& event)
{
    ...
    if (event.is_load_weight()) {
        ...
        dbuffer_t     embeddings = lookup_embeddings(
            margs.vocab_size, margs.dim, embedding_matrix, token_maker.get_tokens());
        ...
        buffer_t freqs_cis = transformer_t::form_full_freqs_cis(dim, heads, max_seq_len).data;
        ...
        uint64_t       top_n = 1;
        vtensor_t<int> top_choices =
            get_top_choices(scores, margs.batch_size, margs.vocab_size, top_n);
        dbuffer_t embeddings =
            lookup_embeddings(margs.vocab_size, margs.dim, embedding_matrix, top_choices);
...
void executor_t::fill_random_f32s(vector<mem_t> const& mems)
{
    for (auto const& mem : mems) {
        dbuffer_t dbuffer(dtype_t::f32, make_buffer_reference(data->data + mem.offset, mem.size));
...
void executor_t::listen()
{
    ...
    while (true) {
        ...
        if (cmd == cmd_t::execute) {
            ...
            memgraph_t memgraph = memgraph_t::from_wire(comm.recv_string_parallel(threadpool, 0));
            execute(memgraph, message);
...
void executor_t::copy_into_data(buffer_t buffer, mem_t mem)
{
    ...
    std::copy(buffer->data, buffer->data + buffer->size, data->data + mem.offset);
}
...
void executor_t::execute(memgraph_t const& memgraph, string message)
{
    ...
    if (this_rank == 0) {
        ...
        DLINEOUT(message << " | num nodes " << memgraph.nodes.size() << " | num move nodes "
                         << num_move_nodes << " | data moved " << mb_moved << "MB");
    }

    exec_graph_t graph = exec_graph_t::make_cpu_exec_graph(
        memgraph, this_rank, kernel_executor, num_channels_per_move);
    ...
    rm_ptr_t resource_manager(new resource_manager_t(
        vector<rm_ptr_t>{rm_ptr_t(new cpu_workspace_manager_t()),
...
void executor_t::read_weight_chunk_server(int chunk, string name, memloc_t const& memloc)
{
    if (this_rank != 0) {
...
void executor_t::read_weight_chunk(int chunk, string name, mem_t const& mem)
{
    dbuffer_t weight_f16 = dbuffer_t(dtype_t::f16, readers.at(chunk).read(name));
    dbuffer_t weight = dbuffer_t(dtype_t::f32, make_buffer_reference(data->data, mem.size));
    std::copy(weight_f16.f16(), weight_f16.f16() + weight_f16.nelem(), weight.f32());
}
...
void executor_t::fill_embedding_matrix_server()
{
...

    {
        ...
        placement_t pl =
            tensor_reader_t::get_placement("tok_embeddings.weight", shape, world_size, num_files);

        relation_t rel{
            .dtype = dtype_t::f32, .placement = pl, .tids = vtensor_t<int>(pl.block_shape())};
        rel.tids.get() = vector_iota<int>(pl.num_parts());

        remap.insert(rel, rel.as_singleton(99, 0));
    }
...
map<int, buffer_t> executor_t::fill_embedding_matrix(remap_relations_t const& remap)
{
    ...
    for (int chunk = 0; chunk != nbid; ++chunk) {
        if (this_rank == (chunk % world_size)) {
            ...
            ret.insert({tid, readers.at(chunk)("tok_embeddings.weight", dtype_t::f32)});
        }
    }
...
dbuffer_t executor_t::get_tensor_server(partition_t const&      src_part,
                                        vector<memloc_t> const& src_mems)
{
    partition_t dst_part = partition_t::singleton(src_part.total_shape());
    relation_t  dst_rel{.dtype = dtype_t::f32,
    ...
    return dbuffer_t(dtype_t::f32, get_data(src_part, src_mems, dst_rel).at(99));
}

relation_t _build_relation(dtype_t dtype, partition_t const& part, vector<memloc_t> const& mems)
{
    ...
    relation_t rel{
        .dtype = dtype, .placement = placement_t(part), .tids = vtensor_t<int>(part.block_shape())};
    ...
    vector<uint64_t> nelems_per_block = rel.placement.partition.all_block_sizes().get();
...
map<int, buffer_t> executor_t::get_data(partition_t const&      src_part,
                                        vector<memloc_t> const& src_mems,
                                        relation_t const&       dst_rel)
{
    ...
    for (int tid = 0; tid != src_mems.size(); ++tid) {
        ...
        data_map.insert({tid, make_buffer_reference(ptr, memloc.size)});
    }
...
void executor_t::push_tensor_server(dbuffer_t               dbuffer,
                                    partition_t             dst_part,
                                    vector<memloc_t> const& dst_mems)
{
    partition_t src_part = partition_t::singleton(dst_part.total_shape());
    relation_t  src_rel{.dtype = dbuffer.dtype,
...
void executor_t::push_data(map<int, buffer_t>      data_map,
                           relation_t const&       src_rel,
                           partition_t const&      dst_part,
                           vector<memloc_t> const& dst_mems)
{
...
event_t event_t::from_proto(es_proto::InferenceEvent const& ie)
{
    if (ie.has_init()) {
        ...
        return event_t(init_t{.world_size = i.world_size(),
                              .mem_size = i.mem_size(),
        ...
        return event_t(load_weight_t{.name = l.name(), .data_locs = data_locs});
    } else if (ie.has_load_data_matrix()) {
        auto const& l = ie.load_data_matrix();
        return event_t(load_data_matrix_t{.batch_size = l.batch_size(),
                                          .seq_len = l.seq_len(),
                                          .d_embed = l.d_embed(),
                                          .mem = mem_t::from_proto(l.mem())});
    } else if (ie.has_load_mask()) {
        auto const& l = ie.load_mask();
        return event_t(load_mask_t{.seq_len = l.seq_len(), .mem = mem_t::from_proto(l.mem())});
    } else if (ie.has_load_freqs_cis()) {
        auto const& l = ie.load_freqs_cis();
        return event_t(load_freqs_cis_t{.dim = l.dim(),
                                        .heads = l.heads(),
        ...
        return event_t(
            execute_t{.message = e.msg(), .memgraph = memgraph_t::from_proto(e.memgraph())});
        ...
        return event_t(
            build_next_data_matrix_t{.src_partition = partition_t::from_proto(b.src_part()),
                                     .src_data_locs = src_data_locs,
        ...
        return event_t(random_t{.mems = std::move(mems)});
    } else {
