...
dataset_reader_t::dataset_reader_t(string tokenizer_filename, string data_filename)
    : tokenizer(tokenizer_filename), file(data_filename, std::ios_base::in | std::ios_base::binary)
{
...
tuple<vector<int>, int> dataset_reader_t::datum(int which, uint64_t seqlen)
{
...
tuple<vector<int>, int> dataset_reader_t::random_datum(uint64_t seqlen)
{
    return datum(runif(num()), seqlen);
}

tuple<vector<vector<int>>, vector<int>> dataset_reader_t::random_data(uint64_t batch_size,
                                                                      uint64_t seqlen)
{
...
dbuffer_t dataset_reader_t::make_embedding(dbuffer_t const&   embedding_matrix,
                                           vector<int> const& tokens)
{
    return ::make_embedding(vocab_size, embedding_matrix, tokens);
}

dbuffer_t dataset_reader_t::one_hot_encode(dtype_t dtype, vector<int> const& tokens)
{
                         ...
                         vector<int> const& tokens)
{
    ...
    touchdim_t td_embed{.d_inn = embed_size,
                        .d_out = embed_size,
                        .offset_inn = 0,
                        .offset_out = 0,
                        .size = embed_size};
    auto       make_select_td = [&](int token, int which) {
        return touchdim_t{.d_inn = vocab_size,
                                .d_out = seqlen,
    ...
    for (uint64_t i = 0; i != seqlen; ++i) {
        ...
        touch_t touch{.selection = {make_select_td(token, i), td_embed},
