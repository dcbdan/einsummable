...
void main_rank_zero(std::unique_ptr<server_base_t>& server,
                    ...
                    autoplace_config_t              config)
{
    ...
    for (auto const& [name, tensor] : weight_map) {
        if (name.find("lora") != string::npos) {
            ...
            relation_t relation =
                model_loader(register_cmd, name, tensor.get_shape().full(), next_tid);
    ...
    server->insert_tensor(full_freqs_cis.get_id(),
    ...
    for (int iter = 1; iter != niter + 1; ++iter) {
        ...
        server->insert_tensor(labels.get_id(),
