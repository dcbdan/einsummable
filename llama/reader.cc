...
buffer_t local_tensor_reader_t::operator()(string const& tensor_name, dtype_t expected_dtype)
{
...
optional<tuple<string, uint64_t>> local_tensor_reader_t::read_next_weight_info()
{
    if (file.eof()) {
        return std::nullopt;
    }
...
tensor_reader_t::tensor_reader_t(communicator_t&                                comm,
                                 ...
                                 int                                            n,
                                 dtype_t                                        d)
    : comm(comm), process_data(process), world_size(world_size), n_total_files(n), dtype(d)
{
...
relation_t tensor_reader_t::operator()(string                  register_cmd,
                                       string const&           name,
                                       ...
                                       int                     starting_tid)
{
    ...
    if (name.find("norm") != string::npos) {
        ...
        return relation_t{.dtype = dtype, .placement = pl, .tids = tids};
    }
    ...
    return relation_t{.dtype = dtype, .placement = placement, .tids = tids};
}
...
map<int, buffer_t> tensor_reader_t::_read(string const& tensor_name, vector<int> const& whiches)
{
...
placement_t tensor_reader_t::get_placement(string const&           name,
                                           vector<uint64_t> const& shape,
                                           int                     world_size,
                                           int                     n_total_files)
{
    ...
    auto        fix = [&](int d) {
        partition.partdims[d] =
            partdim_t::repeat(n_total_files, uint64_div(shape[d], n_total_files, name));
    };
