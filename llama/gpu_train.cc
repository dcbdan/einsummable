...
graph_setup_t make_graph(args_t& pargs, int llama_num_files, uint64_t batch_size)
{
    ...
    updater_desc_t            updater_desc =
        is_adamw ? updater_desc_t{dtype, adamw} : updater_desc_t{dtype, vanilla};
    ...
    vector<tuple<int, fill_t>> init_fills =
        update_weights(updater_desc, graph, old_news, weight_ids, grad_ids);
    ...
    checkpoint_graphs_t checkpoint_graphs(graph, checkpoints, forward_ids);

    return graph_setup_t{.margs = margs,
                         .full_graph = graph,
                    ...
                    autoplace_config_t config)
{
    ...
    DOUT("placement time: "
         << std::chrono::duration_cast<std::chrono::milliseconds>(end_pls - start_pls).count()
         << " ms");
    ...
    {
        relation_t rel = model_loader(register_cmd,
    ...
    for (auto const& [name, id] : info.model_weight_map) {
        ...
        if (name.find("lora") != string::npos) {
            ...
            relation_t relation = model_loader(register_cmd, name, shape, next_tid);
    ...
    server->insert_tensor(info.full_freqs_cis_id,
    ...
    for (int iter = 1; iter != niter + 1; ++iter) {
        ...
        server->insert_tensor(
            ...
            data_loader.make_embedding(embedding_matrix, vector_flatten(data_tokens)));

        server->insert_tensor(info.labels_id,
