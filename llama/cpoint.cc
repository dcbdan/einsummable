...
int _get_label(graph_t const& graph, vector<int> const& checkpoints, int gid)
{
...
int get_label(graph_t const& graph, vector<int> const& checkpoints, int gid)
{
    //  auto iter = std::find(checkpoints.begin(), checkpoints.end(), gid);
    //  if(iter != checkpoints.end()) {
...
ffinfo_t make_ff_simple(uint64_t batch, uint64_t hidden, uint64_t dim, int num_weight = 3)
{
    ...
    scalarop_t difference_squared = scalarop_t::combine(scalarop_t::make_square(z.get_dtype()),
    ...
    scalarop_t update =
        scalarop_t::combine(scalarop_t::make_sub(),
                            {scalarop_t::make_identity(), scalarop_t::make_scale("learning_rate")});
    ...
    return ffinfo_t{.graph = writer.get_graph(),
                    .x = x.get_id(),
                    .y = y.get_id(),
...
ffinfo_t make_llama(uint64_t batch, uint64_t seqlen, int n_layers = -1, int n_files = 1)
{
    ...
    tensor_t embeddings = writer.input(full_shape_t({full_dim_t::singleton(margs.batch_size),
                                                     ...
                                                     margs.full_dim()}));
    tensor_t labels = writer.input(vector<uint64_t>{margs.batch_size, margs.vocab_size}, dtype);
    ...
    tensor_t loss;
    {
        ...
        tensor_t lll = writer.ew("b,b->b", scalarop_t::make_div(dtype), evsubset, evsum);
    ...
    vector<int> checkpoints = vector_from_each_method(model.checkpoints, int, get_id);
...
int get_min_checkpoint(graph_t const&       graph,
...
struct graph_id_manager_t {
    struct context_t {
        context_t(graph_id_manager_t* s, int w) : self(s), which(w) {}
...
memory_usage(graph_t const&                     graph,
             set<int>                           keys,
...
int main(int argc, char** argv)
{
    ...
    auto info = do_llama ? make_llama(batch, seqlen, n_layers, which_llama)
                         : make_ff_simple(batch, hidden, dim, num_ws);

    vector<string> cs{"#ff5733", "#377e20", "#2cb392", "#7af3f5", "#374ded", "#b337ed"};
    ...
    for (int id = 0; id != info.graph.nodes.size(); ++id) {
        ...
        if (iter != info.forward_labels.end()) {
            ...
            int cpoint = get_min_checkpoint(
                info.graph, info.forward_labels, backward_labels, id, info.checkpoints.size());
    ...
    ///////////////////////////////////////////////////////////////////
    subgraph_former_t former{.full_graph = info.graph,
    ...
    {
        auto [us, ks, u] = memory_usage(info.graph, base_keys, base_flops_usage, make_base_message);
        base_usages = us;
        base_keys = ks;
        base_flops_usage = u;
    }
    ...
    for (int i = 0; i != graphs.size(); ++i) {
        ...
        auto [us, ks, u] = memory_usage(graph, keys, flops_usage, make_subgraph_message(i));
