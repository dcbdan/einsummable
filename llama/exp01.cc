...
vector<placement_t> solve_forwardsim(graph_t const&     graph,
...
tuple<vector<placement_t>, vector<relationwise_stat_t>> solve_relationwise(graph_t const& graph,
                                                                           int            nlocs,
                                                                           ...
                                                                           double beta)
{
    ...
    relationwise_mcmc_t mcmc(
        graph, kernel_coster, nlocs, n_threads_per_node, max_blocks, equal_items_t<int>());
...

    {
        ...
        vector<partition_t> parts =
            autopartition(graph, min_sizing, nlocs * n_threads_per_node, mcmc.get_equal_gids());
