...
struct tensor_reader_t {
    ...
    static placement_t get_placement(string const&           tensor_name,
                                     vector<uint64_t> const& shape,
                                     int                     world_size,
                                     int                     n_total_files);

    // Should only be called by rank zero {{{
    relation_t operator()(string                  register_cmd,
    ...
    static string read_cmd()
    {
        return "tensor_reader_t/read";
    }
    static string shutdown_cmd()
    {
        return "tensor_reader_t/shutdown";
    }

private:
    void _shutdown();

    map<int, buffer_t> _read(string const& tensor_name, vector<int> const& whiches);
