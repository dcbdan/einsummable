...
void test_rms_norm()
{
    ...
    auto [buffer_w, shape_w] = read_array(dtype, "[0.4661, 0.9641, 0.3547, 0.0875, 0.7970]");
    ...
    map<int, dbuffer_t> inputs{{x.get_id(), buffer_x}, {rms_norm.weight.get_id(), buffer_w}};
...
void test_rotary_embedding()
{
    ...
    map<int, dbuffer_t> inputs{{x.get_id(), buffer_x}, {freqs_cis.get_id(), buffer_freqs_cis}};
...
void test_ff()
{
    ...
    feedforward_t ff(&writer, "name", full_dim_t::singleton(dim), hidden);
    tensor_t      y = ff.forward(x);
    y = y.save();

    map<int, dbuffer_t> inputs{{x.get_id(), buffer_x},
...
int main()
{
    ...
    auto input_input = [&]() {
        full_shape_t shape(
            {full_dim_t::singleton(bsz), full_dim_t::singleton(seq_len), args.full_dim()});
