...
int main(int argc, char** argv)
{
    ...
    model_args_t margs = model_args_t::llama(num_model_files, args.get<uint64_t>("batch_size"));
    ...
    tensor_t labels =
        writer.input(vector<uint64_t>{margs.batch_size, margs.vocab_size}, predictions.get_dtype());
    ...
    autoplace_config_t config = autoplace_config_t::make_default01(world_size, num_threads);
    ...
    std::cout << world_size << "," << (mem_size / GB) << "," << num_threads << "," << lora_rank
              << "," << model_name << "," << margs.batch_size << "," << margs.max_seq_len << ","
