...
builder_t
builder_t::_make(model_args_t const& args, uint64_t start_pos, uint64_t seqlen, bool is_last)
{
    ...
    tensor_t embeddings = writer.input(full_shape_t(
        {full_dim_t::singleton(args.batch_size), full_dim_t::singleton(seqlen), args.full_dim()}));
...
builder_t builder_t::make_first_token(model_args_t const& args, uint64_t seqlen)
{
    return _make(args, 0, seqlen, true);
}

builder_t builder_t::make_next_token(builder_t const& prev, bool make_last)
{
    if (prev.is_last()) {
