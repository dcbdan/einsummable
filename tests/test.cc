...
tests_t::tests_t() : tests_t("tests") {}

tests_t::tests_t(string name) : node(std::make_unique<node_t>(name)) {}

tests_t::node_t::node_t(string name) : name(name) {}
...
void tests_t::node_t::run(string base) const
{
    ...
    for (auto const& child : children) {
        string name = base.size() == 0 ? get_child_name(child) : base + "/" + get_child_name(child);
        ...
        if (std::holds_alternative<node_ptr_t>(child)) {
            ...
            auto        maybe_error = f();
            if (maybe_error) {
                throw std::runtime_error("failed test: " + name + "\n" +
                                         "reason: " + maybe_error.value());
            }
            DOUT(name);
        } else {
...
tests_t::context_t tests_t::context_t::make_context(string name)
{
...
tests_t::context_t::context_t(node_t& node) : node(node) {}

tests_t::context_t tests_t::make_context(string name)
{
    node_t& ret = node->insert_node(name);
    return context_t(ret);
}
