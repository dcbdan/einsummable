...
{
    partition_t partition({partdim_t::split(3, 1), partdim_t::split(4, 2), partdim_t::split(5, 3)});
...
optional<string>
reblock_20elem_test(dtype_t dtype, placement_t placement_start, placement_t placement_finish)
{
    ...
    dbuffer_t out_buffer = reference_compute_graph(graph.graph, {{gid_inn, inn_buffer}})[gid_out];

    auto [input_gid_to_tids, output_gid_to_tids, taskgraph] =
        taskgraph_t::make(graph.graph, graph.get_placements());

    // taskgraph.print();

    vtensor_t<dbuffer_t> inn_pbuffer = partition_buffer(placement_start.partition, inn_buffer);
    vtensor_t<dbuffer_t> out_pbuffer = partition_buffer(placement_finish.partition, out_buffer);

    map<int, dbuffer_t> t_input_map = init_buffer_map(input_gid_to_tids.at(gid_inn), inn_pbuffer);

    auto t_out_map = typed_reference_compute_taskgraph_from_graph_info(
        taskgraph, t_input_map, graph.graph, output_gid_to_tids);

    vtensor_t<dbuffer_t> t_out_pbuffer =
        get_partitioned_buffer(t_out_map, output_gid_to_tids[gid_out]);
...
{
    placement_t placement_start(partition_t({partdim_t::split(20, 5)}),
                                vtensor_t<int>({5}, {0, 0, 0, 0, 0}));
    placement_t placement_finish(partition_t({partdim_t::split(20, 3)}),
...
{
    placement_t placement_start(partition_t({partdim_t::split(20, 1)}), vtensor_t<int>({1}, {0}));
    placement_t placement_finish(partition_t({partdim_t::split(20, 2)}),
                                 vtensor_t<int>({2}, {0, 0}));
...
{
    placement_t placement_start(partition_t({partdim_t::split(20, 2)}),
                                vtensor_t<int>({2}, {0, 0}));
    placement_t placement_finish(partition_t({partdim_t::split(20, 1)}), vtensor_t<int>({1}, {0}));
