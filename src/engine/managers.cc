...
optional<tuple<int, bool>> group_manager_t::try_to_acquire_impl(int const& group_id)
{
...
optional<threadpool_resource_t> threadpool_manager_t::try_to_acquire_impl(unit_t const&)
{
    if (num_avail == 0) {
        return std::nullopt;
    }
