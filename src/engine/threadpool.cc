...
threadpool_t::threadpool_t(int num_to_launch) : is_stopped(false), start_threadpool(clock_now())
{
    get_numa_info();
    ...
    for (int i = 0; i != num_to_launch; ++i) {
        threads.emplace_back([this, i, max_num_threads] { this->runner(i % max_num_threads); });
    }
}
