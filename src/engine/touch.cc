...
#define _touch4_frame(name, type_t, inner_ops)                                                     \
    ...
    {                                                                                              \
        out += t0.offset_out * t1.d_out * t2.d_out * t3.d_out +                                    \
               t1.offset_out * t2.d_out * t3.d_out + t2.offset_out * t3.d_out + t3.offset_out;     \
        inn += t0.offset_inn * t1.d_inn * t2.d_inn * t3.d_inn +                                    \
               t1.offset_inn * t2.d_inn * t3.d_inn + t2.offset_inn * t3.d_inn + t3.offset_inn;     \
        for (uint64_t i0 = 0; i0 != t0.size; ++i0) {                                               \
            for (uint64_t i1 = 0; i1 != t1.size; ++i1) {                                           \
                for (uint64_t i2 = 0; i2 != t2.size; ++i2) {                                       \
                    inner_ops out += t3.d_out;                                                     \
                    inn += t3.d_inn;                                                               \
                }                                                                                  \
...
#define _call_touch1(name, T)                                                                      \
    name(ts[0], reinterpret_cast<T*>(out), reinterpret_cast<T const*>(inn))
#define _call_touch2(name, T)                                                                      \
    name(ts[0], ts[1], reinterpret_cast<T*>(out), reinterpret_cast<T const*>(inn))
#define _call_touch3(name, T)                                                                      \
    name(ts[0], ts[1], ts[2], reinterpret_cast<T*>(out), reinterpret_cast<T const*>(inn))
#define _call_touch4(name, T)                                                                      \
    name(ts[0], ts[1], ts[2], ts[3], reinterpret_cast<T*>(out), reinterpret_cast<T const*>(inn))
