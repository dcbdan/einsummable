...
gpu_workspace_manager_t::gpu_workspace_manager_t()
{
    ...
    for (int gpu = 0; gpu < num_gpus; ++gpu) {
        ...
        // for (int i = 0; i < 2; ++i) {
        //   void* mem;
        //   handle_cuda_error(cudaMalloc(&mem, 16 * 1024 * 1024), "gpu_workspace_manager_t. cuda
        //   malloc"); data[gpu].emplace_back(mem, 16 * 1024 * 1024);
        // }
        // for (int i = 0; i < num_gpus; ++i) {
        //   void* mem;
        //   handle_cuda_error(cudaMalloc(&mem, 32 * 1024 * 1024), "gpu_workspace_manager_t. cuda
        //   malloc"); data[gpu].emplace_back(mem, 32 * 1024 * 1024);
        // }
    }
}
...
gpu_workspace_manager_t::try_to_acquire_impl(gpu_workspace_desc_t const& desc)
{
    ...
    for (int i = 0; i != data_here.size(); ++i) {
        ...
        if (size_ >= size) {
            gpu_workspace_resource_t rsrc{.device = device, .ptr = mem, .size = size_};
    ...
    return gpu_workspace_resource_t{.device = device, .ptr = mem, .size = size};
}

void gpu_workspace_manager_t::release_impl(gpu_workspace_resource_t const& resource)
{
    std::unique_lock lk(m);
