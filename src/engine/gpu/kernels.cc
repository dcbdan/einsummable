  ...
  castable_t castable)
{
    ...
    vector<void const*> inns, void* work, uint64_t worksize)
  {
    ...
    cutensorContract(
      handle, 
      plan,              // TODO: c can change locations, can cutensorContrcation
    ...
    handle_cutensor_error(cutensorReduce(handle, plan,
                alpha, inns[0],
                (const void*)&beta,  out, 
                out, work, worksize, stream));
  ...
  return std::make_pair(worksize, f);
}
...
build_cutensor_elementwise(cutensor_elementwise_op_t op)
{
  ...
  if(std::holds_alternative<cutensor_elementwise_op_t::unary_t>(op.op)){
    ...
    typeCompute = dtype_to_computetype(unary.arg.scale.dtype);
    ...
    //std::cout << "Log message15" << std::endl;
    handle_cutensor_error(cutensorCreatePlan(handle,
                                    &plan,
                                    desc,
                                    planPref,
    ...
    //std::cout << "Log message16" << std::endl;
    ...
    return [type,binary,swapped,plan]
    (cudaStream_t stream, cutensorHandle_t const handle, void* out, vector<void const*> inns){
      void* ptr1;
      void* ptr2;
      float16_t alpha1, beta1;
    ...
    typeCompute = dtype_to_computetype(ternary.a0.scale.dtype);
    ...
    cutensorOperationDescriptor_t desc;
    handle_cutensor_error(cutensorCreateElementwiseTrinary(handle, 
                                                  &desc,
                                                  descA, modeA.data(), /* unary operator A */ ternary.a0.op,
                                                  descB, modeB.data(), /* unary operator B */ ternary.a1.op,
                                                  descC, modeC.data(), /* unary operator C */ ternary.a2.op,
                                                  descC, modeC.data(),
                                                  /* binary operator AC  */ ternary.op_0_1,
                                                  /* binary operator ABC */ ternary.op_01_2,
                                                  typeCompute));
      ...
      )
    {
      ...
      handle_cutensor_error(cutensorElementwiseTrinaryExecute(handle, plan,
                                                alpha,  inns[0],
                                                beta ,  inns[1],
                                                gamma,  inns[2],
                                                 out, stream));
