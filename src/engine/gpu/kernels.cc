...
void touch1(touchdim_t const& t0,
            void*             out,
            void const*       inn,
            cudaStream_t      stream,
            int               choice,
...
void touch2(touchdim_t const& t0,
            touchdim_t const& t1,
            void*             out,
            void const*       inn,
            cudaStream_t      stream,
...
void touch3(touchdim_t const& t0,
            touchdim_t const& t1,
            touchdim_t const& t2,
            void*             out,
            void const*       inn,
...
void touch4(touchdim_t const& t0,
            touchdim_t const& t1,
            touchdim_t const& t2,
            touchdim_t const& t3,
            void*             out,
...
cutensor_elementwise_kernel_t cutensor_silu_elementwise(uint64_t size)
{
    ...
    handle_cutensor_error(cutensorCreateTensorDescriptor(
        handle, &descA, nmodeA, extent_A.data(), nullptr /* stride */, typeA, kAlignment));

    cutensorTensorDescriptor_t descC;
    handle_cutensor_error(cutensorCreateTensorDescriptor(
        handle, &descC, nmodeC, extent_C.data(), nullptr /* stride */, typeC, kAlignment));
    ...
    handle_cutensor_error(
        cutensorCreateElementwiseBinary(handle,
                                        ...
                                        descA,
                                        modeA.data(),
                                        /* unary operator A  */ CUTENSOR_OP_SIGMOID,
                                        descC,
                                        modeC.data(),
                                        /* unary operator C  */ CUTENSOR_OP_IDENTITY,
                                        descC,
                                        modeC.data(),
                                        /* unary operator AC */ CUTENSOR_OP_MUL,
                                        typeCompute));
    ...
    return
        [plan](cudaStream_t stream, cutensorHandle_t handle, void* out, vector<void const*> inns) {
            void*     ptr1;
            void*     ptr2;
            ...
            handle_cutensor_error(cutensorElementwiseBinaryExecute(
                handle, plan, alpha, inns[0], beta, inns[0], out, stream));
        };
}
...
cutensor_elementwise_op_t make_mul_op(einsummable_t const& e)
{
    cutensor_elementwise_op_t op;
    op.join_shape = e.join_shape;

    cutensor_elementwise_op_t::arg_t a0{
        scalar_t::one(dtype_t::f32), CUTENSOR_OP_IDENTITY, e.inns[0]};
    cutensor_elementwise_op_t::arg_t a1{
        scalar_t::one(dtype_t::f32), CUTENSOR_OP_IDENTITY, e.inns[1]};

    cutensorOperator_t op_0_1 = CUTENSOR_OP_MUL;

    cutensor_elementwise_op_t::binary_t bi_op{op_0_1, a0, a1};
...
cutensor_elementwise_kernel_t build_cutensor_type_conversion(einsummable_t const& e)
{
    ...
    return [typeCompute, plan](
        ...
        handle_cutensor_error(
            cutensorPermute(handle, plan, alpha, inns[0], out, stream /* stream */));
    };
}

cutensor_kernel_t build_elementwise_and_pow(cutensor_elementwise_op_t op, uint64_t a_size)
{
    cutensorDataType_t          typeA = CUTENSOR_R_32F;
    cutensorDataType_t          typeB = CUTENSOR_R_32F;
    cutensorDataType_t          typeC = CUTENSOR_R_32F;
    ...
    handle_cutensor_error(cutensorCreateElementwiseBinary(handle,
                                                          &desc,
                                                          descA,
                                                          modeA.data(),
                                                          /* unary operator A  */ binary.lhs.op,
                                                          descC,
                                                          modeC.data(),
                                                          /* unary operator C  */ binary.rhs.op,
                                                          descC,
                                                          modeC.data(),
                                                          /* unary operator AC */ binary.op,
                                                          typeCompute));
    ...
    return [plan, binary, swapped, a_size, type](cudaStream_t        stream,
                                                 cutensorHandle_t    handle,
                                                 void*               out,
        ...
        if (swapped) {
            handle_cutensor_error(cutensorElementwiseBinaryExecute(
                handle, plan, alpha, work, beta, inns[0], out, stream));
        } else {
            handle_cutensor_error(cutensorElementwiseBinaryExecute(
                handle, plan, alpha, inns[0], beta, work, out, stream));
        }
    };
}