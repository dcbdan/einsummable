...
exec_graph_t exec_graph_t::make_gpu_exec_graph(memgraph_t const&            memgraph,
                                               int                          this_rank,
                                               ...
                                               map<string, scalar_t> const& scalar_vars)
{
    ...
    for (int mid = 0; mid != memgraph.nodes.size(); ++mid) {
        ...
        if (node.op.is_inputmem() || node.op.is_inputsto() || node.op.is_partialize() ||
            ...
            if (apply.is_einsummable()) {
                ...
                einsummable_t einsum = apply.get_einsummable()
                ...
                gpu_einsummable_t* op = new gpu_einsummable_t(
                    gpu_kms[loc], einsum, apply.mems, node.op.get_apply_loc());
                ...
                if (workspace_info.known()) {
                    ...
                    throw std::runtime_error(
                        "workspace size is not known; this should not happen in cutensor 2.0");
                }
                // insert into the graph
                insert(op_ptr_t(op), mid);
                // DOUT("Inserted einsummable op for node " << mid);
            } else if (apply.is_touch()) {
                gpu_touch_t* op = new gpu_touch_t(gpu_kms[loc],
                                                  apply.get_touch(),
                                                  apply.group,
                                                  apply.mems,
                ...
                throw std::runtime_error(
                    "should not reach: node op is apply but not einsummable or touch");
            }
            ...
            if (fill.is_constant()) {
                ...
                throw std::runtime_error(
                    "should not reach: constant fill is neither constant nor lowertri");
            }
        } else {
    ...
    fprintf(
        stdout,
        "Exec_Graph finished. evict_count: %d, evict_bytes: %lu, load_count: %d, load_bytes: %lu\n",
        evict_count,
        evict_bytes,
        load_count,
        load_bytes);
...
desc_ptr_t gpu_einsummable_t::resource_description() const
{
    // 1st: gpu memory ptr
    // 2nd: a stream
...
void gpu_einsummable_t::launch(resource_ptr_t rsrc, std::function<void()> callback) const
{
    ...
    gpu_km(einsummable, stream, out_mem, inn_mems, maybe_workspace);
...
desc_ptr_t gpu_touch_t::resource_description() const
{
    // 1st: gpu memory ptr
    // 2nd: a stream
...
void gpu_touch_t::launch(resource_ptr_t rsrc, std::function<void()> callback) const
{
    vector<resource_ptr_t> const& resources = resource_manager_t::get_resource(rsrc);
    ...
    gpu_km(this_touch, stream, out_mem, inn_mem);
...
desc_ptr_t gpu_copy_t::resource_description() const
{
...
void gpu_copy_t::launch(resource_ptr_t rsrc, std::function<void()> callback) const
{
    vector<resource_ptr_t> const& resources = resource_manager_t::get_resource(rsrc);
    ...
    void* src_mem = increment_void_ptr(src_buffer, src_offset);

    void* dst_mem = increment_void_ptr(dst_buffer, dst_offset);

    // print 100 elements of src

    auto        stream = streampool_manager_t::get_resource(resources[2]).stream;
    cudaError_t cudaError =
        cudaMemcpyAsync(dst_mem, src_mem, move.size, cudaMemcpyDeviceToDevice, stream);
...
void gpu_evict_t::launch(resource_ptr_t rsrc, std::function<void()> callback) const
{
    vector<resource_ptr_t> const& resources = resource_manager_t::get_resource(rsrc);

    void* global_buffer = global_buffers_t::get_resource(resources[0]);

    void* gpu_memory = increment_void_ptr(global_buffer, gpu_offset);
...
desc_ptr_t gpu_load_t::resource_description() const
{
    // 1st: gpu memory ptr
    // 2nd: a stream
    // 3rd: storage object
...
void gpu_load_t::launch(resource_ptr_t rsrc, std::function<void()> callback) const
{
    vector<resource_ptr_t> const& resources = resource_manager_t::get_resource(rsrc);
    ...
    handle_cuda_error(cudaStreamAddCallback(
                          stream,
                          [](cudaStream_t stream, cudaError_t status, void* user_data) {
                              gpu_load_info_t* info_ptr =
                                  reinterpret_cast<gpu_load_info_t*>(user_data);
                              auto& info = *info_ptr;
...
desc_ptr_t gpu_constant_t::resource_description() const
{
    // 1st: gpu memory ptr
    // 2nd: a stream
    vector<desc_ptr_t> ret;
...
desc_ptr_t gpu_lowerTri_t::resource_description() const
{
    // 1st: gpu memory ptr
    // 2nd: a stream
    vector<desc_ptr_t> ret;
...
void gpu_constant_t::launch(resource_ptr_t rsrc, std::function<void()> callback) const
{
    vector<resource_ptr_t> const& resources = resource_manager_t::get_resource(rsrc);
...
void gpu_lowerTri_t::launch(resource_ptr_t rsrc, std::function<void()> callback) const
{
    vector<resource_ptr_t> const& resources = resource_manager_t::get_resource(rsrc);
    ...
    if (fill.upper == scalar_t::negative_inf(dtype_t::f32)) {
        ...
        auto     l = fill_t::lowertri_t{.lower = fill.lower,
                                        .upper = new_upper,
                                        .nrow = fill.nrow,
                                        .ncol = fill.ncol,
                                        .start = fill.start};
    ...
    // DOUT("lower tri fill lower: " << fill.lower << " upper: " << fill.upper << " offset: " <<
    // gpu_offset
