...
private:
    ...
    void execute_matmul(matmul_t const& m,
                        cudaStream_t    stream,
                        void*           out,
                        void const*     lhs,
    ...
    void execute_contraction(contraction_t const& c,
                             cudaStream_t         stream,
                             void*                out,
                             void const*          lhs,
                             void const*          rhs,
    ...
    void execute_reduction(reduction_t const&  r,
                           cudaStream_t        stream,
                           void*               out,
    ...
    vector<cutensorPlan_t> make_elementwise_plans(list_simple_scalarop_t const& sops,
                                                  vector<uint64_t> const&       join_shape,
                                                  vector<vector<int>> const&    inns,
                                                  int                           out_rank) const;

    cutensorPlan_t sop_scale_plan(simple_scalarop_t::scale_t const& op,
                                  vector<int> const&                inn_idxs,
                                  vector<uint64_t> const&           out_shape) const;

    cutensorPlan_t sop_scale_add_plan(scalar_t const&         scale,
                                      vector<int> const&      inn_idxs_,
                                      vector<uint64_t> const& out_shape_) const;

    cutensorPlan_t sop_scale_mul_plan(scalar_t const&         scale,
                                      vector<int> const&      inn_idxs,
                                      vector<uint64_t> const& out_shape) const;

    cutensorPlan_t sop_unary_plan(simple_scalarop_t::unary_t const& op,
                                  vector<int> const&                inn_idxs,
                                  vector<uint64_t> const&           out_shape) const;

    cutensorPlan_t sop_binary_plan(simple_scalarop_t::binary_t const& op,
                                   vector<int> const&                 lhs_idxs,
                                   vector<int> const&                 rhs_idxs,
                                   vector<uint64_t> const&            out_shape) const;

    cutensorPlan_t sop_binary_plan_different_shape(simple_scalarop_t::binary_t const& op,
                                                   vector<int> const&                 lhs_idxs,
                                                   vector<int> const&                 rhs_idxs,
                                                   vector<uint64_t> const& out_shape) const;

    void execute_sop_scale(simple_scalarop_t::scale_t const& op,
                           cudaStream_t                      stream,
                           void*                             out_mem,
                           void const*                       inn_mem,
                           cutensorPlan_t                    plan) const;

    void execute_sop_scale_add(scalar_t const& scale,
                               cudaStream_t    stream,
                               void*           out_mem,
                               void const*     inn_mem,
                               cutensorPlan_t  plan) const;

    void execute_sop_scale_mul(scalar_t const& scale,
                               cudaStream_t    stream,
                               void*           out_mem,
                               void const*     inn_mem,
                               cutensorPlan_t  plan) const;

    void execute_sop_unary(simple_scalarop_t::unary_t const& op,
                           cudaStream_t                      stream,
                           void*                             out_mem,
                           void const*                       inn_mem,
                           cutensorPlan_t                    plan) const;

    void execute_sop_binary(simple_scalarop_t::binary_t const& op,
                            cudaStream_t                       stream,
                            void*                              out_mem,
                            void const*                        lhs_mem,
                            void const*                        rhs_mem,
    ...
    void execute_sop_binary_different_shape(simple_scalarop_t::binary_t const& op,
                                            cudaStream_t                       stream,
                                            void*                              out_mem,
                                            void const*                        lhs_mem,
                                            void const*                        rhs_mem,
    ...
    void execute_elementwise(elementwise_t const& op,
                             cudaStream_t         stream,
                             void*                out,
                             vector<void const*>  inns,
                             void*                work_mem,
...
public:
    ...
    void operator()(touch_t const& touch, cudaStream_t stream, void* out, void const* inn) const;

    void operator()(einsummable_t const&             e,
                    cudaStream_t                     stream,
                    void*                            out,
                    vector<void const*>              inns,
                    optional<tuple<void*, uint64_t>> workspace = std::nullopt) const;

    void operator()(list_simple_scalarop_t const&    sop,
                    cudaStream_t                     stream,
                    void*                            out,
                    void const*                      inns,
                    optional<tuple<void*, uint64_t>> workspace = std::nullopt) const;
...
private:
    using kernel_info_t = std::variant<matmul_t,
                                       contraction_t,
                                       reduction_t,
                                       elementwise_t,
                                       type_conversion_t,
                                       pow_and_elementwise_t,
                                       custom_kernel_1_t,
                                       power_t,
                                       scale_t,
                                       custom_kernel_2_t,
    ...
    void call(kernel_info_t const&             kernel,
              cudaStream_t                     stream,
              void*                            out,
              vector<void const*>              inns,
