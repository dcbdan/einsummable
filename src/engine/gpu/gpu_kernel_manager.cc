...
kernel_manager_t::kernel_manager_t()
    : kernel_manager_t(0){DOUT("!!! Note: Creating kernel manager without a device id !!!")}

      kernel_manager_t::kernel_manager_t(int device)
    : device(device)
{
...
{
    switch (uop) {
        case simple_scalarop_t::uop_t::identity:
            return CUTENSOR_OP_IDENTITY;
        case simple_scalarop_t::uop_t::neg:
            return CUTENSOR_OP_NEG;
        case simple_scalarop_t::uop_t::sqrt:
            return CUTENSOR_OP_SQRT;
        case simple_scalarop_t::uop_t::conj:
            return CUTENSOR_OP_CONJ;
        case simple_scalarop_t::uop_t::rcp:
            return CUTENSOR_OP_RCP;
        case simple_scalarop_t::uop_t::sigmoid:
            return CUTENSOR_OP_SIGMOID;
        case simple_scalarop_t::uop_t::log:
            return CUTENSOR_OP_LOG;
        case simple_scalarop_t::uop_t::exp:
            return CUTENSOR_OP_EXP;
        case simple_scalarop_t::uop_t::relu:
            return CUTENSOR_OP_RELU;
        // default: return CUTENSOR_OP_IDENTITY;
        default:
            throw std::runtime_error("uop_to_cutensorOp: Unknown uop");
    }
}
...
{
    switch (bop) {
        case simple_scalarop_t::bop_t::add:
            return CUTENSOR_OP_ADD;
        case simple_scalarop_t::bop_t::mul:
            return CUTENSOR_OP_MUL;
        case simple_scalarop_t::bop_t::min:
            return CUTENSOR_OP_MIN;
        case simple_scalarop_t::bop_t::max:
...
// example:
// Found custom kernel 4: es[32768,4096]+ ab,ab,a->a |
// *[hole|f32@0,*[hole|f32@1,*[constant{f32|-1},power{-2}[hole|f32@2]]]] join_shape: {32768,4096}
// inns: {{0,1},{0,1},{0}}
// out_rank: 1
...
// example:
// Found custom kernel 5: es[32768]: a,a->a |
// +[*[constant{f32|0.999},hole|f32@0],*[constant{f32|0.000999987},power{2}[hole|f32@1]]]
// join_shape: {32768}
// inns: {{0},{0}}
// out_rank: 1
...
bool kernel_manager_t::is_custom_kernel5(einsummable_t e)
{
    ...
    if (op_str.size() >= 11) {
        ...
        if (op_str.substr(op_str.size() - 11, 11) == "pow(x1,2)))" &&
            op_str.substr(0, 5) == "((f32") {
...
bool kernel_manager_t::is_large_workspace2(einsummable_t e)
{
    ...
    scalarop_t compare =
        parse_with_ss<scalarop_t>("*[*[hole|f32@0,power{-1}[hole|f32@1]],hole|f32@2]");
    if (e.join == compare) {
        return true;
    }
    return false;
...
bool kernel_manager_t::is_softmax_v3_elementwise(einsummable_t e)
{
    ...
    if (op == parse_with_ss<scalarop_t>("*[exp[*[constant{f32|0.0883883},+[hole|f32@0,*[constant{"
                                        "f32|-1},hole|f32@1]]]],power{-1}[hole|f32@2]]")) {
...
kernel_manager_t::custom_kernel_4_t kernel_manager_t::build_custom_kernel4(einsummable_t const& e)
{
    ...
    elementwise_t elementwise{.sops = sop.value(),
...
bool kernel_manager_t::is_c64_elementwise_multiply(einsummable_t e)
{
    if (e.inns.size() == 2) {
        ...
        if (op_str == "(x0*x1)" && e.inn_dtype(0) == dtype_t::c64 &&
            e.inn_dtype(1) == dtype_t::c64) {
...
optional<workspace_info_t> kernel_manager_t::build(einsummable_t const& e_)
{
    cudaSetDevice(device);
    ...
    if (einsummable.has_aggregation()) {
        ...
        if (einsummable.inns.size() != 1) {
            ...
            // }
                ...
                scalar_t               s1(*reinterpret_cast<const float*>(bytes.data()));
                float                  scale = s1.f32();
                v3_softmax_reduction_t v3_softmax{.a = einsummable.join_shape[0],
                                                  .b = einsummable.join_shape[1],
                                                  .constant = scale};
                kernels.insert({einsummable, v3_softmax});
                return workspace_info_t(0);
            } else {
                DOUT("Error: Reduction with more than one input tensor. Einsummable: "
                     << einsummable);
                return std::nullopt;
            }
        }
        ...
        if (!einsummable.join.is_identity()) {
            ...
            // at least I think
            // (see this:
            // https://docs.nvidia.com/cuda/cutensor/latest/api/cutensor.html#reduction-operations)
            // reduction_t reduct = make_reduction_negate(einsummable);

            special_negateSum_reduction_t reduct{.a = einsummable.join_shape[0],
                                                 .b = einsummable.join_shape[1]};
    ...
    if (is_scale_and_increment(einsummable)) {
        ...
        uint64_t size = einsummable.join_shape[0];
        auto lambda = [scale, increment, size](cudaStream_t stream, float* out, const float* in) {
    ...
    if (is_elementwise_with_pow(einsummable)) {
        ...
        if (worksize > 100 * 1024 * 1024) {
            DOUT("NOTE: Elementwise with pow workspace size: " << worksize << " for "
                                                               << einsummable);
        }
    ...
    if (is_c64_elementwise_multiply(einsummable)) {
        ...
        elementwise_t kernel{
            .sops = list,
            .plans = make_elementwise_plans(
                list, einsummable.join_shape, einsummable.inns, einsummable.out_rank),
    ...
    if (is_custom_kernel5(einsummable)) {
        ...
        elementwise_t kernel{
            .sops = sop.value(),
            .plans = make_elementwise_plans(
                sop.value(), einsummable.join_shape, einsummable.inns, einsummable.out_rank),
            .join_shape = einsummable.join_shape,
            .inns = einsummable.inns,
            .out_rank = einsummable.out_rank};
        kernels.insert({einsummable, kernel});
    ...
    if (is_softmax_v3_elementwise(einsummable)) {
        ...
        v3_softmax_elementwise_t v3_softmax{
            .a = einsummable.join_shape[0], .b = einsummable.join_shape[1], .constant = scale};
        kernels.insert({einsummable, v3_softmax});
        return workspace_info_t(0);
    }
    ...
    optional<list_simple_scalarop_t> maybe_sops = list_simple_scalarop_t::make(einsummable.join);
    if (maybe_sops) {
        ...
        elementwise_t kernel{
            .sops = sops,
            .plans = make_elementwise_plans(
                sops, einsummable.join_shape, einsummable.inns, einsummable.out_rank),
            .join_shape = einsummable.join_shape,
            .inns = einsummable.inns,
            .out_rank = einsummable.out_rank};
    ...
    if (is_custom_kernel2(einsummable)) {
        ...
        if (lhs[0] != rhs[0]) {
            throw std::runtime_error(
                "custom kernel 2 only supports form of ab,a->ab; shape mismatch");
        }
        ...
        custom_kernel_2_t custom_kernel{
            .nrows = nrows, .ncols = ncols, .dtype = einsummable.inn_dtype(0)};
...
workspace_info_t
kernel_manager_t::workspace_size(kernel_manager_t::kernel_info_t const& kernel) const
{
...
void kernel_manager_t::operator()(touch_t const& touch,
                                  cudaStream_t   stream,
                                  void*          out,
...
void kernel_manager_t::operator()(einsummable_t const&             e,
                                  cudaStream_t                     stream,
                                  void*                            out,
                                     ...
                                     void*                     out) const
{
    ...
    cudaSetDevice(device);
    fillTri_dispatch(out,
                     l.nrow,
                     l.ncol,
                     l.start,
                     *reinterpret_cast<const uint64_t*>(l.lower.data),
                     *reinterpret_cast<const uint64_t*>(l.upper.data),
                     stream,
                     type);
}
                                     ...
                                     void*                     out) const
{
    ...
    // print value
    // printf("value: %f\n", *(float*)(c.value.raw()));
    fill_constant_dispatch(
        out, num_elements, *reinterpret_cast<const uint64_t*>(c.value.data), stream, type);
}

void kernel_manager_t::call(kernel_manager_t::kernel_info_t const& kernel,
                            cudaStream_t                           stream,
                            void*                                  out,
                            vector<void const*>                    inns,
                            optional<tuple<void*, uint64_t>>       maybe_workspace) const
{
    ...
    if (holds_alternative<matmul_t>(kernel)) {
        ...
        auto const& [dtype, ni, nj, nk, _0, _1, _2] = m;
        execute_matmul(m, stream, out, inns[0], inns[1]);
        ...
        execute_contraction(c, stream, out, inns[0], inns[1], workspace, wsz);
        ...
        conditional_assignment_dispatch(
            out, inns[0], nrows, ncols, inns[1], 1, 0, stream, dtype_info);
    } else if (holds_alternative<custom_kernel_4_t>(kernel)) {
        auto const& [elementwise,
                     reduction,
                     worksize,
                     elementwise_output_offset,
                     reduction_offset] = get<custom_kernel_4_t>(kernel);
        auto [workspace, wsz] = maybe_workspace.value();
        ...
        // reduction
        execute_reduction(reduction,
                          stream,
                          out,
                          {elementwise_output},
                          reduction_workspace,
                          wsz - elementwise_output_offset);
        ...
        softmax_v3_reduction_dispatch(out, inns[0], inns[1], a, b, constant, stream);
        ...
        softmax_v3_elementwise_dispatch(out, inns[0], inns[1], inns[2], a, b, constant, stream);
...
optional<kernel_manager_t::matmul_t> kernel_manager_t::make_matmul(einsummable_t const& e)
{
    ...
    matmul_t ret{.dtype = e.out_dtype(),
                 .ni = e.join_shape[0],
                 .nj = e.join_shape[2],
                 .nk = e.join_shape[1],
                 .trans_l = false,
...
kernel_manager_t::contraction_t kernel_manager_t::make_contraction(einsummable_t const& einsummable)
{
    ...
    handle_cutensor_error(cutensorCreateContraction(cutensor_handle,
                                                    &c.desc,
                                                    c.descA,
                                                    modeA.data(),
    ...
    handle_cutensor_error(
        cutensorCreatePlanPreference(cutensor_handle, &planPref, algo, CUTENSOR_JIT_MODE_NONE));

    uint64_t                           workspaceSizeEstimate = 0;
    const cutensorWorksizePreference_t workspacePref = CUTENSOR_WORKSPACE_DEFAULT;
    handle_cutensor_error(cutensorEstimateWorkspaceSize(
        cutensor_handle, c.desc, planPref, workspacePref, &workspaceSizeEstimate));
    ...
    cutensorStatus_t status =
        cutensorCreatePlan(cutensor_handle, &c.plan, c.desc, planPref, workspaceSizeEstimate);
    ...
    handle_cutensor_error(cutensorPlanGetAttribute(cutensor_handle,
                                                   c.plan,
...
void kernel_manager_t::execute_matmul(kernel_manager_t::matmul_t const& matmul,
                                      cudaStream_t                      stream,
                                      void*                             out,
                                      void const*                       lhs,
                                      void const*                       rhs) const
{
    ...
    if (dtype == dtype_t::f16) {
        ...
        cublasCgemm(cublas_handle,
                    ...
                    reinterpret_cast<cuComplex const*>(lhs),
                    ldl,
                    reinterpret_cast<cuComplex const*>(rhs),
                    ldr,
...
void kernel_manager_t::execute_contraction(kernel_manager_t::contraction_t const& c,
                                           cudaStream_t                           stream,
                                           void*                                  out,
                                           void const*                            lhs,
                                           void const*                            rhs,
                                           ...
                                           uint64_t given_worksize) const
{
    ...
    handle_cutensor_error(
        cutensorContract(cutensor_handle,
...
kernel_manager_t::reduction_t kernel_manager_t::make_reduction(einsummable_t const& e)
{
    ...
    handle_cutensor_error(cutensorCreateReduction(cutensor_handle,
                                                  &desc,
                                                  descA,
                                                  modeA.data(),
                                                  CUTENSOR_OP_IDENTITY,
    ...
    const cutensorWorksizePreference_t workspacePref = CUTENSOR_WORKSPACE_DEFAULT;
    handle_cutensor_error(cutensorEstimateWorkspaceSize(
        cutensor_handle, desc, planPref, workspacePref, &workspaceSizeEstimate));

    cutensorPlan_t plan;
    handle_cutensor_error(
        cutensorCreatePlan(cutensor_handle, &plan, desc, planPref, workspaceSizeEstimate));
    ...
    return reduction_t{.dtype = type, .worksize = worksize, .plan = plan

    };
}

// negate then reduce
kernel_manager_t::reduction_t kernel_manager_t::make_reduction_negate(einsummable_t const& e)
{
    // DOUT("reduction join: " << e.join);
    std::vector<int> modeA = e.inns[0];
    std::vector<int> modeC;
...
void kernel_manager_t::execute_reduction(kernel_manager_t::reduction_t const& r,
                                         cudaStream_t                         stream,
                                         void*                                out,
                                         vector<void const*>                  inns,
                                         void*                                work,
                                         uint64_t                             given_worksize) const
{
    ...
    handle_cutensor_error(cutensorReduce(cutensor_handle,
                                         r.plan,
                                         alpha,
                                         inns[0],
                                         (const void*)&beta,
...
static vector<int> _which_ew_memory(list_simple_scalarop_t const& sops)
{
...
uint64_t
kernel_manager_t::elementwise_workspace_size(kernel_manager_t::elementwise_t const& e) const
{
...
void kernel_manager_t::execute_elementwise(elementwise_t const& e,
                                           cudaStream_t         stream,
                                           void*                out_mem,
                                           ...
                                           uint64_t             given_worksize) const
{
    ...
    for (int which_sop = 0; which_sop != e.sops.ops.size(); ++which_sop) {
        ...
        void* this_out_mem =
            which_sop == e.sops.ops.size() - 1 ? out_mem : get_workspace_at(which_sop);

        if (sop.is_scale()) {
            execute_sop_scale(
                sop.get_scale(), stream, this_out_mem, get_inn_memory_at(args[0]), plan);
            // cudaDeviceSynchronize();
            // DOUT("intermediate output scale: ");
            // printFloatGPU(this_out_mem, 32);
        } else if (sop.is_unary()) {
            execute_sop_unary(
                sop.get_unary(), stream, this_out_mem, get_inn_memory_at(args[0]), plan);
            // cudaDeviceSynchronize();
            ...
            if (lhs_idxs == out_idxs && rhs_idxs != out_idxs) {
                ...
                execute_sop_binary(sop.get_binary(),
                                   stream,
                                   this_out_mem,
                                   get_inn_memory_at(args[0]),
                                   get_inn_memory_at(args[1]),
                                   plan,
                                   true);
                ...
                special_elementwise_mul_dispatch(this_out_mem,
                                                 e.join_shape[0],
                                                 e.join_shape[1],
                                                 get_inn_memory_at(args[0]),
                                                 get_inn_memory_at(args[1]),
                ...
                execute_sop_binary_different_shape(sop.get_binary(),
                                                   stream,
                                                   this_out_mem,
                                                   get_inn_memory_at(args[0]),
                                                   get_inn_memory_at(args[1]),
...
vector<cutensorPlan_t> kernel_manager_t::make_elementwise_plans(list_simple_scalarop_t const& sops,
                                                                ...
                                                                int out_rank) const
{
    ...
    for (int which_sop = 0; which_sop != sops.ops.size(); ++which_sop) {
        ...
        if (sop.is_scale()) {
            ret.push_back(sop_scale_plan(sop.get_scale(), get_inn_idxs_at(args[0]), join_shape));
            // DOUT("NOTE: Building SCALE ELEMENTWISE");
        } else if (sop.is_unary()) {
            ret.push_back(sop_unary_plan(sop.get_unary(), get_inn_idxs_at(args[0]), join_shape));
            ...
            if (lhs_idxs == out_idxs || rhs_idxs == out_idxs) {
                ...
                ret.push_back(sop_binary_plan(sop.get_binary(),
                                              get_inn_idxs_at(args[0]),
                                              get_inn_idxs_at(args[1]),
                                              join_shape));
                // DOUT("NOTE: Building BINARY ELEMENTWISE");
                // TODO: we don't need a plan for special elementwise but we need to take the space
                // for it so it doesn't go out of bounds
            } else {
                // DOUT("NOTE: Building TERNARY ELEMENTWISE")
                ret.push_back(sop_binary_plan_different_shape(sop.get_binary(),
                                                              get_inn_idxs_at(args[0]),
                                                              get_inn_idxs_at(args[1]),
                                                              join_shape));
            }
...
cutensorPlan_t kernel_manager_t::sop_scale_plan(simple_scalarop_t::scale_t const& op,
...
cutensorPlan_t kernel_manager_t::sop_scale_add_plan(scalar_t const&         scale,
                                                    ...
                                                    vector<uint64_t> const& out_shape_) const
{
    ...
    simple_scalarop_t::binary_t binary{
        .op = simple_scalarop_t::bop_t::add, .lhs = unary, .rhs = unary};

    return sop_binary_plan(binary, lhs_idxs, rhs_idxs, out_shape);
}

cutensorPlan_t kernel_manager_t::sop_scale_mul_plan(scalar_t const&         scale,
                                                    vector<int> const&      inn_idxs,
                                                    vector<uint64_t> const& out_shape) const
{
    ...
    handle_cutensor_error(cutensorCreateTensorDescriptor(
        cutensor_handle, &descA, nmodeA, extent_A.data(), nullptr /* stride */, typeA, kAlignment));

    cutensorTensorDescriptor_t descC;
    handle_cutensor_error(cutensorCreateTensorDescriptor(
        cutensor_handle, &descC, nmodeA, extent_A.data(), nullptr /* stride */, typeA, kAlignment));
    ...
    cutensorPlan_t plan;
    handle_cutensor_error(
        cutensorCreatePlan(cutensor_handle, &plan, desc, planPref, 0 /* workspaceSizeLimit */));
    return plan;
}

cutensorPlan_t kernel_manager_t::sop_unary_plan(simple_scalarop_t::unary_t const& op,
                                                vector<int> const&                inn_idxs,
                                                vector<uint64_t> const&           out_shape) const
{
    uint32_t const kAlignment = 128;
    ...
    if (uop == simple_scalarop_t::uop_t::square) {
        simple_scalarop_t::unary_t  unit_unary{.scale = scalar_t::one(type),
                                               .op = simple_scalarop_t::uop_t::identity};
        simple_scalarop_t::unary_t  scale_unary{.scale = scale,
                                                .op = simple_scalarop_t::uop_t::identity};
        simple_scalarop_t::binary_t new_binary{
            .op = simple_scalarop_t::bop_t::mul, .lhs = scale_unary, .rhs = unit_unary};

        return sop_binary_plan(new_binary, inn_idxs, inn_idxs, out_shape);
    }
...
cutensorPlan_t kernel_manager_t::sop_binary_plan(simple_scalarop_t::binary_t const& op,
                                                 ...
                                                 vector<uint64_t> const&            out_shape) const
{
    ...
    cutensorTensorDescriptor_t descC;
    handle_cutensor_error(cutensorCreateTensorDescriptor(
        cutensor_handle, &descC, nmodeC, extent_C.data(), nullptr /* stride */, typeC, kAlignment));

    cutensorOperationDescriptor_t desc;

    handle_cutensor_error(cutensorCreateElementwiseBinary(cutensor_handle,
                                                          &desc,
                                                          descA,
                                                          modeA.data(),
                                                          opElemmentwise_lhs,
...
cutensorPlan_t
kernel_manager_t::sop_binary_plan_different_shape(simple_scalarop_t::binary_t const& op,
                                                  vector<int> const&                 lhs_idxs,
                                                  vector<int> const&                 rhs_idxs,
                                                  vector<uint64_t> const& out_shape) const
{
    ...
    cutensorTensorDescriptor_t descB;
    handle_cutensor_error(cutensorCreateTensorDescriptor(
        cutensor_handle, &descB, nmodeB, extent_B.data(), nullptr /* stride */, typeB, kAlignment));
    ...
    cutensorOperationDescriptor_t desc;
    handle_cutensor_error(
        cutensorCreateElementwiseTrinary(cutensor_handle,
                                         &desc,
                                         descA,
                                         modeA.data(),
                                         /* unary operator A */ opElemmentwise_lhs,
                                         descB,
                                         modeB.data(),
                                         /* unary operator B */ opElemmentwise_rhs,
                                         descC,
                                         modeC.data(),
    ...
    cutensorPlan_t plan;
    handle_cutensor_error(
        cutensorCreatePlan(cutensor_handle, &plan, desc, planPref, 0 /*workspaceSizeEstimate*/));

    return plan;
}

void kernel_manager_t::execute_sop_scale(simple_scalarop_t::scale_t const& op,
                                         cudaStream_t                      stream,
                                         void*                             out_mem,
                                         void const*                       inn_mem,
...
void kernel_manager_t::execute_sop_scale_add(scalar_t const& scale,
                                             cudaStream_t    stream,
                                             void*           out_mem,
                                             void const*     inn_mem,
                                             cutensorPlan_t  plan) const
...
void kernel_manager_t::execute_sop_scale_mul(scalar_t const& scale,
                                             cudaStream_t    stream,
                                             void*           out_mem,
                                             void const*     inn_mem,
                                             cutensorPlan_t  plan) const
{
    void const* alpha = scale.raw();
    handle_cutensor_error(
        cutensorPermute(cutensor_handle, plan, alpha, inn_mem, out_mem, stream /* stream */));
}

void kernel_manager_t::execute_sop_unary(simple_scalarop_t::unary_t const& op,
                                         cudaStream_t                      stream,
                                         void*                             out_mem,
                                         void const*                       inn_mem,
                                         cutensorPlan_t                    plan) const
{
    ...
    if (uop == simple_scalarop_t::uop_t::square) {
        ...
        return execute_sop_binary(new_binary, stream, out_mem, inn_mem, inn_mem, plan, false);
    }
    void const* alpha = scale.raw();
    handle_cutensor_error(
        cutensorPermute(cutensor_handle, plan, alpha, inn_mem, out_mem, stream /* stream */));
...
void kernel_manager_t::execute_sop_binary(simple_scalarop_t::binary_t const& op,
                                          cudaStream_t                       stream,
                                          void*                              out_mem,
                                          void const*                        lhs_mem,
                                          void const*                        rhs_mem,
                                          ...
                                          bool                               swapped) const
{
    ...
    handle_cutensor_error(cutensorElementwiseBinaryExecute(
        cutensor_handle, plan, alpha, lhs_mem, beta, rhs_mem, out_mem, stream));
}

// we execute sop binary with different shapes such as
// ab,bc->abc, then join op is f(x,y,z) = binary_op(x,y) + 0*z
// here we assume
void kernel_manager_t::execute_sop_binary_different_shape(simple_scalarop_t::binary_t const& op,
                                                          cudaStream_t                       stream,
                                                          void*          out_mem,
                                                          void const*    lhs_mem,
                                                          void const*    rhs_mem,
                                                          ...
                                                          bool           swapped) const
{
    ...
    handle_cutensor_error(cutensorElementwiseTrinaryExecute(
        cutensor_handle, plan, alpha, lhs_mem, beta, rhs_mem, gamma, out_mem, out_mem, stream));
}
