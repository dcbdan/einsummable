...
host_buffer_holder_t::host_buffer_holder_t(uint64_t size) : size(size)
{
    ...
    handle_cuda_error(
        cudaHostAlloc(&data, size, cudaHostAllocPortable | cudaHostAllocWriteCombined),
        "host buffer construction");
}

host_buffer_holder_t::~host_buffer_holder_t()
{
    handle_cuda_error(cudaFreeHost(data), "host buffer destructor");
}
