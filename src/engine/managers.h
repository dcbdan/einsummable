...
struct global_buffers_t : rm_template_t<int, void*> {
    global_buffers_t(void* ptr) : global_buffers_t(vector<void*>{ptr}) {}

    global_buffers_t(vector<void*> ps) : ptrs(ps) {}

    static desc_ptr_t make_desc()
    {
        return rm_template_t::make_desc(0);
    }
...
struct group_manager_t : rm_template_t<int, tuple<int, bool>> {
    group_manager_t() {}
...
struct threadpool_resource_t {
    threadpool_resource_t(int i, threadpool_manager_t* s) : id(i), self(s) {}
...
struct threadpool_manager_t : rm_template_t<unit_t, threadpool_resource_t> {
