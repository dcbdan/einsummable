...
struct datamap_manager_t : rm_template_t<vector<tuple<int, uint64_t>>, vector<buffer_t>> {
    datamap_manager_t(map<int, buffer_t>& data) : data(data) {}
...
private:
    optional<vector<buffer_t>> try_to_acquire_impl(vector<tuple<int, uint64_t>> const& info)
    {
...
struct rp_touch_t : exec_graph_t::op_base_t {
    rp_touch_t(touch_t const& t, int out_tid, int b) : touch(t), inn_tid(b)
    {
        if (touch.castable) {
            throw std::runtime_error("not supporting aggregation in repartition graph");
        }
        uint64_t out_size = dtype_size(touch.dtype) *
...
struct rp_send_t : exec_graph_t::op_base_t {
    rp_send_t(int a, int b, int c) : src_tid(a), dst_tid(b), dst(c) {}
...
struct rp_recv_t : exec_graph_t::op_base_t {
    rp_recv_t(int a, uint64_t b, int c) : dst_tid(a), size(b), src(c) {}
    ...
    {
        return resource_manager_t::make_desc(
            vector<desc_ptr_t>{datamap_manager_t::make_desc(dst_tid, size),
...
exec_graph_t create_repartition_execgraph(int this_rank, taskgraph_t const& taskgraph)
{
    ...
    for (auto const& tid : taskgraph.get_order()) {
        ...
        if (node.op.is_input()) {
            ...
            if (src == this_rank) {
                ...
                int send_eid =
                    graph.insert(op_ptr_t(new rp_send_t(inn_tid, tid, dst)), {recv_ready_eid});
            } else if (dst == this_rank) {
                int recv_ready_eid = graph.insert(op_ptr_t(new notify_recv_ready_t(tid, src)), {}
                                                  // no deps: always ready to recv
                );

                int recv_eid =
                    graph.insert(op_ptr_t(new rp_recv_t(tid, size, src)), {recv_ready_eid});
...
rm_ptr_t create_repartition_resource_manager(communicator_t&     communicator,
                                             map<int, buffer_t>& data,
...
void repartition(communicator_t&          comm,
                 ...
                 threadpool_t*            threadpool)
{
    ...
    _update_map_with_new_tg_inns(data, remap_gid, gid_to_inn, _remap, this_rank);
    ...
    _update_map_with_new_tg_outs(data, remap_gid, gid_to_out, _remap, this_rank);
}
