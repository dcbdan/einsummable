...
//   recv: recv peers > rank
//   setup endpoints between rank and >rank
communicator_t::communicator_t(string addr_zero, bool is_server, int world_size_, int n_channels)
    : world_size(world_size_), is_listening(false)
{
    ...
    if (world_size > 1) {
        ...
        start_listen_notify(sizeof(_notify_processor_t::msg_t), [&](vector<uint8_t> data) {
...
void communicator_t::send_parallel(threadpool_t& threadpool,
                                   int           dst,
                                   void const*   data,
                                   uint64_t      size)
{
...
void communicator_t::recv_parallel(threadpool_t& threadpool, int src, void* data, uint64_t size)
{
...
std::future<void> communicator_t::send_async(int dst, int channel, void const* data, uint64_t size)
{
...
std::future<void> communicator_t::recv_async(int src, int channel, void* data, uint64_t size)
{
    std::promise<void> barrier_;
    std::future<void>  ret = barrier_.get_future();
    std::thread        t(
...
void communicator_t::start_listen_notify(uint64_t                                  msg_size,
        ...
        listen_thread = std::thread([this, msg_size, callback, &recvs] {
            ...
            while (listeners.size() > 0) {
                ...
                for (int n = 0; n < nfds; ++n) {
                    auto* listener =
                        static_cast<communicator_t::wire_t::listener_t*>(events[n].data.ptr);
...
void communicator_t::_setup_context()
{
    ...
    ucp_params.field_mask = UCP_PARAM_FIELD_FEATURES | UCP_PARAM_FIELD_NAME |
                            UCP_PARAM_FIELD_MT_WORKERS_SHARED | UCP_PARAM_FIELD_ESTIMATED_NUM_PPN;
    ucp_params.name = "c04";
    ...
    handle_ucs_error(ucp_init(&ucp_params, NULL, &ucp_context));

    ucp_context_attr_t attr;
    memset(&attr, 0, sizeof(attr));
    attr.field_mask = UCP_ATTR_FIELD_THREAD_MODE;
    handle_ucs_error(ucp_context_query(ucp_context, &attr));
...
vector<communicator_t::addr_data_t> communicator_t::_oob_recv_addrs(int oob_sock)
{
...
void communicator_t::_oob_send_addrs(int oob_sock, vector<addr_data_t>& addr)
{
    ...
    for (auto& a : addr) {
        ...
        int x = write(oob_sock, a.raw(), msg_size);
...
{
    int x = write(oob_sock, &rank, sizeof(int));
...
vector<communicator_t::addr_data_t> communicator_t::_wire_recv_addrs(int src)
{
    int n;
...
communicator_t::_get_peers_less(vector<vector<communicator_t::addr_data_t>> const& all_addr,
                                int                                                this_rank)
{
    vector<addr_data_t> ret;
...
vector<communicator_t::addr_data_t>
communicator_t::_get_peers_more(vector<vector<communicator_t::addr_data_t>> const& all_addr,
                                int                                                this_rank)
{
    vector<addr_data_t> ret;
...
communicator_t::addr_data_t communicator_t::_notify_processor_t::msg_t::get_addr() const
{
    addr_data_t ret;
...
communicator_t::wire_t::wire_t(communicator_t* self) : has_endpoint(false)
{
    ...
    // create the worker
    {
        ...
        handle_ucs_error(ucp_worker_create(self->ucp_context, &worker_params, &worker));
    }
    ...
    // create the addr; check the
    {
        ...
        worker_attr.field_mask = UCP_WORKER_ATTR_FIELD_ADDRESS | UCP_WORKER_ATTR_FIELD_THREAD_MODE;
...
void communicator_t::wire_t::create_endpoint(communicator_t::addr_data_t const& addr)
{
    ...
    handle_ucs_error(ucp_ep_create(worker, &ep_params, &endpoint));
    has_endpoint = true;
}
...
void communicator_t::wire_t::send(void const* data, uint64_t size)
{
    ...
    if (status != NULL) {
        if (UCS_PTR_IS_ERR(status)) {
            throw std::runtime_error("send fail: " + write_with_ss(UCS_PTR_STATUS(status)));
        }
...
void communicator_t::wire_t::recv(void* data, uint64_t size)
{
    ...
    if (status != NULL) {
        if (UCS_PTR_IS_ERR(status)) {
            throw std::runtime_error("recv fail: " + write_with_ss(UCS_PTR_STATUS(status)));
        }
...
void communicator_t::wire_t::listener_t::start()
{
    ...
    request_status = ucp_stream_recv_nbx(
        self.endpoint, reinterpret_cast<void*>(data.data()), size, &size, &param);
