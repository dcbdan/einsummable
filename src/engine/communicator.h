...
struct communicator_t {
    ...
    void start_listen_notify(uint64_t                             msg_size,
                             std::function<bool(vector<uint8_t>)> callback,
                             bool                                 constant_poll = false);

    template <typename T>
    void start_listen_notify_type(std::function<bool(T const&)> callback,
                                  bool                          constant_poll = false)
    {
        start_listen_notify(
            sizeof(T),
    ...
    void send_vector(int dst, vector<T> const& xs)
    {
        ...
        send(dst, reinterpret_cast<void const*>(xs.data()), n * sizeof(T));
    }
    template <typename T>
    vector<T> recv_vector(int src)
    {
        ...
        vector<T> ret(n);
        recv(src, reinterpret_cast<void*>(ret.data()), n * sizeof(T));
        return ret;
    }
    template <typename T>
...
private:
    ...
    struct wire_t {
        ...
        ucp_address_t* get_ucp_addr()
        {
            return ucp_addr;
        }
        uint64_t get_ucp_addr_size()
        {
            return ucp_addr_size;
        }
...
private:
    ...
    static vector<addr_data_t> _get_peers_less(vector<vector<addr_data_t>> const& all_addr,
                                               int                                rank);

    static vector<addr_data_t> _get_peers_more(vector<vector<addr_data_t>> const& all_addr,
                                               int                                rank);

    struct _notify_processor_t {
        _notify_processor_t(int n) : num_msg(n) {}

        struct msg_t {
            // TODO: hopefully this is big enough to store a ucx address
            static uint64_t const max_addr_size = 500;

            enum { recv_ep, send_ep, stop } ep;
            int      rank;
            int      channel;
            uint64_t addrsize;
        ...
        void wait()
        {
            when_done.get_future().get();
        }

        vector<msg_t>& get_recvd_messages()
        {
