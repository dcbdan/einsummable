...
struct data_manager_desc_t {
    ...
    data_manager_desc_t(int write_tid, vector<int> const& read_tids)
        : data_manager_desc_t(vector<int>{write_tid}, read_tids)
    {
    }

    data_manager_desc_t(vector<int> const& ws, vector<int> const& rs);
...
struct data_manager_resource_t {
    tuple<vector<void*>, vector<void const*>> extract() const
    {
...
private:
    data_manager_resource_t(vector<int> const&         out_tids_,
...
struct data_manager_t : rm_template_t<data_manager_desc_t, data_manager_resource_t> {
    struct info_t {
        int      usage_rem;
        bool     is_save;
        uint64_t size;
    ...
    data_manager_t(map<int, buffer_t>&     data,
                   map<int, info_t> const& infos,
                   uint64_t                max_memory_usage);

    optional<data_manager_resource_t> try_to_acquire_impl(data_manager_desc_t const& desc);
...
private:
    int& get_usage_rem(int tid)
    {
        return infos.at(tid).usage_rem;
    }
    uint64_t const& get_size(int tid) const
    {
        return infos.at(tid).size;
    }
};
