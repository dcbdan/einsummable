...
static vector<exec_graph_t::op_ptr_t> create_parallel_bmm(full_contraction_t const& c,
                                                          int                       num_threads,
                                                          ...
                                                          int                               rhs_tid)
{
    ...
    for (int which_b = 0; which_b != pd_b.num_parts(); ++which_b) {
        for (int which_i = 0; which_i != pd_i.num_parts(); ++which_i) {
            ret.emplace_back(new cpu_tg_batchmatmul_t(dinfos,
                                                      c.dtype,
                                                      c.nb,
                                                      pd_b.offset_at(which_b),
                                                      pd_b.size_at(which_b),
                                                      c.ni,
                                                      pd_i.offset_at(which_i),
                                                      pd_i.size_at(which_i),
                                                      c.nj,
                                                      c.nk,
                                                      c.trans_lhs,
                                                      c.trans_rhs,
...
exec_graph_t::make_cpu_tg_exec_graph(taskgraph_t const&           taskgraph,
                                     int                          this_rank,
                                     ...
                                     map<string, scalar_t> const& scalar_vars)
{
        ...
        for (auto const& node : taskgraph.nodes) {
            if (node.op.is_apply()) {
                einsummable_t e = node.op.get_apply()
                                      .einsummable.replace_scalar_variables(scalar_vars)
                                      .merge_adjacent_dims();
    ...
    auto get_new_tid = [&](uint64_t size) {
        _new_tid -= 1;
        dinfos.insert({_new_tid, dinfo_t{.usage_rem = 0, .is_save = false, .size = size}});
        return _new_tid;
    };
    int  _group_id = 0;
    auto new_group_id = [&] {
    ...
    auto permute_input_if_necc =
        [&](int inn_tid, dtype_t dtype, optional<full_contraction_t::permute_t> const& maybe) {
            if (maybe) {
                auto const&       p = maybe.value();
                uint64_t          size = dtype_size(dtype) * product(p.inn_shape);
                int               out_tid = get_new_tid(size);
                cpu_tg_permute_t* op =
                    new cpu_tg_permute_t(dinfos, dtype, p.inn_shape, p.out_perm, out_tid, inn_tid);
    ...
    for (int tid = 0; tid != taskgraph.nodes.size(); ++tid) {
        ...
        if (node.op.is_move()) {
            ...
            if (src == this_rank) {
                ...
                dinfos.insert(
                    {tid,
                     dinfo_t{.usage_rem = 0, .is_save = node.is_save, .size = node.op.out_size()}});
            } else {
    ...
    for (int const& tid : taskgraph.get_order()) {
        ...
        if (node.op.is_input()) {
            ...
            cpu_tg_fill_constant_t* op = new cpu_tg_fill_constant_t(dinfos, tid, fill);
            insert_from_tid(op_ptr_t(op), tid);
        } else if (node.op.is_apply()) {
            auto const&   apply = node.op.get_apply();
            einsummable_t e =
                apply.einsummable.replace_scalar_variables(scalar_vars).merge_adjacent_dims();

            if (e.is_contraction() && num_threads_per_contraction > 1) {
                dtype_t            dtype = e.out_dtype();
                full_contraction_t c =
                    full_contraction_t::make(dtype, e.join_shape, e.inns[0], e.inns[1], e.out_rank);
                ...
                if (c.perm_out) {
                    auto const&       p = c.perm_out.value();
                    cpu_tg_permute_t* op =
                        new cpu_tg_permute_t(dinfos, dtype, p.inn_shape, p.out_perm, tid, tmp_tid);
                ...
                cpu_tg_einsummable_t* op = new cpu_tg_einsummable_t(
                    dinfos, cpu_executor, e, tid, apply.inns, maybe_worksize.value());
            ...
            if (src == this_rank) {
                ...
                int ready_eid = graph.insert(
                    op_ptr_t(new exec_graph_t::wait_recv_ready_t(tid, dst)), {inn_eid});

                int send_eid =
                    graph.insert(op_ptr_t(new tg_send_t(dinfos, inn_tid, tid, dst)), {ready_eid});
            } else if (dst == this_rank) {
                int ready_eid =
                    graph.insert(op_ptr_t(new exec_graph_t::notify_recv_ready_t(tid, src)), {}
                                 // no deps: always ready to recv
                    );

                int recv_eid =
                    graph.insert(op_ptr_t(new tg_recv_t(dinfos, tid, size, src)), {ready_eid});
...
void cpu_tg_einsummable_t::launch(resource_ptr_t rsrc, std::function<void()> callback) const
{
...
void cpu_tg_batchmatmul_t::launch(resource_ptr_t rsrc, std::function<void()> callback) const
{
    vector<resource_ptr_t> const& resources = resource_manager_t::get_resource(rsrc);
    ...
    thread_resource.launch([this, callback, out_mem, lhs_mem, rhs_mem] {
        {
...
void cpu_tg_permute_t::launch(resource_ptr_t rsrc, std::function<void()> callback) const
{
    vector<resource_ptr_t> const& resources = resource_manager_t::get_resource(rsrc);
...
void cpu_tg_fill_constant_t::launch(resource_ptr_t rsrc, std::function<void()> callback) const
{
    vector<resource_ptr_t> const& resources = resource_manager_t::get_resource(rsrc);
...
desc_ptr_t cpu_tg_fill_constant_t::resource_description() const
{
    vector<desc_ptr_t> ret;
...
desc_ptr_t tg_send_t::resource_description() const
{
    return resource_manager_t::make_desc(
        vector<desc_ptr_t>{notifier_t::make_desc(unit_t{}),
...
{
    return resource_manager_t::make_desc(
        vector<desc_ptr_t>{data_manager_t::make_desc(dst_tid, vector<int>{}),
...
void tg_touch_t::launch(resource_ptr_t rsrc, std::function<void()> callback) const
{
    ...
    thread_resource.launch([this, callback, this_touch, out_mem, inn_mem] {
        {
            auto g = get_timetracker().make_totals_gremlin(
                "touch", product(vector_from_each_member(this_touch.selection, uint64_t, d_out)));
