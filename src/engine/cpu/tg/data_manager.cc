...
data_manager_desc_t::data_manager_desc_t(vector<int> const& ws, vector<int> const& rs)
...
data_manager_t::data_manager_t(map<int, buffer_t>&     data,
    ...
    : data(data), infos(infos), max_memory_usage(max_memory_usage)
{
    ...
    if (current_memory_usage > max_memory_usage) {
        std::cout << "current memory usage: " << current_memory_usage
                  << " max memory usage: " << max_memory_usage << std::endl;
...
data_manager_t::try_to_acquire_impl(data_manager_desc_t const& desc)
{
    ...
    data_manager_resource_t ret(out_tids,
                                inn_tids,
    ...
    if (current_memory_usage + mem_required > max_memory_usage) {
        throw std::runtime_error(
            "data_manger: waiting for memory to be free is disallowed"); // TODO remove
        return std::nullopt;
    }
...
void data_manager_t::release_impl(data_manager_resource_t const& resource)
{
    ...
    vector<int> used_tids = vector_concatenate(resource.out_tids, resource.inn_tids);
