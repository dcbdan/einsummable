...
full_contraction_t full_contraction_t::make(dtype_t                 dtype,
                                            vector<uint64_t> const& shape,
                                            vector<int> const&      lhs_inn_modes,
                                            vector<int> const&      rhs_inn_modes,
                                            int                     out_rank)
{
    auto maybe = contraction_t::make_bs_is_js_ks(lhs_inn_modes, rhs_inn_modes, out_rank);
    if (!maybe) {
    ...
    for (bool lhs_t : {false, true}) {
        for (bool rhs_t : {false, true}) {
            ...
            auto perm_lhs = contraction_t::permute_info_t::from_inn_shape(
                lhs_shape, lhs_inn_modes, plan.modes_lhs());
            auto perm_rhs = contraction_t::permute_info_t::from_inn_shape(
                rhs_shape, rhs_inn_modes, plan.modes_rhs());
            auto perm_out = contraction_t::permute_info_t::from_out_shape(
                out_shape, plan.modes_out(), out_modes);

            full_contraction_t f{.dtype = dtype,
            ...
            if (!perm_lhs.is_no_op()) {
                ...
                f.perm_lhs =
                    permute_t{.inn_shape = perm_lhs.inn_shape, .out_perm = perm_lhs.out_perm};
            }
            if (!perm_rhs.is_no_op()) {
                cost += product(rhs_shape);
                f.perm_rhs =
                    permute_t{.inn_shape = perm_rhs.inn_shape, .out_perm = perm_rhs.out_perm};
            }
            if (!perm_out.is_no_op()) {
                cost += product(out_shape);
                f.perm_out =
                    permute_t{.inn_shape = perm_out.inn_shape, .out_perm = perm_out.out_perm};
            }
...
void contraction_t::operator()(void* workspace, void* out, void const* lhs, void const* rhs) const
{
...
bool contraction_t::can_make(vector<int> const& lhs_inn_modes,
                             vector<int> const& rhs_inn_modes,
                             int                out_rank)
{
...
optional<tuple<vector<int>, vector<int>, vector<int>, vector<int>>>
contraction_t::make_bs_is_js_ks(vector<int> const& lhs_inn_modes,
                                vector<int> const& rhs_inn_modes,
                                int                out_rank)
{
    vector<int> bs;
    ...
    int join_rank = 1 + std::max(*std::max_element(lhs_inn_modes.begin(), lhs_inn_modes.end()),
                                 *std::max_element(rhs_inn_modes.begin(), rhs_inn_modes.end()));

    for (int i = 0; i != join_rank; ++i) {
        bool in_lhs =
            std::find(lhs_inn_modes.begin(), lhs_inn_modes.end(), i) != lhs_inn_modes.end();
        bool in_rhs =
            std::find(rhs_inn_modes.begin(), rhs_inn_modes.end(), i) != rhs_inn_modes.end();
...
contraction_t contraction_t::make(dtype_t                 dtype,
                                  vector<uint64_t> const& shape,
                                  vector<int> const&      lhs_inn_modes,
                                  vector<int> const&      rhs_inn_modes,
                                  int                     out_rank)
                                  ...
                                  contraction_t::batching_t const& plan)
{
    ...
    auto perm_lhs = permute_info_t::from_inn_shape(lhs_shape, lhs_inn_modes, plan.modes_lhs());
    auto perm_rhs = permute_info_t::from_inn_shape(rhs_shape, rhs_inn_modes, plan.modes_rhs());
    auto perm_out = permute_info_t::from_out_shape(out_shape, plan.modes_out(), out_modes);
    ...
    for (auto const& x : plan.bs) {
        total_b_elem *= shape[x];
    }
    uint64_t total_i_elem = 1;
    for (auto const& x : plan.is) {
        total_i_elem *= shape[x];
    }
    uint64_t total_j_elem = 1;
    for (auto const& x : plan.js) {
        total_j_elem *= shape[x];
    }
    uint64_t total_k_elem = 1;
    for (auto const& x : plan.ks) {
        total_k_elem *= shape[x];
    }
...
contraction_t::permute_info_t contraction_t::permute_info_t::drop_leading(int n) const
{
    ...
    return from_inn_shape(
        vector<uint64_t>(inn_shape.begin() + n, inn_shape.end()), inn_modes, out_modes);
}
...
uint64_t contraction_t::permute_info_t::leading_nelem() const
{
    return product(vector<uint64_t>(inn_shape.begin(), inn_shape.begin() + num_leading_modes()));
}
