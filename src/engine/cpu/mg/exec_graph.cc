...
exec_graph_t exec_graph_t::make_cpu_exec_graph(memgraph_t const&            memgraph,
                                               int                          this_rank,
                                               ...
                                               map<string, scalar_t> const& scalar_vars)
{
    ...
    for (int mid = 0; mid != memgraph.nodes.size(); ++mid) {
        ...
        if (node.op.is_inputmem() || node.op.is_inputsto() || node.op.is_partialize() ||
            node.op.is_alloc() || node.op.is_del()) {
            ...
            cpu_fill_constant_t* op = new cpu_fill_constant_t(constant.offset, fill);
            ...
            if (apply.is_einsummable()) {
                ...
                cpu_einsummable_t* op =
                    new cpu_einsummable_t(cpu_executor, e, apply.mems, maybe_worksize.value());

                // insert into the graph
                insert_from_mid(op_ptr_t(op), mid);
            } else if (apply.is_touch()) {
                cpu_touch_t* op =
                    new cpu_touch_t(cpu_executor, apply.get_touch(), apply.group, apply.mems);
            ...
            for (int i = 0; i != n_moves; ++i) {
                ...
                if (src == this_rank) {
                    ...
                    mem_t mem{.offset = src_offset + partial_offset, .size = partial_size};

                    int recv_ready_eid = graph.insert(
                        op_ptr_t(new exec_graph_t::wait_recv_ready_t(moveid, dst)), local_deps);

                    int send_eid = graph.insert(
                        op_ptr_t(new exec_graph_t::send_t(moveid, dst, mem)), {recv_ready_eid});

                    partial_move_eids.push_back(send_eid);
                } else if (dst == this_rank) {
                    // On this machine, we are recving.
                    mem_t mem{.offset = dst_offset + partial_offset, .size = partial_size};

                    int recv_ready_eid =
                        graph.insert(op_ptr_t(new notify_recv_ready_t(moveid, src)), local_deps);

                    int recv_eid =
                        graph.insert(op_ptr_t(new recv_t(moveid, src, mem)), {recv_ready_eid});
            ...
            cpu_evict_t* op = new cpu_evict_t(evict.dst.id, evict.src.as_mem());
            insert_from_mid(op_ptr_t(op), mid);
        } else if (node.op.is_load()) {
            auto const& load = node.op.get_load();
            cpu_load_t* op = new cpu_load_t(load.src.id, load.dst.as_mem());
            insert_from_mid(op_ptr_t(op), mid);
        } else {
            throw std::runtime_error("should not reach");
        }
...
desc_ptr_t cpu_einsummable_t::resource_description() const
{
...
void cpu_einsummable_t::launch(resource_ptr_t rsrc, std::function<void()> callback) const
{
    ...
    void* out_mem = increment_void_ptr(global_buffer, mems[0].offset);

    vector<void const*> inn_mems;
    inn_mems.reserve(mems.size() - 1);
    for (int i = 1; i != mems.size(); ++i) {
        inn_mems.push_back(increment_void_ptr(global_buffer, mems[i].offset));
    }
    ...
    thread_resource.launch([this, callback, out_mem, inn_mems, maybe_workspace] {
...
desc_ptr_t cpu_copy_t::resource_description() const
{
    vector<desc_ptr_t> ret;
    ret.emplace_back(global_buffers_t::make_desc());
    ret.emplace_back(threadpool_manager_t::make_desc());
...
void cpu_copy_t::launch(resource_ptr_t rsrc, std::function<void()> callback) const
{
    vector<resource_ptr_t> const& resources = resource_manager_t::get_resource(rsrc);
    ...
    void* inn_mem = reinterpret_cast<uint8_t const*>(increment_void_ptr(global_buffer, src_offset));

    void* out_mem = reinterpret_cast<uint8_t*>(increment_void_ptr(global_buffer, dst_offset));

    thread_resource.launch([this, callback, out_mem, inn_mem] {
...
desc_ptr_t cpu_touch_t::resource_description() const
{
    vector<desc_ptr_t> ret;
...
void cpu_fill_constant_t::launch(resource_ptr_t rsrc, std::function<void()> callback) const
{
    vector<resource_ptr_t> const& resources = resource_manager_t::get_resource(rsrc);
    ...
    thread_resource.launch([this, callback, out_mem] {
        initialize_fill(this->fill, out_mem);
        callback();
    });
}

desc_ptr_t cpu_fill_constant_t::resource_description() const
{
    vector<desc_ptr_t> ret;
...
void cpu_touch_t::launch(resource_ptr_t rsrc, std::function<void()> callback) const
{
    vector<resource_ptr_t> const& resources = resource_manager_t::get_resource(rsrc);
    ...
    void const* inn_mem = increment_void_ptr(global_buffer, mems[1].offset);
...
desc_ptr_t cpu_evict_t::resource_description() const
{
    vector<desc_ptr_t> ret;
...
void cpu_evict_t::launch(resource_ptr_t rsrc, std::function<void()> callback) const
{
    vector<resource_ptr_t> const& resources = resource_manager_t::get_resource(rsrc);

    void* ptr = increment_void_ptr(global_buffers_t::get_resource(resources[0]), mem.offset);

    cpu_storage_t* storage = cpu_storage_manager_t::get_resource(resources[1]).ptr;

    std::thread thread([this, callback, storage, ptr] {
        buffer_t data = make_buffer_reference(static_cast<uint8_t*>(ptr), mem.size);
...
desc_ptr_t cpu_load_t::resource_description() const
{
    vector<desc_ptr_t> ret;
...
void cpu_load_t::launch(resource_ptr_t rsrc, std::function<void()> callback) const
{
    vector<resource_ptr_t> const& resources = resource_manager_t::get_resource(rsrc);
