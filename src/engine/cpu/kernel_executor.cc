...
cpu_kernel_executor_t::cpu_kernel_executor_t()
{
    ...
    binfos = {{fix("ij,jk->ik"), {false, false, false, false, false}},
...
static void
reduce_exp_sub(uint64_t na, uint64_t nb, void* out_, void const* lhs_, void const* rhs_);

static void reduce_exp_subscale(uint64_t     na,
                                uint64_t     nb,
                                float const& alpha,
                                void*        out_,
                                void const*  lhs_,
...
optional<uint64_t> cpu_kernel_executor_t::build(einsummable_t const& e_)
{
    ...
    if (einsummable.is_permutation()) {
        ...
        kernels.insert({einsummable,
                        tensor_permute_t{.dtype = einsummable.out_dtype(),
    ...
    if (einsummable.is_straight_elementwise()) {
        ...
        if (n == 1) {
            ...
            if (maybe) {
                ...
                kernels.insert({einsummable,
                                unary_straight_ew_t{
                                    .n = product(einsummable.join_shape), .data = data, .f = f}});
                return 0;
            } else {
                return std::nullopt;
            }
    ...
    if (estr == "ab,a->ab" || estr == "ab,b->ab" || estr == "a,ab->ab" || estr == "b,ab->ab") {
        ...
        if (maybe) {
            auto const& [data, f] = maybe.value();
            kernels.insert({einsummable,
                            binary_212_ew_t{.na = einsummable.join_shape[0],
    ...
    if (estr == "ab,a->a") {
        ...
        if (einsummable.join == sub_then_exp && einsummable.castable.value() == castable_t::add) {
            uint64_t na = einsummable.join_shape[0];
            uint64_t nb = einsummable.join_shape[1];
            kernels.insert({einsummable, [na, nb](void* out, vector<void const*> inns) {
        ...
        if (match_key == key && einsummable.castable.value() == castable_t::add) {
            uint64_t na = einsummable.join_shape[0];
            uint64_t nb = einsummable.join_shape[1];
            kernels.insert({einsummable, [na, nb, bytes](void* out, vector<void const*> inns) {
    ...
    if (estr == "ab,a,a->ab") {
        ...
        if (maybe) {
            auto const& [data, f] = maybe.value();
            kernels.insert({einsummable,
                            ternary_2112_ew_t{.na = einsummable.join_shape[0],
                                              .nb = einsummable.join_shape[1],
                                              .data = data,
                                              .f = f}});
            return 0;
    ...
    if (estr == "ab->a") {
        ...
        if (maybe_scale && einsummable.castable.value() == castable_t::add) {
            ...
            kernels.insert({einsummable,
                            sum_then_scale_ab_a_t{.na = einsummable.join_shape[0],
                                                  .nb = einsummable.join_shape[1],
                                                  .value = value}});
            return 0;
        }
        ...
        auto reduction =
            build_ab_a_reduction_kernel(einsummable.out_dtype(), einsummable.castable.value());

        kernels.insert({einsummable,
                        reduction_ab_a_t{.na = einsummable.join_shape[0],
                                         .nb = einsummable.join_shape[1],
                                         .f = reduction}});
        return 0;
    }
    ...
    if (estr == "abcd,bd->abcd" && einsummable.join.out_dtype() == dtype_t::c64) {
        scalarop_t mulconj = scalarop_t::combine(
            ...
            {scalarop_t::make_identity(dtype_t::c64), scalarop_t::make_conjugate(dtype_t::c64)});
        ...
        if (einsummable.join.is_mul()) {
            ...
            kernel_t kernel = [na, nb, nc, nd](void* out, vector<void const*> inns) {
                ...
                return c64_mul_abcd_bd_to_abcd(na,
                                               nb,
                                               nc,
                                               nd,
            ...
            kernel_t kernel = [na, nb, nc, nd](void* out, vector<void const*> inns) {
                using T = std::complex<float>;
                return c64_mulconj_abcd_bd_to_abcd(na,
                                                   nb,
                                                   nc,
                                                   nd,
                                                   reinterpret_cast<T*>(out),
    ...
    if (estr == "ab,ab,a->a") {
        ...
        if (f == einsummable.join) {
            ...
            kernel_t kernel = [na, nb](void* out, vector<void const*> inns) {
                return custom01_float_ab_ab_a_to_a(na,
                                                   nb,
    ...
    if (einsummable.is_contraction()) {
        if (!contraction_t::can_make(
                einsummable.inns[0], einsummable.inns[1], einsummable.out_rank)) {
            return std::nullopt;
        }

        auto c = contraction_t::make(einsummable.out_dtype(),
...
void cpu_kernel_executor_t::operator()(touch_t const& touch, void* out, void const* inn) const
{
    // auto gremlin = cpu_kernel_timetracker.make_totals_gremlin("touch");
    execute_touch(touch.simplify(), out, inn);
}

void cpu_kernel_executor_t::operator()(einsummable_t const&             e,
                                       void*                            out,
...
void cpu_kernel_executor_t::call(cpu_kernel_executor_t::kernel_info_t const& kernel,
                                 void*                                       out,
                                 vector<void const*>                         inns,
...
cpu_kernel_executor_t::make_batch_matmul(einsummable_t const& e)
{
    ...
    return batch_matmul_t{
        .dtype = e.out_dtype(), .info = info, .nb = nb, .ni = ni, .nj = nj, .nk = nk};
}

std::function<void(void*, vector<void const*>)> build_einsummable(einsummable_t const& e)
{
...
#define _unary_ew_loop(name, TO, T, op)                                                            \
    void name(uint8_t const* d, uint64_t n, void* _out, void const* _x0)                           \
    {                                                                                              \
...
#define _binary_ew_loop(name1, name2, name3, TO, T0, T1, op)                                       \
    void name1(uint8_t const* d, uint64_t n, void* _out, void const* _x0, void const* _x1)         \
    {                                                                                              \
        TO*       out = reinterpret_cast<TO*>(_out);                                               \
    ...
    void name2(                                                                                    \
        uint8_t const* d, uint64_t n1, uint64_t n2, void* _out, void const* _x0, void const* _x1)  \
    {                                                                                              \
        TO*       out = reinterpret_cast<TO*>(_out);                                               \
        T0 const* x0 = reinterpret_cast<T0 const*>(_x0);                                           \
        T1 const* x1 = reinterpret_cast<T1 const*>(_x1);                                           \
...
#define _binary_ew_loop_mkl(name1, name2, name3, T, vecop)                                         \
    ...
    {                                                                                              \
        ...
        T const* x1 = reinterpret_cast<T const*>(_x1);                                             \
        for (uint64_t _i = 0; _i != n1; ++_i) {                                                    \
            vecop(n2, x0 + n2 * _i, 1, x1 + _i, 0, out + n2 * _i, 1);                              \
        }                                                                                          \
    }                                                                                              \
    void name3(                                                                                    \
        uint8_t const* d, uint64_t n1, uint64_t n2, void* _out, void const* _x0, void const* _x1)  \
    {                                                                                              \
        ...
        T const* x1 = reinterpret_cast<T const*>(_x1);                                             \
        for (uint64_t _i = 0; _i != n1; ++_i) {                                                    \
            vecop(n2, x0 + n2 * _i, 1, x1, 1, out + n2 * _i, 1);                                   \
        }                                                                                          \
    }
...
inline void u0(uint8_t const* d, uint64_t n, void* _out, void const* _x0)
{
    ...
    cblas_saxpy(
        n, alpha, reinterpret_cast<float const*>(_x0), 1, reinterpret_cast<float*>(_out), 1);
}

_unary_ew_loop(u1,
               float,
               float,
               (x0[i0] / ((*((float*)(d + 0))) + _exp(((*((float*)(d + 4))) * x0[i0])))))

    //_unary_ew_loop(u2,float,float,((*((float*)(d+0)))+((*((float*)(d+4)))*x0[i0])))
    inline void u2(uint8_t const* d, uint64_t n, void* _out, void const* _x0)
{
...
inline void u3(uint8_t const* d, uint64_t n, void* _out, void const* _x0)
{
    vsExpI(n, reinterpret_cast<float const*>(_x0), 1, reinterpret_cast<float*>(_out), 1);
}

//_unary_ew_loop(u4,float,float,x0[i0])
inline void u4(uint8_t const* d, uint64_t n, void* _out, void const* _x0)
{
    cblas_scopy(n, reinterpret_cast<float const*>(_x0), 1, reinterpret_cast<float*>(_out), 1);
}

//_unary_ew_loop(u5,float,float,_invsqrt(x0[i0]))
inline void u5(uint8_t const* d, uint64_t n, void* _out, void const* _x0)
{
    vsInvSqrtI(n, reinterpret_cast<float const*>(_x0), 1, reinterpret_cast<float*>(_out), 1);
}

//_unary_ew_loop(u6,float,float,_square(x0[i0]))
inline void u6(uint8_t const* d, uint64_t n, void* _out, void const* _x0)
{
    vsMulI(n,
           reinterpret_cast<float const*>(_x0),
           1,
           reinterpret_cast<float const*>(_x0),
           1,
...
_ternary_ew_loop(t1,
                 ...
                 float,
                 float,
                 (_exp(((*((float*)(d + 0))) * (x0[i0] - x1[i1]))) / x2[i2]));

optional<tuple<vector<uint8_t>, void (*)(uint8_t const*, uint64_t, void*, void const*)>>
...
optional<
    tuple<vector<uint8_t>, void (*)(uint8_t const*, uint64_t, void*, void const*, void const*)>>
lookup_binary_straight_ew_kernel(scalarop_t op)
{
    // TODO: this shouldn't have to happen as op should always be simplified
    //       to a unique value. For some reason
    //       a kernel wasn't normalized in the same way as the key
    ...
    using kernel_t = void (*)(uint8_t const*, uint64_t, void*, void const*, void const*);
...
optional<tuple<vector<uint8_t>,
               void (*)(uint8_t const*, uint64_t, void*, void const*, void const*, void const*)>>
lookup_ternary_straight_ew_kernel(scalarop_t op)
{
    // TODO: this shouldn't have to happen as op should always be simplified
    //       to a unique value. For some reason
    //       a kernel wasn't normalized in the same way as the key
...
lookup_binary_212_ew_kernel(scalarop_t op, bool is_ab_a)
{
    // TODO: this shouldn't have to happen as op should always be simplified
    //       to a unique value. For some reason
    //       a kernel wasn't normalized in the same way as the key
    ...
    using kernel_t = void (*)(uint8_t const*, uint64_t, uint64_t, void*, void const*, void const*);
...
lookup_ternary_2112_ew_kernel(scalarop_t op)
{
    // TODO: this shouldn't have to happen as op should always be simplified
    //       to a unique value. For some reason
    //       a kernel wasn't normalized in the same way as the key
...
template <typename T>
void naive_sgemm(uint64_t const& ni,
                 uint64_t const& nj,
                 uint64_t const& nk,
...
// F           T          ji,jk->ik
// T           T          ji,kj->ik
void matrix_multiply_update(dtype_t     dtype,
                            uint64_t    ni,
                            uint64_t    offset_i,
                            uint64_t    size_i,
                            uint64_t    nj,
...
=======
        ...
        cblas_hgemm(CblasRowMajor,
                    ...
                    nj,
                    one,
                    reinterpret_cast<f16_t const*>(lhs),
                    sl,
                    reinterpret_cast<f16_t const*>(rhs),
                    sr,
                    update ? one : zero,
...
void matrix_multiply(dtype_t     dtype,
                     uint64_t    ni,
                     uint64_t    offset_i,
                     uint64_t    size_i,
                     uint64_t    nj,
                     ...
                     void const* rhs)
{
    matrix_multiply_update(
        dtype, ni, offset_i, size_i, nj, nk, trans_lhs, trans_rhs, out, lhs, rhs, false);
}

// b<ij> , b<jk> -> b<ik>
void batch_matrix_multiply(dtype_t     dtype,
                           uint64_t    offset_b,
                           uint64_t    size_b,
                           bool        batched_out,
                           bool        batched_lhs,
...
void c64_mul_abcd_bd_to_abcd(uint64_t                   na,
                             uint64_t                   nb,
                             uint64_t                   nc,
                             uint64_t                   nd,
...
void c64_mulconj_abcd_bd_to_abcd(uint64_t                   na,
                                 uint64_t                   nb,
                                 uint64_t                   nc,
                                 uint64_t                   nd,
                                 std::complex<float>*       out,
...
void permute_kernel(dtype_t                 dtype,
...
template <typename T>
void _scale_kernel(T const& v, uint64_t n, void* _out)
{
    T* out = reinterpret_cast<T*>(_out);
    for (uint64_t i = 0; i != n; ++i) {
        out[i] *= v;
...
void scale_kernel(scalar_t const& v, uint64_t nelem, void* out)
{
...
// b->ab
void broadcast_b_ab_kernel(scalar_t const& scale,
                           uint64_t        nelem_a,
                           uint64_t        nelem_b,
                           void*           out,
                           void const*     inn)
{
    {
        ...
        for (uint64_t i = 0; i != nelem_a; ++i) {
            std::memcpy(reinterpret_cast<void*>(_out), inn, sz_b);
            _out += sz_b;
        }
    }
...
// a->ab
template <typename T>
void _broadcast_a_ab_kernel(uint64_t nelem_a, uint64_t nelem_b, T* out, T const* inn)
{
...
void broadcast_a_ab_kernel(scalar_t const& scalar,
                           uint64_t        nelem_a,
                           uint64_t        nelem_b,
                           void*           out,
                           void const*     inn)
{
    if (scalar.dtype == dtype_t::f16) {
        _broadcast_a_ab_kernel(nelem_a,
                               nelem_b,
...
template <typename T>
void _sum_then_scale_ab_a_kernel(T const& val, uint64_t n1, uint64_t n2, T* out, T const* inn)
{
    double total;
    for (uint64_t i = 0; i != n1; ++i) {
...
void sum_then_scale_ab_a_kernel(scalar_t    value,
                                uint64_t    nelem_a,
                                uint64_t    nelem_b,
                                void*       out,
                                void const* inn)
...
void custom01_float_ab_ab_a_to_a(uint64_t     na,
                                 uint64_t     nb,
                                 float*       out,
                                 float const* x0,
                                 float const* x1,
...
template <typename T>
void _lowertri_fill(T one, T zero, int64_t nrow, int64_t ncol, int64_t start, T* out)
{
    if (start >= nrow) {
...
void lowertri_fill(scalar_t one,
                   scalar_t zero,
                   int64_t  nrow,
                   int64_t  ncol,
                   int64_t  start,
                   void*    out)
{
    if (one.dtype == dtype_t::f16) {
        _lowertri_fill(one.f16(), zero.f16(), nrow, ncol, start, reinterpret_cast<float16_t*>(out));
    } else if (one.dtype == dtype_t::f32) {
        _lowertri_fill(one.f32(), zero.f32(), nrow, ncol, start, reinterpret_cast<float*>(out));
    } else if (one.dtype == dtype_t::f64) {
        _lowertri_fill(one.f64(), zero.f64(), nrow, ncol, start, reinterpret_cast<double*>(out));
    } else if (one.dtype == dtype_t::c64) {
        _lowertri_fill(
            one.c64(), zero.c64(), nrow, ncol, start, reinterpret_cast<std::complex<float>*>(out));
    } else {
...
void reduce_exp_sub(uint64_t na, uint64_t nb, void* out_, void const* lhs_, void const* rhs_)
{
