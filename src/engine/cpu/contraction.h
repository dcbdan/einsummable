...
struct full_contraction_t {
    ...
    static full_contraction_t make(dtype_t                 dtype,
...
private:
    ...
    struct permute_info_t {
        ...
        static permute_info_t
        from_inn_shape(vector<uint64_t> inn_shape, vector<int> inn_ord, vector<int> out_ord)
        {
            return permute_info_t{inn_shape, as_out_perm(inn_ord, out_ord)};
        }

        static permute_info_t
        from_out_shape(vector<uint64_t> out_shape, vector<int> inn_ord, vector<int> out_ord)
        {
...
public:
    ...
    void operator()(void* workspace, void* out, void const* lhs, void const* rhs) const;

    static bool
    can_make(vector<int> const& lhs_inn_modes, vector<int> const& rhs_inn_modes, int out_rank);

    static contraction_t make(dtype_t                 dtype,
                              vector<uint64_t> const& shape,
                              vector<int> const&      lhs_inn_modes,
                              vector<int> const&      rhs_inn_modes,
                              int                     out_rank);
...
private:
    struct batching_t {
        ...
        {
            return vector_concatenate(
                bs, lhs_t ? vector_concatenate(js, is) : vector_concatenate(is, js));
        }
        vector<int> modes_rhs() const
        {
            return vector_concatenate(
                bs, rhs_t ? vector_concatenate(ks, js) : vector_concatenate(js, ks));
        }
