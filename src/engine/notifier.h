...
struct notifier_t : rm_template_t<unit_t, notifier_resource_t> {
    // starts a notifier on comm
    notifier_t(communicator_t& comm, recv_channel_manager_t& recv_channel_manager);
...
private:
    ...
    struct msg_t {
        enum { recv_ready, send_ready, stop } msg_type;
        union {
            struct {
                int id;
            } recv_info;
