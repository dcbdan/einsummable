...
struct exec_graph_t {
    ...
    static exec_graph_t make_cpu_exec_graph(memgraph_t const&            memgraph,
                                            int                          this_rank,
    ...
    make_cpu_tg_exec_graph(taskgraph_t const&           taskgraph,
                           int                          this_rank,
                           cpu_kernel_executor_t&       cpu_executor,
    ...
    // note: the nodes of this graph may reference
    //       the provided executor
    static exec_graph_t make_gpu_exec_graph(memgraph_t const&            memgraph,
                                            int                          this_rank,
    ...
    struct node_t {
        ...
        inline desc_ptr_t resource_description() const
        {
            return op->resource_description();
        }
    ...
    struct notify_recv_ready_t : op_base_t {
        notify_recv_ready_t(int a, int b) : id(a), dst(b) {}
    ...
    struct wait_recv_ready_t : op_base_t {
        wait_recv_ready_t(int a, int b) : id(a), src(b) {}
    ...
    struct send_t : op_base_t {
        send_t(int a, int b, mem_t const& c) : id(a), dst(b), mem(c) {}
    ...
    struct recv_t : op_base_t {
        recv_t(int a, int b, mem_t const& c) : id(a), src(b), mem(c) {}
