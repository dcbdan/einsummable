...
send_channel_manager_t::try_to_acquire_impl(int const& loc)
{
    ...
    if (maybe_channel) {
        return send_channel_manager_resource_t{
            .self = this, .loc = loc, .channel = maybe_channel.value()};
    } else {
        return std::nullopt;
    }
}
...
void send_channel_manager_resource_t::send(void const* ptr, uint64_t bytes) const
{
    self->comm.send(loc, channel, ptr, bytes);
}

optional<int> send_channel_manager_t::acquire_channel(int loc)
{
    if (num_remaining == 0) {
        return std::nullopt;
    }
...
recv_channel_manager_t::try_to_acquire_impl(tuple<int, int> const& desc)
{
    ...
    if (queue.front() == id) {
        return recv_channel_manager_resource_t{
            .self = this, .id = id, .loc = src, .channel = channel};
    } else {
        return std::nullopt;
    }
}

void recv_channel_manager_t::release_impl(recv_channel_manager_resource_t const& r)
{
...
void recv_channel_manager_t::recv(int id, int loc, int channel, void* ptr, uint64_t num_bytes)
{
