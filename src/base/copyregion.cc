...
copyregion_t::copyregion_t(partition_t const& out,
...
partition_t union_pair_partitions(partition_t const& aa, partition_t const& bb)
{
    ...
    for (int i = 0; i != aa.partdims.size(); ++i) {
        pds.push_back(partdim_t::unions({aa.partdims[i], bb.partdims[i]}));
    }

    return partition_t(pds);
}

copyregion_full_t::copyregion_full_t(partition_t const& a, partition_t const& b)
    : aa(a),
      bb(b),
      rr(union_pair_partitions(a, b)),
      idx_aa(0),
      index_aa(a.partdims.size(), 0),
      offset_aa(a.partdims.size(), 0),
      idx_bb(0),
      index_bb(a.partdims.size(), 0),
      offset_bb(a.partdims.size(), 0),
      idx_rr(0),
      index_rr(a.partdims.size(), 0),
...
partition_t broadcast_inn_partition(partition_t const& join_partition,
...
copyregion_join_inn_t::copyregion_join_inn_t(partition_t const& partition_join,
