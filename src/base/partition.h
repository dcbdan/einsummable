...
struct partition_t {
    ...
    vector<tuple<uint64_t, uint64_t>> get_hrect(vector<int> const& idxs) const;

    // hrect:  hyper-rectangular subset of uint64s
    // region: hyper-rectangular subset of blocks
    vector<tuple<int, int>> get_exact_region(vector<tuple<uint64_t, uint64_t>> const& hrect) const;

    vector<tuple<int, int>> get_region(vector<tuple<uint64_t, uint64_t>> const& hrect) const;

    // If multiple index cover the area given by hrect,
    // throw an error
    vector<int> get_index_covering(vector<tuple<uint64_t, uint64_t>> const& hrect) const;
...
bool partitions_region(vector<vector<tuple<uint64_t, uint64_t>>> const& hrects,
