...
struct placement_t {
    placement_t(partition_t const& p, vtensor_t<int> const& locs) : partition(p), locations(locs) {}

    placement_t(partition_t const& p) : placement_t(p, vtensor_t<int>(p.block_shape())) {}
