...
#define vector_from_each_member(items, member_type, member_name)                                   \
    [](auto const& xs) {                                                                           \
        std::vector<member_type> ret;                                                              \
        ret.reserve(xs.size());                                                                    \
        std::transform(xs.begin(), xs.end(), std::back_inserter(ret), [](auto const& x) {          \
            return x.member_name;                                                                  \
        });                                                                                        \
        return ret;                                                                                \
    }(items)

#define vector_from_each_method(items, type, method)                                               \
    [](auto const& xs) {                                                                           \
        std::vector<type> ret;                                                                     \
        ret.reserve(xs.size());                                                                    \
        std::transform(xs.begin(), xs.end(), std::back_inserter(ret), [](auto const& x) {          \
...
#define vector_from_each_tuple(items, which_type, which)                                           \
    [](auto const& xs) {                                                                           \
        std::vector<which_type> ret;                                                               \
        ret.reserve(xs.size());                                                                    \
        std::transform(xs.begin(), xs.end(), std::back_inserter(ret), [](auto const& x) {          \
            return std::get<which>(x);                                                             \
        });                                                                                        \
        return ret;                                                                                \
    }(items)
...
vector<tuple<T1, T2>> vector_zip(vector<T1> const& lhs, vector<T2> const& rhs)
{
...
template <typename T1, typename T2>
tuple<vector<T1>, vector<T2>> vector_unzip(vector<tuple<T1, T2>> const& xs)
{
    vector<T1> lhs;
    vector<T2> rhs;
...
template <typename T, typename F>
void set_erase_if_inplace(set<T>& xs, F f)
{
...
Iter max_element_transform(Iter first, Iter last, F f)
{
...
template <typename Iter, typename F>
Iter min_element_transform(Iter first, Iter last, F f)
{
...
template <typename T>
optional<string> check_concat_shapes(int dim, vector<vector<T>> const& shapes)
{
    if (shapes.size() == 0) {
...
vector<int> as_out_perm(vector<int> const& inn, vector<int> const& out);

template <typename T>
vector<T> forward_permute(vector<int> const& out_perm, vector<T> const& inn_shape)
{
...
template <typename T>
vector<T> backward_permute(vector<int> const& out_perm, vector<T> const& out_shape)
{
    vector<int> modes = vector_iota<int>(out_perm.size());

    return forward_permute(as_out_perm(out_perm, modes), out_shape);
}
