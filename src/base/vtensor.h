...
struct vtensor_t {
    ...
    vtensor_t(vector<int> const& shape) : vtensor_t(shape, vector<T>(product(shape))) {}

    vtensor_t(vector<int> const& shape, T const& val)
        : vtensor_t(shape, vector<T>(product(shape), val))
    {
    }

    vtensor_t(vector<int> const& shape, vector<T> const& vec) : shape(shape), vec(vec)
    {
        if (shape.size() == 0) {
            if (vec.size() != 0) {
    ...
    vtensor_t(vector<int> const& shape, vector<T>&& v) : shape(shape), vec(std::move(v))
    {
        if (shape.size() == 0) {
            if (vec.size() != 0) {
                throw std::runtime_error("vec is not empty");
