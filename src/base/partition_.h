inline vector<uint64_t> partition_t::total_shape() const
{
    return vector_from_each_method(partdims, uint64_t, total);
}

inline int partition_t::num_parts() const
{
    return product(this->block_shape());
}

inline vector<int> partition_t::block_shape() const
{
    return vector_from_each_method(partdims, int, num_parts);
}

inline vector<int> partition_t::from_bid(int const& bid) const
{
    vector<int> shape = block_shape();
    return index_to_idxs(shape, bid);
}

inline bool partition_t::refines(partition_t const& other) const
{
...
inline vector<uint64_t> partition_t::tensor_shape_at(vector<int> const& idxs) const
{
...
inline vtensor_t<uint64_t> partition_t::all_block_sizes() const
{
    vector<int> shape = block_shape();
...
inline uint64_t partition_t::max_block_size() const
{
    uint64_t ret = 1;
...
inline uint64_t partition_t::block_size_at_bid(int bid) const
{
    return product(tensor_shape_at(from_bid(bid)));
}

inline partition_t partition_t::subset(vector<tuple<int, int>> const& region) const
{
    ...
    for (int i = 0; i != region.size(); ++i) {
        ...
        pds.push_back(
            partdim_t::from_sizes(vector<uint64_t>(full_szs.begin() + b, full_szs.begin() + e)));
    }

    return partition_t(pds);
}

inline partition_t partition_t::subset(vector<tuple<uint64_t, uint64_t>> const& hrect) const
{
...
inline vector<tuple<uint64_t, uint64_t>> partition_t::get_hrect(vector<int> const& idxs) const
{
    if (idxs.size() != partdims.size()) {
...
inline vector<tuple<int, int>>
partition_t::get_exact_region(vector<tuple<uint64_t, uint64_t>> const& region) const
{
    if (region.size() != partdims.size()) {
...
inline vector<tuple<int, int>>
partition_t::get_region(vector<tuple<uint64_t, uint64_t>> const& region) const
{
    if (region.size() != partdims.size()) {
...
inline vector<int>
partition_t::get_index_covering(vector<tuple<uint64_t, uint64_t>> const& hrect) const
{
    ...
    for (auto const& [b, e] : ret) {
        ...
        if (b + 1 != e) {
            throw std::runtime_error("get_index_covering: cannot have multiple index");
        }
    }
