...
struct timetracker_t {
    ...
    using timestamp_t = typename std::chrono::time_point<std::chrono::steady_clock>;
    using duration_t = typename std::chrono::duration<double>;
    using interval_t = tuple<timestamp_t, timestamp_t>;

    struct gremlin_t {
        gremlin_t(timetracker_t& self,
                  string         name_,
                  bool           is_interval,
                  bool           is_total,
                  uint64_t       f = 0);
