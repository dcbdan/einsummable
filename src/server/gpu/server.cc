...
gpu_mg_server_t::gpu_mg_server_t(communicator_t& c,
                                 // one buffer per gpu
    ...
    : server_mg_base_t(c, allocator_settings_t::gpu_alignment_settings())
{
    ...
    std::exclusive_scan(
        num_gpus_per_node.begin(), num_gpus_per_node.end(), start_gpus_per_node.begin(), 0);
                                       ...
                                       map<string, scalar_t> const& scalar_vars)
{
    ...
    exec_graph_t graph = exec_graph_t::make_gpu_exec_graph(memgraph,
                                                           comm.get_this_rank(),
                                                           kernel_managers,
    ...
    vector<rm_ptr_t> rms{rm_ptr_t(new gpu_workspace_manager_t()),
...
struct id_memstoloc_t {
    ...
    id_memstoloc_t(int id, memstoloc_t const& memstoloc) : id(id)
    {
...
server_mg_base_t::make_mg_info_t gpu_mg_server_t::recv_make_mg_info()
{
    make_mg_info_t ret{
        .mem_sizes = all_mem_sizes, .data_locs = data_locs, .which_storage = get_which_storage()};
...
void gpu_mg_server_t::storage_remap_server(vector<vector<std::array<int, 2>>> const& remaps)
{
    if (!bool(storage)) {
...
void gpu_mg_server_t::rewrite_data_locs_server(map<int, memstoloc_t> const& new_data_locs)
{
...
{
    auto iter = std::upper_bound(start_gpus_per_node.begin(), start_gpus_per_node.end(), loc);
...
buffer_t gpu_mg_server_t::local_copy_data(int tid)
{
    ...
    if (d.is_memloc()) {
        ...
        cudaError_t error = cudaMemcpy(ret_buffer->data,
...
void gpu_mg_server_t::local_insert_tensors(map<int, tuple<int, buffer_t>> data)
{
    ...
    for (auto const& [tid, loc_tensor] : data) {
        ...
        if (maybe_offset) {
            ...
            cudaMemcpy(increment_void_ptr(mems[local_gpu], offset),
                       tensor->data,
                       tensor->size,
                       cudaMemcpyHostToDevice);

            memloc_t memloc{.offset = offset, .size = tensor->size, .loc = global_gpu};
            ...
            stoloc_t stoloc{.loc = comm.get_this_rank(), .id = id};
