...
void server_dist_base_t::listen()
{
    ...
    while (true) {
        ...
        if (cmd == cmd_t::execute_tg) {
            ...
            insert_relation_helper(remap_relations_t::from_wire(comm.recv_string(0)),
                                   map<int, buffer_t>());
...
void server_dist_base_t::register_listen(string key, std::function<void()> f)
{
    if (comm.get_this_rank() == 0) {
...
dbuffer_t server_dist_base_t::get_tensor(relation_t const& relation)
{
    ...
    remap.insert(relation, relation.as_singleton(99, local_candidate_location()));
...
void server_dist_base_t::insert_constant_relation(relation_t const& dst_relation, scalar_t value)
{
...
void server_dist_base_t::_local_insert_constant_relation(relation_t const& dst_relation,
                                                         scalar_t          value)
{
...
void server_dist_base_t::erase_tids_server(vector<tuple<int, int>> const& loc_tid_pairs)
{
...
void server_dist_base_t::insert_relation(relation_t const& relation, dbuffer_t src_tensor)
{
    ...
    // remap here is with respect to locations
    remap_relations_t remap;
    remap.insert(relation.as_singleton(99, local_candidate_location()), relation);
...
void server_dist_base_t::insert_relation_helper(remap_relations_t remap, map<int, buffer_t> data)
{
...
void server_dist_base_t::erase_tids(vector<tuple<int, int>> const& loc_tid_pairs)
{
...
void server_dist_base_t::convert_remap_to_compute_node(remap_relations_t& remap)
{
...
map<int, buffer_t> server_dist_base_t::local_copy_source_data(remap_relations_t const& remap)
{
