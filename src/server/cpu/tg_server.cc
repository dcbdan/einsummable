                                  ...
                                  map<string, scalar_t> const& scalar_vars)
{
    ...
    auto [graph, dinfos] = exec_graph_t::make_cpu_tg_exec_graph(taskgraph,
                                                                this_rank,
                                                                kernel_executor,
                                                                num_channels_per_move,
                                                                num_threads_per_contraction,
                                                                scalar_vars);
    ...
    if (world_size > 1) {
        ...
        resource_manager = rm_ptr_t(new resource_manager_t(
            vector<rm_ptr_t>{rm_ptr_t(new data_manager_t(local_data, dinfos, max_memory_usage)),
...
{
    remap_relations_t remap_relations = remap_relations_t::from_wire(comm.recv_string(0));
    _remap(remap_relations);
}
...
void cpu_tg_server_t::_remap(remap_relations_t const& remap_relations)
{
    ...
    _update_map_with_new_tg_inns(local_data, remap_gid, gid_to_inn, remap_relations, this_rank);
    // after: data_locs is with respect to the tasgkraph inns

    _execute_tg(taskgraph);

    _update_map_with_new_tg_outs(local_data, remap_gid, gid_to_out, remap_relations, this_rank);
}
...
void cpu_tg_server_t::local_insert_tensors(map<int, tuple<int, buffer_t>> data)
{
    int this_rank = comm.get_this_rank();
