...
void cpu_mg_server_t::execute_memgraph(memgraph_t const&            memgraph,
                                       bool                         for_remap,
                                       map<string, scalar_t> const& scalar_vars)
{
    ...
    exec_graph_t graph = exec_graph_t::make_cpu_exec_graph(
        memgraph, this_rank, kernel_executor, num_channels_per_move, scalar_vars);
...
buffer_t cpu_mg_server_t::local_copy_data(int tid)
{
    ...
    if (d.is_mem()) {
        ...
        std::copy(mem->data + offset, mem->data + (offset + size), ret->data);
...
server_mg_base_t::make_mg_info_t cpu_mg_server_t::recv_make_mg_info()
{
...
void cpu_mg_server_t::storage_remap_server(vector<vector<std::array<int, 2>>> const& remaps)
{
    int world_size = comm.get_world_size();
...
void cpu_mg_server_t::rewrite_data_locs_server(map<int, memstoloc_t> const& new_data_locs)
{
...
void cpu_mg_server_t::local_insert_tensors(map<int, tuple<int, buffer_t>> data)
{
    int this_rank = comm.get_this_rank();
    ...
    for (auto const& [tid, loc_tensor] : data) {
        ...
        if (maybe_offset) {
            ...
            std::copy(tensor->data, tensor->data + tensor->size, mem->data + offset);

            memsto = memsto_t(mem_t{.offset = offset, .size = tensor->size});
        } else {
