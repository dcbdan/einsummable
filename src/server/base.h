...
protected:
    ...
    virtual void execute(taskgraph_t const&           taskgraph,
...
public:
    ...
    void insert_constant(int gid, relation_t const& relation, scalar_t value);
    void insert_constant(int gid, placement_t const& dst_pl, scalar_t value);

    // Get a tensor here and partition it across the cluster
    // into managed data
    void insert_tensor(int gid, relation_t const& dst_relation, dbuffer_t src_tensor);
    void insert_tensor(int gid, placement_t const& placement, dbuffer_t src_tensor);
    void insert_tensor(int gid, vector<uint64_t> const& shape, dbuffer_t src_tensor);

    virtual void insert_constant_relation(relation_t const& dst_relation, scalar_t value) = 0;
    virtual void insert_relation(relation_t const& dst_relation, dbuffer_t src_tensor) = 0;
    ...
    virtual void local_insert_tensors(map<int, tuple<int, buffer_t>> data) = 0;
    ...
    // if none is given
    virtual threadpool_t* get_cpu_threadpool()
    {
        return nullptr;
    }
...
struct server_dist_base_t : server_base_t {
    server_dist_base_t(communicator_t& c) : comm(c) {}
...
protected:
    // This is protected because it will invalidate gid_map.
    void execute(taskgraph_t const& taskgraph, map<string, scalar_t> const& scalar_vars);

public:
    dbuffer_t get_tensor(relation_t const& src_relation);

    void insert_constant_relation(relation_t const& dst_relation, scalar_t value);
    void insert_relation(relation_t const& dst_relation, dbuffer_t src_tensor);
    void insert_relation_helper(remap_relations_t remap, map<int, buffer_t> data);
...
private:
    ...
    {
        static vector<string> ret{"execute_tg",
                                  "remap",
                                  "erase_tids",
                                  "get_tensor",
                                  "insert_constant",
...
protected:
    virtual void execute_tg_server(taskgraph_t const&           taskgraph,
                                   map<string, scalar_t> const& sclar_vars) = 0;
    virtual void execute_tg_client() = 0;

    void _local_insert_constant_relation(relation_t const& dst_relation, scalar_t value);

    virtual void remap_server(remap_relations_t const& remap_relations) = 0;
    virtual void remap_client() = 0;

    void erase_tids_server(vector<tuple<int, int>> const& loc_tid_pairs);
...
struct server_mg_base_t : server_dist_base_t {
    ...
    virtual void execute_memgraph(memgraph_t const&            memgraph,
                                  bool                         for_remap,
                                  map<string, scalar_t> const& scalar_vars = {}) = 0;
    ...
    virtual void storage_remap_server(vector<vector<std::array<int, 2>>> const& remaps) = 0;
    ...
    create_storage_remaps(int                          world_size,
