...
uint64_t _add_to_ret(map<int, partition_t>&     ret,
                     ...
                     int                        start_id)
{
    auto get_which_best_plan = [](vector<_plan_t> const& plans) {
        ...
        return std::distance(plans.begin(), iter);
    };
    ...
    while (to_process.size() > 0) {
        ...
        vector_concatenate_into(to_process, vector_zip(next_inns, next_which));
...
vector<vector<int>> compute_equal_sums(int n, int d)
{
    static std::unordered_map<tuple<int, int>, vector<vector<int>>, tuple_pair_int_hash_t> cache;
...
struct get_parts_t {
    ...
    vector<partition_t> operator()(int gid) const
    {
        ...
        if (search_space == parts_space_t::contraction) {
            ...
            vector<vector<int>> pps = compute_equal_sums(log2_n, shape.size());
            vector_concatenate_into(pps, compute_equal_sums(log2_n + 1, shape.size()));
            vector_concatenate_into(pps, compute_equal_sums(log2_n + 2, shape.size()));
            return fix(shape, pps);
        } else {
    ...
    static optional<partition_t> get_part(vector<uint64_t> const& shape, vector<int> const& ps)
    {
        vector<partdim_t> pd;
        pd.reserve(ps.size());
    ...
    static vector<partition_t> fix(vector<uint64_t> const& shape, vector<vector<int>> const& pps)
    {
...
struct compute_cost_t {
    ...
    cost(int gid, partition_t const& join_part, optional<partition_t> const& maybe_usage_part)
    {
        ...
        if (node.op.is_einsummable()) {
            ...
            if (e.is_contraction()) {
                ...
                // Way 0:
                {
                    vector<int> out_block_shape(op_block_shape.begin(),
                                                op_block_shape.begin() + e.out_rank);
                    int         out_n_blocks = product(out_block_shape);
                    compute_cost +=
                        product(e.out_shape()) / out_n_blocks * (op_n_blocks - out_n_blocks);
                }
        ...
        if (maybe_usage_part) {
            ...
            partition_t out_part = [&] {
                vector<partdim_t> pds(join_part.partdims.begin(),
    ...
    uint64_t
    operator()(int gid, partition_t const& join_part, optional<partition_t> const& maybe_usage_part)
    {
        auto [compute_cost, repart_cost] = cost(gid, join_part, maybe_usage_part);
        return compute_cost + repart_cost;
    }

    uint64_t compute_repart_cost(partition_t const& out_part, partition_t const& refi_part)
    {
        ...
        if (!exact) {
            ...
            for (int r = 0; r != out_part.partdims.size(); ++r) {
                num_intersection *=
                    std::max(out_part.partdims[r].spans.size(), refi_part.partdims[r].spans.size());
            }
...
template <typename F>
optional<partition_t> _get_refi_partition01(graph_t const& graph, int gid, F get_partition)
{
    optional<partition_t> refi_partition;
    {
        ...
        if (graph.nodes[gid_fix].outs.size() > 0) {
            refi_partition = twolayer_construct_refinement_partition(graph, gid_fix, get_partition);
        }
    }
...
uint64_t _solve_tree(map<int, partition_t>& ret,
                     graph_t const&         graph,
                     ap_tree_t const&       tree,
                     int                    max_branching,
    ...
    for (int const& gid : all_gids) {
        ...
        vector<optional<partition_t>> all_refi_parts;
        {
            ...
            do {
                ...
                all_refi_parts.push_back(_get_refi_partition01(graph, gid, get_partition));
        ...
        for (auto const& part : possible_parts) {
            ...
            do {
                ...
                uint64_t cost = cost_from_fixed + cost_from_children + cost_from_node_compute +
                                cost_from_node_repart;
            ...
            plans.push_back(
                _plan_t{.partition = part, .which = best_which, .cost = best_cost.value()});
        }
    }
...
vector<partition_t> _build_vector(graph_t const& graph, map<int, partition_t> const& parts)
{
    vector<partition_t> ret;
    ...
    for (int gid = 0; gid != graph.nodes.size(); ++gid) {
        ...
        if (node.op.is_formation()) {
            ...
            vector<partdim_t> pds(inn_part.partdims.begin(), inn_part.partdims.begin() + rank);
                            ...
                            parts_space_t  search_space)
{
    ...
    // 3.
    get_parts_t           get_possible_partitions{.graph = graph,
                                                  .n_compute = n_compute,
                                                  .log2_n = log2_n,
                                                  .search_space = search_space,
                                                  .set_parts = std::nullopt};
    compute_cost_t        compute_cost{.graph = graph,
                                       .cache = {},
                                       .exact = false,
    ...
    for (int const& root_id : btrees.dag_order_inns_to_outs()) {
        ...
        uint64_t    cost_tree = _solve_tree(
            partitions_so_far, graph, tree, max_branching, get_possible_partitions, compute_cost);
    }
...
{
    compute_cost_t compute_cost{
        .graph = graph, .cache = {}, .exact = true, .discount_input_factor = 1};

    auto get_partition = [&](int gid) -> partition_t const& { return partitions.at(gid); };
    ...
    for (int gid = 0; gid != graph.nodes.size(); ++gid) {
        ...
        optional<partition_t> refi_partition = _get_refi_partition01(graph, gid, get_partition);
...
struct compute_cost02_t {
    ...
    uint64_t compute_cost(int gid, partition_t const& join_part)
    {
    ...
    uint64_t repartition_cost(partition_t const& join_part, partition_t const& refi_part)
    {
        ...
        partition_t out_part = partition_t(
            vector<partdim_t>(join_part.partdims.begin(), join_part.partdims.begin() + out_rank));
...
template <typename F>
optional<partition_t> _get_refi_partition02(graph_t const& graph, int gid, F get_partition)
{
    auto const& node = graph.nodes[gid];
    if (node.outs.size() > 0) {
        return twolayer_construct_refinement_partition(graph, gid, get_partition);
    }
    ...
    vector<partition_t> parts;
    {
        ...
        for (int id : graph.get_reverse_order()) {
            ...
            for (auto const& part : get_possible_parts(id)) {
                ...
                if ((bool(best_part) && cost < best_cost) || (!bool(best_part))) {
                    best_cost = cost;
                    best_part = part;
                }
            }
...
vector<partition_t> apart03(graph_t const& graph, map<tuple<int, int>, partdim_t> const& init_parts)
{
    ...
    for (int const& gid : graph.get_order()) {
        ...
        if (node.op.is_input() || node.op.is_fill()) {
            ...
            ret[gid] = partition_t(vector<partdim_t>(inn_pds.begin(), inn_pds.begin() + rank));
            ...
            for (auto const& [inn_gid, inn_region] : vector_zip(node.inns, select.inn_regions)) {
                auto const& inn_pds = ret[inn_gid].partdims;
                for (int d = 0; d != out_pds.size(); ++d) {
                    auto const& inn_pd = inn_pds[d];
                    auto const& selectdim = inn_region[d];
                    partdim_t   part_portion_pd =
                        inn_pd.subset(selectdim.offset_inn, selectdim.offset_inn + selectdim.size);
                    ...
                    if (selectdim.offset_out + selectdim.size < select.out_shape[d]) {
                        portion_sizes.push_back(select.out_shape[d] -
                                                (selectdim.offset_out + selectdim.size));
                    }
                    partdim_t portion_pd = partdim_t::from_sizes(portion_sizes);

                    out_pds[d] = partdim_t::unions({out_pds[d], portion_pd});
                }
            }
            ret[gid] = partition_t(out_pds);
...
static partition_t cutoff_part(vector<uint64_t> shape, uint64_t cutoff)
{
    vector<uint64_t> splits(shape.size(), 1);

    auto div = [](uint64_t a, uint64_t b) { return (a + b - 1) / b; };
    ...
    auto deduce_partition = [&](int gid) -> optional<partition_t> {
        ...
        if (node.op.is_formation()) {
            ...
            if (inn_part) {
                ...
                return partition_t(vector<partdim_t>(inn_pds.begin(), inn_pds.begin() + rank));
            } else {
                return std::nullopt;
            }
        }
