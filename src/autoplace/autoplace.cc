...
vector<placement_t> autoplace01(graph_t const& graph, autoplace_config_t const& config)
{
    ...
    return alocate01(graph, parts, config.n_locs(), config.flops_per_byte_moved());
}
                                ...
                                vector<tuple<int, int>> const& equal_pls)
{
    ...
    vector<partition_t> parts = apart02(graph, config.n_compute(), fixed_parts, equal_pls);

    auto ret = alocate02(
        graph, parts, config.n_locs(), config.flops_per_byte_moved(), fixed_locs, equal_pls);
