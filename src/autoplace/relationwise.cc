...
relationwise_t::relationwise_t(graph_t const& g, vector<partition_t> const& parts) : graph(g)
{
    std::function<partition_t const&(int)> get_partition = [&parts](int gid) -> partition_t const& {
        return parts[gid];
    };
    ...
    for (int gid = 0; gid != graph.nodes.size(); ++gid) {
        ...
        ginfos.push_back(ginfo_t{
            .partition = part,
            .joins = twolayer_construct_joins(graph, gid, part),
            .locations = vector<int>(nblocks, -1),
            .refinement_partition =
                has_refinement ? optional<partition_t>(twolayer_construct_refinement_partition(
        ...
        twolayer_insert_refi_outs_from_join_deps(graph, gid, ginfo.joins, get_refis);
    }
}
...
vector<placement_t> relationwise_t::get_placements() const
{
    ...
    for (auto const& ginfo : ginfos) {
        ret.emplace_back(ginfo.partition,
...
placement_t relationwise_t::get_placement_at(int gid) const
{
    ...
    return placement_t(ginfo.partition,
                       vtensor_t<int>(ginfo.partition.block_shape(), ginfo.locations));
}

std::function<partition_t const&(int)> relationwise_t::f_get_partition() const
{
    return [this](int gid) -> partition_t const& { return ginfos.at(gid).partition; };
}

std::function<partition_t const&(int)> relationwise_t::f_get_refinement_partition() const
{
...
std::function<vector<refinement_t>&(int)> relationwise_t::f_get_mutable_refis()
{
    return [this](int gid) -> vector<refinement_t>& { return ginfos.at(gid).refis.value(); };
}
...
int relationwise_t::num_agg_blocks_at(int gid) const
{
    ...
    partition_t agg_partition(vector<partdim_t>(ginfo.partition.partdims.begin() + out_rank,
