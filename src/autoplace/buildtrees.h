...
struct bgraph_t {
    ...
    map<int, node_t> const& get() const
    {
        return items;
    }
    ...
    void insert(int id, T const& value, set<int> const& inns, set<int> const& outs)
    {
        ...
        auto [_, did_insert] =
            items.insert({id, node_t{.outs = outs, .inns = inns, .item = value}});
        ...
        for (auto const& [id, node] : items) {
            new_items.insert({id,
                              typename bgraph_t<U>::node_t{
                                  .outs = node.outs, .inns = node.inns, .item = f(id, node.item)}});
        }
    ...
    {
        return map_items_with_id([&f](int id, T const& t) { return f(t); });
    }

    struct is_connected_t {
        is_connected_t(map<int, set<int>>&& v) : all_inns_of(v) {}
...
public:
    void insert_root(int id, T const& value)
    {
        ...
        root_id = id;
        auto [_, did_insert] =
            items.insert({id, node_t{.out = std::nullopt, .inns = {}, .item = value}});
        if (!did_insert) {
    ...
    void insert_leaf(int id, int out, T const& value)
    {
        ...
        auto [_, did_insert] = items.insert({id, node_t{.out = out, .inns = {}, .item = value}});
        if (!did_insert) {
...
bool _tree_can_consume(bgraph_t<T> const& graph,
                       F&                 is_path_from_to,
...
template <typename T>
set<int> _compute_tree_outs(bgraph_t<T> const&   graph,
                            btree_t<T> const&    tree,
...
template <typename T>
set<int>
_get_tree_outs(bgraph_t<T> const& graph, btree_t<T> const& tree, map<int, int> const& id_to_root)
{
    if (tree.is_empty()) {
        return {};
    }
    ...
    for (auto const& root_id : graph.dag_order_outs_to_inns()) {
        ...
        for (; idx != to_process.size(); ++idx) {
            ...
            for (int const& inn_id : graph.get_inns(id)) {
                ...
                bool can_consume = _tree_can_consume(graph, is_path_from_to, tree, inn_id, id);
                if (can_consume) {
