  ...
  bool with_goofy_topology)
{
  ...
  int _start_loc = 0;
  auto get_start_loc_rest_locs = [&]() -> tuple<int, vector<int>> {
    int ret = _start_loc;
    // update start loc
    _start_loc = (_start_loc + 1) % nlocs;

    vector<int> the_rest;
    for(int i = 1; i != nlocs; ++i) {
      the_rest.push_back( (_start_loc + i) % nlocs );
    }
    return {ret, the_rest};
  };
    ...
    if(is_full_plan(gid))
    {
      ...
      for(vector<int> const& join_locs: alocate03_create_join_locs(nlocs, n_join_bid)) {
        ...
        for(int bid = 0; bid != n_form_bid; ++bid) {
          ...
          auto [best_loc, the_rest_locs] = get_start_loc_rest_locs();
          uint64_t best_cost = rw.compute_at(jid, best_loc);
          for(int const& loc: the_rest_locs) {
