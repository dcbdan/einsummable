...
struct autoplace_config_t {
    int n_compute() const
    {
        return _n_locs * _n_compute_per_loc;
    }
    int n_locs() const
    {
        return _n_locs;
    }
    uint64_t flops_per_byte_moved() const
    {
        return _flops_per_byte_moved;
    }
    int max_branching() const
    {
        return _max_branching.value();
    }
    uint64_t discount_input_factor() const
    {
        return _discount_input_factor.value();
    }
    parts_space_t search_space() const
    {
        return _search_space.value();
    }

    autoplace_config_t(int __n_locs, int __n_compute_per_loc, uint64_t __fps_per_byte)
        : _n_locs(__n_locs),
          _n_compute_per_loc(__n_compute_per_loc),
          _flops_per_byte_moved(__fps_per_byte)
    {
    }

    static autoplace_config_t
    make_default01(int n_locs, int n_compute_per_loc, uint64_t discount_input_factor_ = 1)
    {
...
vector<placement_t> autoplace01(graph_t const& graph, autoplace_config_t const& config);

// fixed_pls: these nodes have this placement
// equal_pls: these nodes have the same placement
vector<placement_t> autoplace02(graph_t const&                 graph,
