...
void mpi_t::_send_recv(
  ...
  int tag)
{
  int64_t const max_size = 1073741824 / sizeof(uint8_t); // 2^30 / itemsize
  ...
  while(numel_rem > 0) {
    ...
    int result = is_send
        ? MPI_Ssend(data, num_to_comm, MPI_CHAR, loc, tag, MPI_COMM_WORLD)
        : MPI_Recv( data, num_to_comm, MPI_CHAR, loc, tag, MPI_COMM_WORLD,
                    MPI_STATUS_IGNORE)
        ;
    ...
    data = (void*)((char*)data + (sizeof(uint8_t) * num_to_comm));
