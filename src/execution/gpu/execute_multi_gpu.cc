...
gpu_execute_state_t::gpu_execute_state_t(memgraph_t const &input_memgraph,
                                         std::vector<float*> mem_ptrs)
    : memgraph(input_memgraph), memory_base_ptrs(mem_ptrs) {
    ...
    if (memgraph.nodes[i].op.is_einsummable()) {
      ...
      if (my_einsummable.is_contraction()) {
        ...
        if (einsum_to_contraction.find(my_einsum_merged) ==
          ...
          int loc = memgraph.nodes[i].op.get_apply_loc();
          cudaSetDevice(loc);
...
void execute(const memgraph_t &memgraph, std::vector<float*> mem_ptrs) {
  // create a gpu_execute_state_t
  gpu_execute_state_t gpu_execute_state(memgraph, mem_ptrs);
...
// EXECUTION SLOWER depending on the number of initial streams
struct callback_data_t {s
  std::mutex *m_ptr;
  ...
  while (true) {
    ...
    while (true) {
      ...
      if (node.op.is_inputmem() || node.op.is_inputsto() || node.op.is_del() ||
        ...
        // and this operation's type
        throw std::runtime_error("Error: Operation not supported");
      }
    }
