...
struct graph_t {
    ...
    // {{{
    int insert_input(vector<uint64_t> shape, dtype_t dtype = default_dtype());

    int insert_einsummable(einsummable_t e, vector<int> inns);

    int insert_formation(int inn, bool is_save = true);
    ...
    int insert_constant(fill_t::constant_t const& c)
    {
        return insert_fill(fill_t(c));
    }

    int insert_fill(fill_t const& fill);
    int insert_fill(fill_t::constant_t const& c)
    {
        return insert_fill(fill_t(c));
    }

    int insert_concat(int dim, vector<int> inns);

    int insert_subset(vector<tuple<uint64_t, uint64_t>> hrect, int inn);
    ...
    void print_graphviz(std::ostream& out, map<int, string> get_color = map<int, string>{}) const;
    void print_graphviz(std::ostream&              out,
                        vector<partition_t> const& p,
                        map<int, string>           get_color = map<int, string>{}) const;
    void print_subset_graphviz(std::ostream&              out,
                               set<int> const&            gids,
...
public:
    ...
    struct complexer_t {
        ...
        bool is_to_real() const
        {
            return dtype_is_real(dtype);
        }
        bool is_to_complex() const
        {
            return dtype_is_complex(dtype);
        }
    };
    ...
    private:
        using _op_t = std::
            variant<input_t, formation_t, complexer_t, squeezer_t, fill_t, select_t, einsummable_t>;
    ...
    public:
        ...
        bool is_complexer() const
        {
            return std::holds_alternative<complexer_t>(op);
        }
        bool is_squeezer() const
        {
            return std::holds_alternative<squeezer_t>(op);
        }
        bool is_fill() const
        {
        ...
        {
            return std::holds_alternative<select_t>(op);
        }
        ...
        complexer_t const& get_complexer() const
        {
            return std::get<complexer_t>(op);
        }
        squeezer_t const& get_squeezer() const
        {
            return std::get<squeezer_t>(op);
        }
        fill_t const& get_fill() const
        {
            return std::get<fill_t>(op);
        }
        select_t const& get_select() const
        {
            return std::get<select_t>(op);
        }
...
private:
    ...
    set<int>
    compute_nodeset(vector<int> const& outs, vector<int> const& inns, bool include_inns_outs) const;
    ...
    struct backprop_tensor_t {
        ...
        static backprop_tensor_t ones(dtype_t const& dtype, vector<uint64_t> const& shape);
        static backprop_tensor_t zeros(dtype_t const& dtype, vector<uint64_t> const& shape);
    ...
    backprop_tensor_t build_grad_term(int id, int which_inn, backprop_tensor_t grad_id);

    backprop_tensor_t build_grad_term_einsummable(einsummable_t const& e,
                                                  int                  out_id,
                                                  vector<int> const&   inn_ids,
                                                  int                  which_inn,
                                                  backprop_tensor_t    grad_id);

    backprop_tensor_t backprop_tensor_aggregate(backprop_tensor_t const& tensor,
                                                vector<int> const&       es_inns,
                                                int                      out_rank);

    backprop_tensor_t build_grad_term_contraction(einsummable_t const& e,
                                                  vector<int> const&   inn_ids,
                                                  int                  which_inn,
                                                  backprop_tensor_t    grad_id);

    backprop_tensor_t build_grad_term_ew(einsummable_t const& e,
                                         vector<int>          inn_ids,
                                         int                  which_inn,
                                         backprop_tensor_t    grad_id);

    backprop_tensor_t
    build_grad_term_select(select_t const& select, int which_inn, backprop_tensor_t grad_id);

    backprop_tensor_t build_grad_term_complexer(backprop_tensor_t grad_id);

    backprop_tensor_t build_grad_term_squeezer(vector<uint64_t> const& inn_shape,
                                               backprop_tensor_t       grad_id);

    backprop_tensor_t build_grad_term_reduction_add(vector<uint64_t> const& join_shape,
                                                    vector<int> const&      inn,
                                                    int                     out_rank,
                                                    backprop_tensor_t       grad_id);

    backprop_tensor_t build_grad_term_reduction_mul(vector<uint64_t> const& join_shape,
                                                    vector<int> const&      inn,
                                                    int                     out_rank,
                                                    int                     out_id,
                                                    int                     inn_id,
    ...
    backprop_tensor_t build_grad_term_reduction_maxmin(vector<uint64_t> const& join_shape,
                                                       vector<int> const&      inn,
                                                       int                     out_rank,
                                                       int                     out_id,
                                                       int                     inn_id,
    ...
    // to be formed.
    int insert_einsummable_for_backprop(einsummable_t e, vector<int> inns);
};
...
struct graph_constructor_t {
    int insert_input(placement_t placement, dtype_t dtype = default_dtype());
    int insert_input(partition_t partition, dtype_t dtype = default_dtype());
    int insert_input(vector<uint64_t> shape, dtype_t dtype = default_dtype());

    int insert_einsummable(placement_t placement, einsummable_t e, vector<int> inns);
    int insert_einsummable(partition_t partition, einsummable_t e, vector<int> inns);
    int insert_einsummable(einsummable_t e, vector<int> inns);

    int insert_formation(placement_t placement, int inn, bool is_save = true);
    int insert_formation(partition_t partition, int inn, bool is_save = true);
    int insert_formation(int inn, bool is_save = true);

    int insert_to_complex(placement_t placement, int inn);
    int insert_to_complex(partition_t partition, int inn);
    int insert_to_complex(int inn);

    int insert_to_real(placement_t placement, int inn);
    int insert_to_real(partition_t partition, int inn);
    int insert_to_real(int inn);

    // TODO: add insert_fill

    int insert_concat(placement_t placement, int dim, vector<int> inns);
    int insert_concat(partition_t partition, int dim, vector<int> inns);
    int insert_concat(int dim, vector<int> inns);

    int insert_subset(placement_t placement, vector<tuple<uint64_t, uint64_t>> hrect, int inn);
    int insert_subset(partition_t partition, vector<tuple<uint64_t, uint64_t>> hrect, int inn);
    int insert_subset(vector<tuple<uint64_t, uint64_t>> hrect, int inn);
