...
struct taskgraph_t {
    static tuple<map<int, vtensor_t<int>>, // for each input, the taskgraph ids of the blocks
                 map<int, vtensor_t<int>>, // for each save graph id, the taskgraph ids of the
                                           // blocks
    ...
    int insert_input(int loc, dtype_t dtype, vector<uint64_t> shape, bool is_save = false);

    int insert_constant(int loc, fill_t const& fill, bool is_save = false);

    int insert_einsummable(int loc, einsummable_t e, vector<int> inns, bool is_save = false);

    int insert_move(int src, int dst, int inn, bool is_save = false);

    int insert_consumed_aggregate(int         loc,
                                  dtype_t     dtype,
                                  castable_t  castable,
                                  vector<int> inns,
                                  bool        is_save = false);

    int insert_select_subset(int                 loc,
                             vector<regiondim_t> selection,
                             int                 inn,
                             dtype_t             dtype,
                             bool                is_save = false);

    int new_partial(int loc, dtype_t dtype, vector<uint64_t> write_shape, bool is_save = false);

    void add_to_partial(int id_out, int id_inn, touch_t touch, bool consume = false);

    void
    add_to_partial_the_full_aggregate(int id_out, int id_inn, castable_t castable, bool consume);
    // }}}
...
private:
    optional<tuple<map<int, int>, taskgraph_t>> remove_passthrough_partials() const;

    tuple<map<int, int>, taskgraph_t> prune() const;
...
public:
    ...
    void print_graphviz(std::ostream& out, vector<string> const& colors) const;
    ...
    int out_loc(int id) const
    {
        return nodes[id].op.out_loc();
    }

    bool is_save(int id) const
    {
        return nodes[id].is_save;
    }

    bool is_local_to(int id, int loc) const
    {
        return nodes[id].op.is_local_to(loc);
    }
...
private:
    ...
    struct partialize_t {
        ...
        static partialize_t make_from_touches(int loc, vector<tuple<int, touch_t>> const&);
    ...
    friend bool operator==(partialize_t::out_regiondim_t const& lhs,
...
public:
    ...
    public:
        ...
        constant_t& get_constant()
        {
            return std::get<constant_t>(op);
        }

        partialize_t& get_partialize()
        {
...
private:
    ...
    struct _unit_simplify_t {
        ...
        _unit_simplify_t(taskgraph_t const& tg, dtype_t dtype, partial_unit_t const& unit);
...
struct multiple_placement_t {
    multiple_placement_t(partition_t const& pa, vtensor_t<set<int>> const& ls);
    ...
    // with join_placement
    static multiple_placement_t make_einsummable_input(placement_t const&   join_placement,
                                                       einsummable_t const& einsummable,
                                                       int                  which_input);
    static multiple_placement_t
    make_select_input(placement_t const& join_placement, select_t const& select, int which_input);
