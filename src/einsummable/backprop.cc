...
vector<int> graph_t::backprop(int out, vector<int> weights)
{
    ...
    backprop_state_t state{.grads = {}, .self = *this, .nodeset = std::move(nodeset)};
...
graph_t::backprop_tensor_t::backprop_tensor_t() : op(-1) {}

graph_t::backprop_tensor_t::backprop_tensor_t(int id) : op(id) {}

graph_t::backprop_tensor_t::backprop_tensor_t(fill_t::constant_t const& c) : op(c) {}

graph_t::backprop_tensor_t
graph_t::backprop_tensor_t::backprop_tensor_t::ones(dtype_t const&          dtype,
                                                    vector<uint64_t> const& shape)
{
    ...
    return backprop_tensor_t(constant_t{.value = value, .shape = shape});
}

graph_t::backprop_tensor_t
graph_t::backprop_tensor_t::backprop_tensor_t::zeros(dtype_t const&          dtype,
                                                     vector<uint64_t> const& shape)
{
    return backprop_tensor_t(constant_t{.value = scalar_t::zero(dtype), .shape = shape});
}
...
void graph_t::backprop_state_t::start(int out_id)
{
    ...
    backprop_tensor_t tensor = backprop_tensor_t::ones(op.out_dtype(), op.out_shape());
    grads.insert({out_id, tensor});
}

vector<graph_t::backprop_state_t::out_edge_t> graph_t::backprop_state_t::get_out_edges(int id) const
{
    ...
    for (auto const& out : outs) {
        if (nodeset.count(out) > 0) {
            ...
            for (int which_inn = 0; which_inn != inns.size(); ++which_inn) {
                ...
                if (inn == id) {
                    ret.emplace_back(out_edge_t{.out = out, .which_inn = which_inn});
                }
            }
        }
    }
...
graph_t::build_grad_term(int id, int which_inn, backprop_tensor_t grad_id)
{
    ...
    if (op.is_input()) {
        ...
        return build_grad_term_select(op.get_select(), which_inn, grad_id);
    } else if (op.is_einsummable()) {
        return build_grad_term_einsummable(op.get_einsummable(), id, inns, which_inn, grad_id);
    } else {
        throw std::runtime_error("should not reach: missing graph type");
    }
}

graph_t::backprop_tensor_t graph_t::build_grad_term_einsummable(einsummable_t const& e,
                                                                int                  out_id,
                                                                ...
                                                                backprop_tensor_t    grad_id)
{
    ...
    if (e.has_broadcast()) {
        ...
        if (e.is_broadcast()) {
            ...
            return build_grad_term_einsummable(
                e.remove_broadcast(), out_id, inn_ids, which_inn, fixed_grad_id);
        }
    }
    ...
    if (num_inn == 1 && e.join.is_identity()) {
        ...
        if (castable == castable_t::add) {
            return build_grad_term_reduction_add(e.join_shape, e.inns[0], e.out_rank, grad_id);
        } else if (castable == castable_t::mul) {
            return build_grad_term_reduction_mul(
                e.join_shape, e.inns[0], e.out_rank, out_id, inn_ids[0], grad_id);
...
graph_t::backprop_tensor_t
graph_t::backprop_tensor_aggregate(graph_t::backprop_tensor_t const& tensor,
                                   vector<int> const&                inn,
                                   int                               out_rank)
{
    einsummable_t e = einsummable_t::aggregate(
        tensor.shape(*this), inn, out_rank, tensor.dtype(*this), castable_t::add);
    ...
    if (tensor.is_constant()) {
        ...
        value *= multiplier;
        return backprop_tensor_t(constant_t{.value = value, .shape = out_shape});
    } else {
...
graph_t::backprop_tensor_t graph_t::build_grad_term_contraction(einsummable_t const& e,
                                                                vector<int> const&   inn_ids,
                                                                int                  which_inn,
                                                                backprop_tensor_t    grad)
{
    ...
    if (grad.is_zeros()) {
        vector<uint64_t> shape = which_inn == 0 ? e.inn_shape(0) : e.inn_shape(1);
    ...
    if (grad.is_constant()) {
        ...
        vector<uint64_t> new_join_shape =
            einsummable_t::construct_join_shape(new_out_shape, new_inns, {new_inn_shape});
    ...
    auto [new_inns, new_out_rank] = einsummable_t::parse_str(new_str);
    vector<uint64_t> new_join_shape =
        einsummable_t::construct_join_shape(new_o_shape, new_inns, {new_l_shape, new_r_shape});
...
graph_t::backprop_tensor_t graph_t::build_grad_term_ew(einsummable_t const& e,
                                                       vector<int>          inn_ids,
                                                       int                  which_inn,
                                                       backprop_tensor_t    grad)
{
    ...
    if (dtype_is_complex(inn_dtype) || dtype_is_complex(out_dtype)) {
        if (e.join == scalarop_t::make_mul(out_dtype)) {
            ...
            scalarop_t     new_join = scalarop_t::combine(
                ...
                {scalarop_t::make_identity(dtype), scalarop_t::make_conjugate(dtype)});
            if (which_inn != 0 && which_inn != 1) {
            ...
            auto new_join_shape = einsummable_t::construct_join_shape(
                new_out_shape, new_inns, {new_lhs_shape, new_rhs_shape});
            einsummable_t new_e(new_join_shape, new_inns, new_out_rank, new_join, castable_t::add);
    ...
    if (constant_deri_op && constant_grad) {
        ...
        return backprop_tensor_t(constant_t{.value = v, .shape = inn_shapes[which_inn]});
    } else if (constant_deri_op) {
        scalar_t value = deri_op.eval().convert(out_dtype);

        scalarop_t new_join =
            scalarop_t::combine(scalarop_t::make_mul(out_dtype),
                                vector<scalarop_t>{scalarop_t::make_constant(value),
                                                   scalarop_t::make_identity(out_dtype)});

        new_join = scalarop_t::combine(scalarop_t::make_convert_dtype(out_dtype, inn_dtype),
        ...
        if (new_join.is_constant()) {
            ...
            return backprop_tensor_t(
                constant_t{.value = new_value, .shape = inn_shapes[which_inn]});
        }

        string new_str = out_str + "->" + inn_strs[which_inn];
        auto [new_inns, new_out_rank] = einsummable_t::parse_str(new_str);
        vector<uint64_t> new_join_shape =
            einsummable_t::construct_join_shape(inn_shapes[which_inn], new_inns, {out_shape});
        einsummable_t new_e(new_join_shape, new_inns, new_out_rank, new_join, castable_t::add);
        ...
        scalarop_t new_join =
            scalarop_t::combine(scalarop_t::make_mul(out_dtype),
                                vector<scalarop_t>{deri_op, scalarop_t::make_constant(value)});
        ...
        auto [new_inns, new_out_rank] = einsummable_t::parse_str(new_str);
        vector<uint64_t> new_join_shape =
            einsummable_t::construct_join_shape(inn_shapes[which_inn], new_inns, new_inn_shapes);
        einsummable_t new_e(new_join_shape, new_inns, new_out_rank, new_join, castable_t::add);
        ...
        //       that a hole is used..
        scalarop_t new_join = scalarop_t::combine(scalarop_t::make_mul(out_dtype),
                                                  vector<scalarop_t>{
                                                      scalarop_t::make_identity(out_dtype),
                                                      deri_op,
                                                  });
...
graph_t::backprop_tensor_t
graph_t::build_grad_term_select(select_t const& select, int which_inn, backprop_tensor_t grad)
{
    ...
    if (uses_full_input) {
        if (grad.is_constant()) {
            return backprop_tensor_t(constant_t{.value = grad.get_constant().value,
                                                .shape = select.inn_shape(which_inn)});
        }
        // Subset the gradient and return that
        int const& grad_id = grad.get_id();
        return backprop_tensor_t(insert_subset(select.wrt_output_inn_hrect(which_inn), grad_id));
    }
        ...
        if (maybe_constant_fill) {
            ...
            for (int i = 0; i != rank; ++i) {
                ...
                inn_region.push_back(
                    selectdim_t{.d_inn = size, .offset_inn = 0, .offset_out = beg, .size = size});
                block_shape.push_back(size);
            }

            inn_ids.push_back(insert_constant(maybe_constant_fill.value(), block_shape));
        } else {
            // Case 1b
            vector<uint64_t> out_start =
                select.wrt_output_point(vector_mapfst(block_hrect), which_inn);
            ...
            for (int i = 0; i != rank; ++i) {
                ...
                inn_region.push_back(selectdim_t{
                    .d_inn = o_dim, .offset_inn = o_beg, .offset_out = beg, .size = size});
                block_shape.push_back(size);
            }
...
graph_t::backprop_tensor_t graph_t::build_grad_term_complexer(graph_t::backprop_tensor_t grad)
{
    ...
    if (grad.is_constant()) {
        ...
        if (dtype_is_real(dtype)) {
            ...
            return backprop_tensor_t(constant_t{.value = scalar_t(vv), .shape = complex_shape});
        } else {
            // ( (v,v), (v,v), (v,v) ) ->  (v,v,v,v,v,v)
            if (dtype != dtype_t::c64) {
                throw std::runtime_error("complexer confusion");
            ...
            if (vu.real() == vu.imag()) {
                ...
                real_shape.back() *= 2;
                return backprop_tensor_t(constant_t{.value = scalar_t(v), .shape = real_shape});
            } else {
...
graph_t::backprop_tensor_t graph_t::build_grad_term_squeezer(vector<uint64_t> const&    inn_shape,
                                                             graph_t::backprop_tensor_t grad)
{
    if (grad.is_constant()) {
        return backprop_tensor_t(
            constant_t{.value = grad.get_constant().value, .shape = inn_shape});
    }
...
graph_t::backprop_tensor_t
graph_t::build_grad_term_reduction_add(vector<uint64_t> const&    join_shape,
                                       vector<int> const&         inn,
                                       int                        out_rank,
                                       graph_t::backprop_tensor_t grad)
{
    ...
    vector<uint64_t> inn_shape = einsummable_t::get_input_from_join_(inn, join_shape);

    if (grad.is_constant()) {
        scalar_t value = grad.get_constant().value;

        return backprop_tensor_t(constant_t{.value = value, .shape = inn_shape});
    }
    ...
    auto [new_inns, new_out_rank] = einsummable_t::parse_str(new_str);
    einsummable_t e(inn_shape, new_inns, new_out_rank, scalarop_t::make_identity(dtype));

    return backprop_tensor_t(insert_einsummable_for_backprop(e, {id}));
}

graph_t::backprop_tensor_t
graph_t::build_grad_term_reduction_mul(vector<uint64_t> const&    join_shape,
                                       vector<int> const&         inn,
                                       int                        out_rank,
                                       int                        out_id,
                                       int                        inn_id,
                                       graph_t::backprop_tensor_t grad)
{
    ...
    if (grad.is_constant()) {
        ...
        scalar_t   value = grad.get_constant().value;
        scalarop_t join = scalarop_t::combine(scalarop_t::make_mul(dtype),
        ...
        auto [new_inns, new_out_rank] = einsummable_t::parse_str(new_str);
        auto new_join_shape =
            einsummable_t::construct_join_shape(inn_shape, new_inns, {out_shape, inn_shape});
    ...
    scalarop_t join =
        scalarop_t::combine(scalarop_t::make_mul(dtype), {base, scalarop_t::make_arg(0, dtype)});
    ...
    auto [new_inns, new_out_rank] = einsummable_t::parse_str(new_str);
    auto new_join_shape =
        einsummable_t::construct_join_shape(inn_shape, new_inns, {out_shape, inn_shape, out_shape});
...
graph_t::backprop_tensor_t
graph_t::build_grad_term_reduction_maxmin(vector<uint64_t> const&    join_shape,
                                          vector<int> const&         inn,
                                          int                        out_rank,
                                          int                        out_id,
                                          int                        inn_id,
                                          graph_t::backprop_tensor_t grad)
{
    ...
    auto make_ij_i = [&](scalarop_t const& join) {
        ...
        auto e_join_shape =
            einsummable_t::construct_join_shape(ij_shape, e_inns, {ij_shape, i_shape});
    ...
    auto make_ij_i_i = [&](scalarop_t const& join) {
        ...
        auto const& [e_inns, e_out_rank] = einsummable_t::parse_str(str);
        auto e_join_shape =
            einsummable_t::construct_join_shape(ij_shape, e_inns, {ij_shape, i_shape, i_shape});
        return einsummable_t(e_join_shape, e_inns, e_out_rank, join);
    };
    ...
    auto make_reduction = [&]() {
        ...
        auto const& [e_inns, e_out_rank] = einsummable_t::parse_str(str);
        auto e_join_shape = einsummable_t::construct_join_shape(i_shape, e_inns, {ij_shape});
        auto identity = scalarop_t::make_identity(dtype);
        return einsummable_t(e_join_shape, e_inns, e_out_rank, identity, castable_t::add);
    };

    int which_max =
        insert_einsummable_for_backprop(make_ij_i(scalarop_t::make_is_equal(dtype)), {x_id, y_id});

    int sum_max = insert_einsummable_for_backprop(make_reduction(), {which_max});
    ...
    if (grad.is_constant()) {
        ...
        return backprop_tensor_t(insert_einsummable_for_backprop(e, {which_max, sum_max}));
    }

    scalarop_t join =
        scalarop_t::combine(scalarop_t::make_mul(dtype), {div, scalarop_t::make_arg(0, dtype)});
...
graph_t::backprop_tensor_t graph_t::insert_adds(vector<backprop_tensor_t> const& items_)
{
    if (items_.size() == 0) {
    ...
    if (items.size() == 0) {
        ...
        return backprop_tensor_t(constant_t{.value = sum, .shape = shape});
    }
...
int graph_t::insert_einsummable_for_backprop(einsummable_t e, vector<int> inns)
{
    if (e.is_identity()) {
        // then inns.size() == 1
        return inns[0];
...
set<int> graph_t::compute_nodeset(vector<int> const& upps,
...
vector<int> graph_t::reverse_order_nodeset(set<int> const& nodeset) const
{
    vector<int> ret;
