...
relation_t
relation_t::make_singleton(dtype_t const& dtype, vector<uint64_t> const& shape, int id, int loc)
{
    vector<int> ones(shape.size(), 1);
    placement_t pl(partition_t::singleton(shape));
    pl.locations.get()[0] = loc;
    return relation_t{.dtype = dtype, .placement = pl, .tids = vtensor_t<int>(ones, {id})};
}
...
relation_t relation_t::from_proto(es_proto::Relation const& r)
{
    ...
    return relation_t{.dtype = parse_with_ss<dtype_t>(r.dtype()),
                      .placement = placement,
...
void remap_relations_t::insert(relation_t const& src, relation_t const& dst)
{
    ...
    if (!vector_equal(src.placement.total_shape(), dst.placement.total_shape())) {
...
remap_relations_t remap_relations_t::from_wire(string const& str)
{
    es_proto::RemapRelations r;
    if (!r.ParseFromString(str)) {
    ...
    for (int i = 0; i != r.srcs_size(); ++i) {
        ret.insert(relation_t::from_proto(r.srcs(i)), relation_t::from_proto(r.dsts(i)));
    }
