...
{
    return memloc_t{.offset = offset, .size = size, .loc = loc};
}
...
{
    return mem_t{.offset = offset, .size = size};
}
...
{
    return allocator_settings_t{.strat = allocator_strat_t::lowest_dependency,
                                .alignment_power = 0};
}
...
memgraph_t::memgraph_t(int nl, int nc, vector<int> const& cs, bool pe)
    : num_compute_locs(nl), num_storage_locs(nc), storage_locs(cs), prune_edges(pe)
{
}

memgraph_t::memgraph_t(memgraph_t const& mg)
...
void memgraph_t::print_graphviz(std::ostream& out) const
{
    ...
    if(op.is_inputmem())
    {
      ...
      move_t const& move = op.get_move();
            ...
            label = "move@" + write_with_ss(memloc_t{src_offset, size, src_loc}) + "->" +
            ...
            label = "copy@" + write_with_ss(memloc_t{src_offset, size, loc}) + "->" +
                    write_with_ss(memloc_t{dst_offset, size, loc});
            color = "yellow";
        } else if (op.is_safe_copy()) {
            auto const& [loc, size, src_offset, dst_offset] = op.get_safe_copy();
            ...
            label = "evict@" + write_with_ss(memloc) + "->sto_id" + write_with_ss(stoloc.id);
            ...
            label = string("load@") + "sto_id" + write_with_ss(stoloc.id) + "->" +
            ...
            string              memloc = write_with_ss(memloc_t{par.offset, par.size, par.loc});
    ...
    for (auto const& node : nodes) {
        ...
        if (node.op.is_inputmem()) {
            ...
            if (apply.is_einsummable()) {
                ...
                throw std::runtime_error("should not reach: need to impl another apply type");
            }
            ...
            auto const& [loc, size, src_offset, dst_offset] = node.op.get_safe_copy();
            es_proto::MGCopy* c = n->mutable_copy();
            c->set_loc(loc);
            c->set_size(size);
            c->set_src_offset(src_offset);
...
memgraph_t memgraph_t::from_proto(es_proto::MemGraph const& mg)
{
    ...
    memgraph_t ret(mg.num_compute_locs(), mg.num_storage_locs(), storage_locs);
    ...
    for (int id = 0; id != mg.nodes_size(); ++id) {
        ...
        if (n.has_inputmem()) {
            ...
            op = op_t(inputmem_t{.loc = i.loc(), .offset = i.offset(), .size = i.size()});
        } else if (n.has_inputsto()) {
            auto const& i = n.inputsto();
            op = op_t(inputsto_t{
                .storage_loc = i.storage_loc(), .storage_id = i.storage_id(), .size = i.size()});
        } else if (n.has_constant()) {
            auto const& c = n.constant();
            auto const& f = c.fill();

            op = op_t(
                constant_t{.loc = c.loc(), .offset = c.offset(), .fill = fill_t::from_proto(f)});
        } else if (n.has_apply()) {
            ...
            for (int i = 0; i != nmem; ++i) {
                mems.push_back(mem_t{.offset = a.mems_offset(i), .size = a.mems_size(i)});
            }
            ...
            op = op_t(apply_t{.loc = a.loc(), .mems = mems, .op = aop, .group = a.group()});
        } else if (n.has_move()) {
            auto const& m = n.move();
            op = op_t(move_t{.src = {m.src_loc(), m.src_offset()},
                             .dst = {m.dst_loc(), m.dst_offset()},
                             .size = m.size()});
        } else if (n.has_copy()) {
            auto const& c = n.copy();
            op = op_t(copy_t{.loc = c.loc(),
                             .size = c.size(),
                             .src_offset = c.src_offset(),
                             .dst_offset = c.dst_offset()});
        } else if (n.has_copy()) {
            auto const& c = n.copy();
            op = op_t(safe_copy_t{.loc = c.loc(),
                             .size = c.size(),
                             .src_offset = c.src_offset(),
                             .dst_offset = c.dst_offset()});
        } else if (n.has_evict()) {
            ...
            op = op_t(
                evict_t{.src = memloc_t{.offset = e.offset(), .size = e.size(), .loc = e.loc()},
                        .dst = stoloc_t{.loc = e.storage_loc(), .id = e.storage_id()}});
        } else if (n.has_load()) {
            auto const& l = n.load();
            op = op_t(
                load_t{.src = stoloc_t{.loc = l.storage_loc(), .id = l.storage_id()},
                       .dst = memloc_t{.offset = l.offset(), .size = l.size(), .loc = l.loc()}});
        } else if (n.has_partialize()) {
            auto const& p = n.partialize();
            op = op_t(partialize_t{.loc = p.loc(), .offset = p.offset(), .size = p.size()});
        } else if (n.has_alloc()) {
            auto const& a = n.alloc();
            op = op_t(alloc_t{.loc = a.loc(), .offset = a.offset(), .size = a.size()});
        } else if (n.has_del()) {
            auto const& d = n.del();
            op = op_t(del_t{.loc = d.loc(), .offset = d.offset(), .size = d.size()});
        } else {
...
vector<int> memgraph_t::get_locs_from_storage_loc(int sto_loc) const
{
    vector<int> ret;
...
memgraph_t::inputmem_t memgraph_t::inputmem_t::from_memloc(memloc_t const& m)
{
    return inputmem_t{.loc = m.loc, .offset = m.offset, .size = m.size};
}

memgraph_t::partialize_t memgraph_t::partialize_t::from_memloc(memloc_t const& m)
{
    return partialize_t{.loc = m.loc, .offset = m.offset, .size = m.size};
}

memgraph_t::alloc_t memgraph_t::alloc_t::from_memloc(memloc_t const& m)
{
    return alloc_t{.loc = m.loc, .offset = m.offset, .size = m.size};
}

memgraph_t::del_t memgraph_t::del_t::from_memloc(memloc_t const& m)
{
    return del_t{.loc = m.loc, .offset = m.offset, .size = m.size};
}
...
void memgraph_t::op_t::check_safe_copy() const {}
...
vector<memloc_t> memgraph_t::op_t::get_memlocs() const
{
    if (is_inputmem()) {
        ...
        auto const& [src_loc, src_offset] = move.src;
        auto const& [dst_loc, dst_offset] = move.dst;
        return {memloc_t{.offset = src_offset, .size = move.size, .loc = src_loc},
                memloc_t{.offset = dst_offset, .size = move.size, .loc = dst_loc}};
    } else if (is_copy()) {
        auto const& [loc, size, src_offset, dst_offset] = get_copy();
        return {memloc_t{.offset = src_offset, .size = size, .loc = loc},
                memloc_t{.offset = dst_offset, .size = size, .loc = loc}};
    } else if (is_safe_copy()) {
        auto const& [loc, size, src_offset, dst_offset] = get_safe_copy();
        return {memloc_t{.offset = src_offset, .size = size, .loc = loc},
                memloc_t{.offset = dst_offset, .size = size, .loc = loc}};
    } else if (is_evict()) {
        ...
        return {par.as_memloc()};
    } else if (is_alloc()) {
        auto const& alloc = get_alloc();
        return {alloc.as_memloc()};
    } else if (is_del()) {
        auto const& del = get_del();
        return {del.as_memloc()};
    } else {
...
memstoloc_t memgraph_t::op_t::get_output_memstoloc() const
{
    if (is_inputmem()) {
        ...
        auto const& move = get_move();
        auto const& [dst_loc, dst_offset] = move.dst;
        return memloc_t{.offset = dst_offset, .size = move.size, .loc = dst_loc};
    } else if (is_copy()) {
        auto const& copy = get_copy();
        return memloc_t{.offset = copy.dst_offset, .size = copy.size, .loc = copy.loc};
    } else if (is_safe_copy()) {
        auto const& copy = get_safe_copy();
        return memloc_t{.offset = copy.dst_offset, .size = copy.size, .loc = copy.loc};
    } else if (is_evict()) {
...
einsummable_t const& memgraph_t::apply_t::get_einsummable() const
{
    return std::get<einsummable_t>(op);
}

touch_t const& memgraph_t::apply_t::get_touch() const
{
    return std::get<touch_t>(op);
}

dtype_t memgraph_t::apply_t::out_dtype() const
{
    if (is_einsummable()) {
...
vector<uint64_t> memgraph_t::get_numbyte_on_evict() const
{
