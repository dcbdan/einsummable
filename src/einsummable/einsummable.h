...
struct einsummable_t {
    ...
    // the first option.
    einsummable_t(vector<uint64_t>     join_shape,
                  vector<vector<int>>  inns,
                  int                  out_rank,
                  scalarop_t           join,
    ...
    einsummable_t replace_scalar_variables(map<string, scalar_t> const& vars) const;
    ...
    // ij,jk->ik
    // 02 21  01
    static einsummable_t
    from_matmul(uint64_t di, uint64_t dj, uint64_t dk, dtype_t dtype = default_dtype());
    ...
    static einsummable_t aggregate(vector<uint64_t> const& inn_shape,
                                   vector<int> const&      inn,
                                   int                     out_rank,
                                   dtype_t                 dtype = default_dtype(),
                                   castable_t              castable = castable_t::add);

    static einsummable_t with_new_shape(einsummable_t const&    e,
                                        vector<uint64_t> const& new_join_shape);

    static tuple<vector<vector<int>>, int> parse_str(string einsummable_str);

    static tuple<string, vector<string>> make_str_terms(vector<vector<int>> const& inns,
    ...
    static bool valid_inns_out(vector<vector<int>> const& inns, int out_rank);
    ...
    static optional<vector<uint64_t>>
    construct_join_shape(vector<vector<int>> const&      inns,
                         vector<vector<uint64_t>> const& inn_shapes);

    static vector<uint64_t> construct_join_shape(vector<uint64_t> const&         out_shape,
    ...
    template <typename T>
    static vector<T> get_input_from_join_(vector<int> const& inn, vector<T> const& join_ts)
    {
        vector<T> ret;
    ...
    template <typename T>
    static vector<vector<T>> get_inputs_from_join_(vector<vector<int>> const& inns,
    ...
    static optional<vector<T>> construct_join_shape_(optional<vector<T>> const& maybe_out,
                                                     vector<vector<int>> const& inns,
    ...
    template <typename T, typename F>
    static optional<vector<T>> construct_join_shape_(vector<vector<int>> const& inns,
                                                     vector<vector<T>> const&   inn_shapes,
                                                     T const&                   unassigned,
                                                     F                          equals)
    {
