...
struct multiple_tensor_t {
    ...
    multiple_tensor_t(partition_t p, vtensor_t<vector<locid_t>>&& t);
...
partition_t convert_squeezer_partition(vector<uint64_t> new_shape, partition_t const& part)
{
...
struct taskgraph_make_state_t {
    ...
    vector<std::unordered_map<tg_region_t, vector<multiple_tensor_t::locid_t>>> access_cache;

    int access(int gid, vector<tuple<uint64_t, uint64_t>> const& hrect, int loc);
    // ^ wrt real

    void copy_from_refinement(vector<uint64_t> const& offset_inn_rel,
    ...
    multiple_tensor_t
    form_from_refinement(int gid, multiple_placement_t const& placement, bool wrt_graph = true);
    ...
    vtensor_t<int>
    form_from_refinement(int gid, placement_t const& placement, bool wrt_graph = true);
    ...
    multiple_tensor_t construct_refinement_tensor(dtype_t                     dtype,
...
tuple<map<int, vtensor_t<int>>, // for each input, the tids of the blocks
      map<int, vtensor_t<int>>, // for each save id, the tids of the blocks
      taskgraph_t>              // the actual taskgraph
taskgraph_t::make(graph_t const& graph, vector<placement_t> const& placements)
{
    ...
    auto correct_inns_and_saves = [&](taskgraph_t const& tg, map<int, int> const& to_new_tg) {
...
int taskgraph_make_state_t::access(int gid, vector<tuple<uint64_t, uint64_t>> const& hrect, int loc)
{
    auto const& refined_tensor = refined_tensors.at(gid);
    auto const& refined_part = refined_tensor.partition;

    tg_region_t key = {refined_part.get_exact_region(hrect)};
    ...
    do {
        ...
        // form the touch
        touch_t touch{.selection = make_touch_selection_from_full_small(hrect, inn_hrect),
    ...
    locids.push_back(multiple_tensor_t::locid_t{.loc = loc, .id = builder_id});

    return builder_id;
}

void taskgraph_make_state_t::copy_from_refinement(vector<uint64_t> const& offset_inn,
                                                  vector<uint64_t> const& offset_out,
                                                  vector<uint64_t> const& size,
                                                  int                     inn_gid,
                                                  int                     out_partial)
{
    ...
    for (int i = 0; i != rank; ++i) {
        ...
        base_pds.push_back(
            partdim_t::from_sizes(vector<uint64_t>(sizes.begin() + b, sizes.begin() + e)));
    }
    ...
    do {
        ...
        vector<uint64_t> actual_offset_out = vector_add(offset_out, offset_base);

        touch_t touch{.selection = {}, .castable = std::nullopt, .dtype = dtype};
        auto&   selection = touch.selection;
        selection.reserve(size.size());
        for (int i = 0; i != size.size(); ++i) {
            selection.push_back(touchdim_t{.d_inn = inn_shape[i],
                                           .d_out = out_shape[i],
                                           .offset_inn = 0,
...
multiple_tensor_t
taskgraph_make_state_t::form_from_refinement(int                         gid,
                                             ...
                                             bool                        wrt_graph)
{
    ...
    if (dtype_is_complex(dtype)) {
        if (wrt_graph) {
            auto ret = form_from_refinement(gid, double_last_dim(placement), false);
...
vtensor_t<int>
taskgraph_make_state_t::form_from_refinement(int gid, placement_t const& pl, bool wrt_graph)
{
    multiple_placement_t mpl = multiple_placement_t::from_single_placement(pl);
...
//   rs  { 3, 4, 8 }
vector<tuple<int, int>> _get_sum_breaks(vector<uint64_t> coarse, vector<uint64_t> fine)
{
...
vtensor_t<int> taskgraph_make_state_t::form_select(int gid)
{
    graph_t::node_t const& node = graph.nodes[gid];
    ...
    do {
        ...
        if (rel_inn_hrects.size() == 1) {
            ...
            for (auto const& [rel_inn_hrect, which_inn] : rel_inn_hrects) {
                ...
                vector<uint64_t> rel_out_offset =
                    select.wrt_output_point(rel_inn_offset, which_inn);
                ...
                copy_from_refinement(vector_mapfst(rel_inn_hrect), // B
                                     out_offset,                   // A
...
vtensor_t<int> taskgraph_make_state_t::compute_input(int gid)
{
    graph_t::node_t const& node = graph.nodes[gid];
...
vtensor_t<int> taskgraph_make_state_t::compute_einsummable(int gid)
{
    graph_t::node_t const& node = graph.nodes[gid];
    placement_t const&     pl = placements[gid];
    ...
    for (int which_input = 0; which_input != node.inns.size(); ++which_input) {
        ...
        auto inn_placement =
            multiple_placement_t::make_einsummable_input(pl, base_einsummable, which_input);
    ...
    do {
        ...
        ret.at(index) = taskgraph.insert_einsummable(
            loc, einsummable_t::with_new_shape(base_einsummable, subtensor_shape), inns);
    } while (increment_idxs(shape, index));

    return ret;
}

vtensor_t<int> taskgraph_make_state_t::compute_fill(int gid)
{
    graph_t::node_t const& node = graph.nodes[gid];
    placement_t const&     pl = placements[gid];
    ...
    do {
        ...
        ret.at(index) = taskgraph.insert_constant(loc, full_fill.select(hrect));
    } while (increment_idxs(shape, index));

    return ret;
}

vtensor_t<int> taskgraph_make_state_t::form_relation(int gid)
{
    ...
    if (node.op.is_squeezer()) {
        ...
        placement_t inn_pl(inn_part, vtensor_t<int>(inn_part.block_shape(), pl.locations.get()));
...
void taskgraph_make_state_t::communicate(int join_gid, vtensor_t<int> join_result)
{
    ...
    if (dtype_is_complex(dtype)) {
        if (maybe_castable && maybe_castable.value() != castable_t::add) {
            ...
            multiple_tensor_t ret = construct_refinement_tensor(dtype,
                                                                placements[join_gid],
                                                                join_result,
            ...
            multiple_tensor_t ret = construct_refinement_tensor(complex_to_real(dtype),
                                                                join_placement,
                                                                join_result,
                                                                usage_placement,
                                                                maybe_castable);
        ...
        multiple_tensor_t ret = construct_refinement_tensor(
            dtype, placements[join_gid], join_result, usage_placement, maybe_castable);
...
construct_refinement_placement(graph_t const&                         graph,
                               int                                    join_gid,
                               std::function<placement_t const&(int)> get_placement)
{
    ...
    for (auto const& out_gid : join_node.outs) {
        ...
        if (out_node.op.is_formation()) {
            insert_usage(multiple_placement_t::from_single_placement(out_pl));
        } else if (out_node.op.is_complexer()) {
            if (join_is_complex) {
                // complex -> real
                usage_placements.push_back(multiple_placement_t::from_single_placement(out_pl));
            } else {
                // real -> complex
            ...
            partition_t fix_partition = convert_squeezer_partition(inn_shape, out_pl.partition);
            insert_usage(multiple_placement_t::from_single_placement(
                placement_t(fix_partition,
                            vtensor_t<int>(fix_partition.block_shape(), out_pl.locations.get()))));
            ...
            for (int which_input = 0; which_input != out_node.inns.size(); ++which_input) {
                if (out_node.inns[which_input] == join_gid) {
                    insert_usage(multiple_placement_t::make_einsummable_input(
                        out_pl, out_node.op.get_einsummable(), which_input));
                }
            }
            ...
            for (int which_input = 0; which_input != out_node.inns.size(); ++which_input) {
                if (out_node.inns[which_input] == join_gid) {
                    insert_usage(multiple_placement_t::make_select_input(
                        out_pl, out_node.op.get_select(), which_input));
                }
            }
        } else {
            throw std::runtime_error(
...
multiple_tensor_t
taskgraph_make_state_t::construct_refinement_tensor(dtype_t                     dtype,
                                                    placement_t const&          join_placement,
                                                    vtensor_t<int>              join_result,
                                                    multiple_placement_t const& refinement,
                                                    optional<castable_t>        maybe_castable)
{
    ...
    do {
        ...
        do {
            ...
            vector<uint64_t> selection_shape = vector_from_each_member(subset_info, uint64_t, size);
            vector<uint64_t> refinement_shape = refinement.partition.tensor_shape_at(r_index);
            ...
            vector<regiondim_t> _out_to_selection_rs;
            {
                ...
                for (int i = 0; i != subset_info.size(); ++i) {
                    _out_to_selection_rs.push_back(regiondim_t{.dim = out_tensor_shape[i],
            ...
            vector<touchdim_t> _selection_to_refinement_ts;
            {
                ...
                for (int i = 0; i != subset_info.size(); ++i) {
                    _selection_to_refinement_ts.push_back(
                        touchdim_t{.d_inn = selection_shape[i],
            ...
            touch_t _selection_to_refinement_touch{.selection = _selection_to_refinement_ts,
            ...
            vector<touchdim_t> _out_to_refinement_ts;
            {
                ...
                for (int i = 0; i != subset_info.size(); ++i) {
                    _out_to_refinement_ts.push_back(
                        touchdim_t{.d_inn = out_tensor_shape[i],
                                   .d_out = refinement_shape[i],
            ...
            touch_t _out_to_refinement_touch{
                .selection = _out_to_refinement_ts, .castable = maybe_castable, .dtype = dtype};
            // ref_id by reference!
...
multiple_placement_t::multiple_placement_t(partition_t const& pa, vtensor_t<set<int>> const& ls)
...
multiple_placement_t multiple_placement_t::from_single_placement(placement_t const& p)
{
    ...
    return multiple_placement_t(p.partition, vtensor_t<set<int>>(p.locations.get_shape(), locs));
}
...
multiple_placement_t multiple_placement_t::make_refinement(vector<placement_t> const& ps)
{
    ...
    return multiple_placement_t(std::move(partition), std::move(locations));
}

multiple_placement_t multiple_placement_t::make_refinement(vector<multiple_placement_t> const& ps)
{
    if (ps.size() == 0) {
...
multiple_placement_t multiple_placement_t::make_einsummable_input(placement_t const& join_placement,
                                                                  einsummable_t const& einsummable,
                                                                  int                  which_input)
{
    partition_t partition(
        einsummable.get_input_from_join(join_placement.partition.partdims, which_input));
...
multiple_placement_t multiple_placement_t::make_select_input(placement_t const& join_pl,
                                                             select_t const&    select,
                                                             int                which_input)
{
    placement_t        subset_pl = join_pl.subset(select.wrt_output_inn_hrect(which_input));
...
multiple_tensor_t::multiple_tensor_t(partition_t p, vtensor_t<vector<locid_t>>&& t)
    ...
    : partition(p.partition), tensor(p.partition.block_shape())
{
    ...
    do {
        ...
        for (auto const& loc : locs) {
            tensor_at.push_back(locid_t{.loc = loc, .id = init_value});
        }
    } while (increment_idxs(shape, index));
}
...
int taskgraph_t::insert_input(int loc, dtype_t dtype, vector<uint64_t> shape, bool is_save)
{
    input_t input{
        .loc = loc,
...
int taskgraph_t::insert_constant(int loc, fill_t const& fill, bool is_save)
{
    constant_t constant{
        .loc = loc,
        .fill = fill,
...
int taskgraph_t::insert_einsummable(int loc, einsummable_t e, vector<int> inns, bool is_save)
{
    apply_t apply{.loc = loc, .inns = inns, .einsummable = e};
...
int taskgraph_t::insert_move(int src, int dst, int inn, bool is_save)
{
    move_t move{.src = src, .dst = dst, .inn = inn, .size = nodes[inn].op.out_size()};

    return insert(move, is_save);
}

int taskgraph_t::insert_consumed_aggregate(int         loc,
                                           dtype_t     dtype,
                                           castable_t  castable,
                                           vector<int> inns,
                                           bool        is_save)
{
    ...
    for (auto const& inn : inns) {
        ...
        inputs.push_back(input_op_t{
            .id = inn, .consumable = true, .region = {inn_regiondim_t{.dim = nelem, .offset = 0}}});
    }

    auto unit = partial_unit_t{.castable = castable,
                               .out_region = {out_regiondim_t{.offset = 0, .size = nelem}},
                               .inputs = inputs};

    return insert(partialize_t{.loc = loc, .dtype = dtype, .write_shape = {nelem}, .units = {unit}},
                  is_save);
}

int taskgraph_t::insert_select_subset(int                 loc,
                                      ...
                                      bool                is_save)
{
    ...
    for (auto const& [dim_inn, offset_inn, size] : selection) {
        ...
        ts.push_back(touchdim_t{.d_inn = dim_inn,
                                .d_out = size,
    ...
    touch_t touch{.selection = ts, .castable = optional<castable_t>(), .dtype = dtype};
...
int taskgraph_t::new_partial(int loc, dtype_t dtype, vector<uint64_t> write_shape, bool is_save)
{
    return insert(partialize_t{.loc = loc, .dtype = dtype, .write_shape = write_shape, .units = {}},
                  is_save);
}

void taskgraph_t::add_to_partial(int id_out, int id_inn, touch_t touch, bool consume)
{
    using inn_regiondim_t = partialize_t::inn_regiondim_t;
    using out_regiondim_t = partialize_t::out_regiondim_t;
    using input_op_t = partialize_t::input_op_t;
    ...
    for (auto const& [d_inn, _, offset_inn, offset_out, size] : touch.selection) {
        region_out.push_back(out_regiondim_t{.offset = offset_out, .size = size});
        region_inn.push_back(inn_regiondim_t{.dim = d_inn, .offset = offset_inn});
    }

    input_op_t input{.id = id_inn, .consumable = consume, .region = region_inn};
    ...
    if (!found) {
        partialize.units.push_back(partial_unit_t{
            .castable = touch.castable, .out_region = region_out, .inputs = {input}});
    }
...
void taskgraph_t::add_to_partial_the_full_aggregate(int        id_out,
                                                    int        id_inn,
                                                    castable_t castable,
                                                    bool       consume)
{
    ...
    for (int i = 0; i != write_shape.size(); ++i) {
        ts.push_back(touchdim_t{.d_inn = write_shape[i],
                                .d_out = write_shape[i],
                                .offset_inn = 0,
                                .offset_out = 0,
                                .size = write_shape[i]});
    ...
    touch_t touch{.selection = ts, .castable = optional<castable_t>(castable), .dtype = dtype};
...
vector<tuple<int, touch_t>>
_reach_past(taskgraph_t const& taskgraph, set<int> const& pts, int root_id)
{
...
optional<tuple<map<int, int>, taskgraph_t>> taskgraph_t::remove_passthrough_partials() const
{
    ...
    auto f_remap = [&remap](int id) { return remap.at(id); };
...
tuple<map<int, int>, taskgraph_t> taskgraph_t::prune() const
{
    set<int> keep_nodes;
    ...
    auto          f_xid_to_yid = [&](int xid) -> int { return xid_to_yid.at(xid); };
...
tuple<vector<int>, vector<int>> taskgraph_t::get_input_core_order() const
{
...
taskgraph_t taskgraph_t::from_wire(string const& str)
{
    ...
    for (int id = 0; id != tg.nodes_size(); ++id) {
        ...
        if (n.has_input()) {
            ...
            ret.nodes.emplace_back(op_t(input_t{i.loc(), i.size()}), is_save);
        } else if (n.has_apply()) {
            ...
            ret.nodes.emplace_back(op_t(apply_t{a.loc(), inns, e}), is_save);
        } else if (n.has_move()) {
            auto const& m = n.move();
            ret.nodes.emplace_back(op_t(move_t{m.src(), m.dst(), m.inn(), m.size()}), is_save);
        } else if (n.has_constant()) {
            auto const& c = n.constant();
            auto const& f = c.fill();

            ret.nodes.emplace_back(op_t(constant_t{c.loc(), fill_t::from_proto(f)}), is_save);
            ...
            for (auto const& u : p.units()) {
                ...
                for (auto const& o : u.out_region()) {
                    out_region.push_back(
                        partialize_t::out_regiondim_t{.offset = o.offset(), .size = o.size()});
                }
                ...
                for (auto const& ii : u.inputs()) {
                    ...
                    for (auto const& i : ii.region()) {
                        inn_region.push_back(
                            partialize_t::inn_regiondim_t{.dim = i.dim(), .offset = i.offset()});
                    }

                    inputs.push_back(partialize_t::input_op_t{
                        .id = ii.id(), .consumable = ii.consumable(), .region = inn_region});
                }

                units.push_back(partialize_t::partial_unit_t{
                    .castable = castable, .out_region = out_region, .inputs = inputs});
            }

            dtype_t dtype = parse_with_ss<dtype_t>(p.dtype());

            partialize_t partialize{
                .loc = p.loc(), .dtype = dtype, .write_shape = write_shape, .units = units};
...
void taskgraph_t::_replace_with_new_node(int tid, taskgraph_t::op_t const& op)
{
...
{
    constant_t constant{.loc = out_loc(tid), .fill = fill};
...
bool taskgraph_t::_replace_apply(int tid)
{
    ...
    scalarop_t new_join = scalarop_t::combine(einsummable.join, innops);
    ...
    if (new_join.is_constant()) {
        ...
        fill_t fill(fill_t::constant_t{.value = val, .shape = einsummable.out_shape()});
    ...
    if (einsummable.has_aggregation()) {
        ...
        for (int agg_mode = einsummable.out_rank; agg_mode != einsummable.join_shape.size();
             ++agg_mode) {
        ...
        if (missing_agg_size > 1) {
            ...
            if (castable == castable_t::max || castable == castable_t::min) {
                ...
                scalar_t real_missing_agg_size(new_join.out_dtype(),
                                               write_with_ss(1.0 * missing_agg_size));
                new_join =
                    scalarop_t::combine(scalarop_t::make_scale(real_missing_agg_size), {new_join});
    ...
    auto new_join_shape =
        einsummable_t::construct_join_shape(einsummable.out_shape(), new_e_inns, new_inn_shapes);

    einsummable_t new_einsummable(
        new_join_shape, new_e_inns, new_out_rank, new_join, einsummable.castable);

    apply_t new_apply{.loc = apply.loc, .inns = new_inns, .einsummable = new_einsummable};

    _replace_with_new_node(tid, op_t(new_apply));
    return true;
}

taskgraph_t::_unit_simplify_t::_unit_simplify_t(taskgraph_t const&    tg,
                                                dtype_t               dtype,
...
vector<taskgraph_t::partialize_t::input_op_t> taskgraph_t::_unit_simplify_t::get_inputs() const
{
...
optional<scalar_t> taskgraph_t::_unit_simplify_t::get_constant_at(
...
bool taskgraph_t::_unit_simplify_t::is_annihilator(dtype_t dtype, castable_t castable, scalar_t val)
{
    if (val.dtype != dtype) {
        return false;
    }
    if (dtype_is_complex(dtype) && (castable == castable_t::max || castable == castable_t::min)) {
...
bool taskgraph_t::_unit_simplify_t::is_identity(dtype_t dtype, castable_t castable, scalar_t val)
{
    if (val.dtype != dtype) {
        return false;
    }
...
bool taskgraph_t::_replace_partialize(int tid)
{
    ...
    if (simples[0].is_constant()) {
        ...
        if (is_all_constant) {
            _replace_with_fill(
                tid, fill_t(fill_t::constant_t{.value = val, .shape = partialize.write_shape}));
            return true;
        }
    }
    ...
    for (int i = 0; i != partialize.units.size(); ++i) {
        ...
        new_units.push_back(partial_unit_t{.castable = unit.castable,
                                           .out_region = unit.out_region,
                                           .inputs = simple.get_inputs()});
    }

    partialize_t new_partialize{.loc = partialize.loc,
...
bool operator==(taskgraph_t::partialize_t::out_regiondim_t const& lhs,
...
taskgraph_t::op_t taskgraph_t::op_t::remap(std::function<int(int)> to_new_tid) const
{
    if (is_input()) {
        return *this;
    }
...
taskgraph_t::partialize_t::make_from_touches(int                                loc,
                                             vector<tuple<int, touch_t>> const& inn_touches)
{
    ...
    vector<uint64_t> write_shape =
        vector_from_each_member(std::get<1>(inn_touches[0]).selection, uint64_t, d_out);

    // make sure all the touches have the same write shape
    for (int i = 1; i != inn_touches.size(); ++i) {
        vector<uint64_t> write_shape_ =
            vector_from_each_member(std::get<1>(inn_touches[i]).selection, uint64_t, d_out);
            ...
            for (int i = 0; i != rank; ++i) {
                ...
                if (td.offset_out == 0) {
                    ...
                    if (td.offset_out + td.size == td.d_out) {
                        pds[i].push_back(partdim_t::from_spans({td.offset_out, td.d_out}));
                    } else {
                        pds[i].push_back(partdim_t::from_spans(
                            {td.offset_out, td.offset_out + td.size, td.d_out}));
                    }
                }
            }
        }
    ...
    do {
        ...
        for (int i = 0; i != rank; ++i) {
            auto [b, e] = hrect[i];
            unit.out_region.push_back(out_regiondim_t{.offset = b, .size = e - b});
        }
    } while (increment_idxs(block_shape, idx));
    ...
    for (auto const& [inn_id, touch] : inn_touches) {
        ...
        do {
            ...
            unit.inputs.push_back(input_op_t{.id = inn_id, .consumable = false, .region = {}});
            ...
            for (int i = 0; i != rank; ++i) {
                ...
                inn_rds.push_back(
                    inn_regiondim_t{.dim = td.d_inn, .offset = td.offset_inn + (hb - fb)});
            }
        } while (increment_idxs_region(region, idx));
    }

    partialize_t ret{
        .loc = loc, .dtype = dtype, .write_shape = write_shape, .units = std::move(units.get())};
...
void taskgraph_t::partialize_t::make_parallel()
{
    ...
    for (auto& unit : units) {
        ...
        for (int i = 0; i != n; ++i) {
            ...
            sub_out_region[0] =
                out_regiondim_t{.offset = unit.out_region[0].offset + offset, .size = sub_size};
            ...
            ret.push_back(partial_unit_t{
                .castable = unit.castable, .out_region = sub_out_region, .inputs = sub_inputs});
        }
    }
...
tuple<int, touch_t> taskgraph_t::partialize_t::get_touch(int which_unit, int which_touch) const
{
    ...
    ts.reserve(write_shape.size());
    for (int i = 0; i != write_shape.size(); ++i) {
        ts.push_back(touchdim_t{.d_inn = input.region[i].dim,
                                .d_out = write_shape[i],
...
bool taskgraph_t::partialize_t::is_straight_copy() const
{
    if (units.size() == 1 && units[0].inputs.size() == 1) {
        ...
        for (auto const& t : touch.selection) {
            if (t.d_inn == t.d_out && t.offset_inn == t.offset_out && t.size == t.d_inn) {
...
vector<vector<uint64_t>> taskgraph_t::partialize_t::inn_shapes_of(int inn_id) const
{
...
void taskgraph_t::print_graphviz(std::ostream& out) const
{
    vector<string> colors{
        "#61B292", "#AED09E", "#F1E8A7", "#A8896C", "#A8D8EA", "#AA96DA", "#FCBAD3", "#FFFFD2"};

    print_graphviz(out, colors);
}
void taskgraph_t::print_graphviz(std::ostream& out, vector<string> const& colors) const
{
    using std::endl;
    ...
    for (int id = 0; id != nodes.size(); ++id) {
        ...
        if (op.is_input()) {
            ...
            label =
                "apply" + write_with_ss(id) + "@loc[" + write_with_ss(loc) + "]" + write_with_ss(e);
        } else if (op.is_move()) {
            auto const& [src, dst, _0, _1] = node.op.get_move();
