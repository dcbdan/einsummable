...
bool merge_into_nodeconst_map(map<int, node_t const*>& ret, map<int, node_t const*> const& values)
{
...
static tuple<scalar_t, node_t const*> _pop_mul_scale(node_t const* node)
{
    if (node->op.is_mul()) {
...
static optional<tuple<simple_scalarop_t::unary_t, node_t const*>>
...

    {
        ...
        for (auto const& [uop, m] : ms) {
            ...
            if (maybe) {
                ...
                return value_t{unary_t{.scale = scale, .op = uop}, val.at(0)};
            }
        }
    }
...
static tuple<simple_scalarop_t::unary_t, node_t const*> _pop_unary_success(node_t const* node)
{
    {
    ...
    // if so, we can simplify it to neg(x)
    {
        if (node->op.is_mul()) {
            ...
            if (lhs->op.is_constant() && lhs->op.get_constant().str() == "-1") {
                return {simple_scalarop_t::unary_t{.scale = scalar_t::one(node->dtype),
...
static optional<tuple<simple_scalarop_t, vector<node_t const*>>>
_pop_with_binary_simple_scalarop(simple_scalarop_t::bop_t bop, node_t const* lhs, node_t const* rhs)
{
    using value_t = tuple<simple_scalarop_t, vector<node_t const*>>;

    if (lhs->op.is_constant()) {
        simple_scalarop_t::scale_t op{.bop = bop, .scale = lhs->op.get_constant()};
        return value_t{simple_scalarop_t{.op = op}, {rhs}};
    }
    if (rhs->op.is_constant()) {
    ...
    simple_scalarop_t::binary_t op{.op = bop, .lhs = unary_lhs, .rhs = unary_rhs};
    return value_t{simple_scalarop_t{.op = op}, {child_lhs, child_rhs}};
}

optional<tuple<simple_scalarop_t, vector<node_t const*>>> pop_to_simple_scalarop(node_t const* node)
{
    using value_t = tuple<simple_scalarop_t, vector<node_t const*>>;
    ...
    for (bop_t bop : {bop_t::add, bop_t::mul, bop_t::min, bop_t::max}) {
        ...
        if (maybe) {
            auto const& val = maybe.value();
            return _pop_with_binary_simple_scalarop(bop, val.at(0), val.at(1));
        }
    }
    ...
    if (maybe) {
        ...
        return value_t{simple_scalarop_t{.op = unary}, {child}};
    }
...
{
    return scalarop_t::combine(scalarop_t::make_scale(u.scale),
...
scalarop_t simple_scalarop_t::to_scalarop() const
{
    if (is_scale()) {
        ...
        return scalarop_t::combine(
            ...
            {scalarop_t::make_identity(constant.dtype), scalarop_t::make_constant(constant)});
    } else if (is_unary()) {
        return unary_to_scalarop(get_unary());
    } else if (is_binary()) {
        auto const& [bop, lhs, rhs] = get_binary();
        return scalarop_t::combine(bop_to_scalarop(bop, lhs.scale.dtype),
...
scalarop_t list_simple_scalarop_t::to_scalarop() const
{
    ...
    for (auto const& [simple_op, args] : ops) {
        if (simple_op.is_unary() || simple_op.is_scale()) {
            ...
            auto result = scalarop_t::combine(simple_op.to_scalarop(), {get(args[0], inn_dtype)});
            ...
            auto    result = scalarop_t::replace_arguments(
                simple_op.to_scalarop(), {get(args[0], lhs_dtype), get(args[1], rhs_dtype)});
            results.push_back(result);
        } else {
            throw std::runtime_error("simple_op not unary and not binary?");
        }
...
struct list_maker_t {
    ...
    optional<vector<info_t>> add_op(scalar_ns::node_t const* node)
    {
        ...
        ops.push_back(list_simple_scalarop_t::op_t{.op = scalarop});

        vector<info_t> children;
        for (int arg = 0; arg != child_nodes.size(); ++arg) {
            children.push_back(
                info_t{.out_id = ret_id, .which_arg = arg, .node = child_nodes[arg]});
        }
        return children;
    }
...
optional<list_simple_scalarop_t> list_simple_scalarop_t::make(scalarop_t const& scalarop)
{
    // TODO: we put here to filter out all the identities
    // check if this is the best place for it
    if (scalarop.is_identity()) {
        op_t op = op_t{
            .op = simple_scalarop_t::unary_t{.scale = scalar_t::one(scalarop.out_dtype()),
                                             .op = simple_scalarop_t::uop_t::identity},
        };
        op.args[0] = 0;
        return list_simple_scalarop_t{.ops = {op}};
    }
    ...
    list_simple_scalarop_t ret{.ops = maker.ops};
...
std::ostream& operator<<(std::ostream& out, simple_scalarop_t const& op)
{
    if (op.is_scale()) {
        ...
        auto const& [bop, lhs, rhs] = op.get_binary();
        out << bop << "(" << lhs.scale << " * " << lhs.op << "(x), " << rhs.scale << " * " << rhs.op
            << "(y))";
    } else {
...
{
    switch (op) {
        case simple_scalarop_t::identity:
            out << "identity";
            break;
        case simple_scalarop_t::neg:
            out << "neg";
            break;
        case simple_scalarop_t::sqrt:
            out << "sqrt";
            break;
        case simple_scalarop_t::conj:
            out << "conj";
            break;
        case simple_scalarop_t::rcp:
            out << "rcp";
            break;
        case simple_scalarop_t::sigmoid:
            out << "sigmoid";
            break;
        case simple_scalarop_t::log:
            out << "log";
            break;
        case simple_scalarop_t::exp:
            out << "exp";
            break;
        case simple_scalarop_t::relu:
            out << "relu";
            break;
    }
    return out;
...
{
    switch (op) {
        case simple_scalarop_t::add:
            std::cout << "add";
            break;
        case simple_scalarop_t::mul:
            std::cout << "mul";
            break;
        case simple_scalarop_t::min:
            std::cout << "min";
            break;
        case simple_scalarop_t::max:
            std::cout << "max";
            break;
    }
    return out;
