...
optional<vector<tuple<uint64_t, set<int>>>> allocator_t::allocate_multiple(vector<uint64_t> sizes)
{
...
optional<uint64_t> allocator_t::allocate_without_deps(uint64_t size)
{
...
optional<tuple<uint64_t, set<int>>> allocator_t::allocate_impl(uint64_t size_without_rem,
                                                               bool     no_deps)
{
    ...
    if (maybe_info) {
        ...
        auto occupied_iter = blocks.insert(
            iter, block_t{.beg = offset, .end = offset + size, .dep = optional<int>()});
        if (size != sz) {
            blocks.insert(occupied_iter + 1,
                          block_t{.beg = offset + size,
...
optional<tuple<uint64_t, set<int>>> allocator_t::allocate(uint64_t size_without_rem)
{
...
bool allocator_t::allocate_at_without_deps(uint64_t offset, uint64_t size)
{
    ...
    auto last = binary_search_find(beg, blocks.end(), [&offset, &size](block_t const& blk) {
    ...
    if (unused_end_size != 0) {
        iter = blocks.insert(iter,
                             block_t{.beg = unused_end,
                                     .end = unused_end + unused_end_size,
                                     .dep = optional<int>(-1)});
    }

    iter = blocks.insert(iter, block_t{.beg = offset, .end = offset + size, .dep = std::nullopt});

    if (unused_begin_size != 0) {
        iter = blocks.insert(iter,
                             block_t{.beg = unused_begin,
...
optional<tuple<uint64_t, uint64_t>> allocator_t::get_allocated_region(uint64_t offset) const
{
    auto iter = binary_search_find(
        blocks.begin(), blocks.end(), [&offset](block_t const& blk) { return blk.beg <= offset; });
    ...
    for (auto const& block : blocks) {
        if (block.dep) {
            ...
            if (next_interval) {
                ...
                new_blocks.push_back(block_t{.beg = beg, .end = end, .dep = -1});
...
optional<set<int>>
allocator_t::_find_best_evict_block_ids(uint64_t size, std::function<int(int)> f_block_score) const
{
...
int allocator_t::_get_block_id(uint64_t const& offset)
{
    ...
    auto ret = binary_search_find(
        blocks.begin(), blocks.end(), [&offset](block_t const& blk) { return offset >= blk.beg; });
    if (!ret->occupied()) {
...
double allocator_t::buffer_utilization() const
{
    ...
    uint64_t total_memory =
        blocks.empty() ? 0 : blocks.back().end; // Assuming blocks cover all memory.
    ...
    std::cout << "total memory: " << total_memory << "block count: " << blocks.size()
