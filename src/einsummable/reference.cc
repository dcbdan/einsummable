...
map<int, dbuffer_t> reference_compute_graph(graph_t const& graph, map<int, dbuffer_t> const& inputs)
{
...
map<int, buffer_t> reference_compute_taskgraph(taskgraph_t const&        taskgraph,
                                               map<int, buffer_t> const& inputs)
{
    ...
    for (auto const& id : taskgraph.get_order()) {
        ...
        if (node.op.is_input()) {
            ...
            tensors[id] = {apply.loc, reference_einsummable(apply.einsummable, inputs).data};
            ...
            std::copy(buffer_src->data, buffer_src->data + move.size, buffer_dst->data);
            ...
            for (vector<tuple<int, touch_t>> const& ts : node.op.get_touches()) {
                ...
                reference_touch(touch_t{.selection = t0.selection,
...
void reference_compute_memgraph(memgraph_t const&           memgraph,
                                vector<buffer_t>&           compute_location_buffers,
                                vector<map<int, buffer_t>>& storages)
{
    auto make_buffer_at = [&](int loc, mem_t const& mem) {
        return make_buffer_reference(compute_location_buffers[loc]->data + mem.offset, mem.size);
    };
        ...
        if (op.is_inputmem()) {
            ...
            if (apply.is_einsummable()) {
                ...
                for (int i = 1; i != mems.size(); ++i) {
                    ...
                    inn_buffers.push_back(make_dbuffer_at(loc, mems[i], inn_dtype));
                }
                ...
                vector<dbuffer_t> inn_buffers;
                for (int i = 1; i != mems.size(); ++i) {
                    inn_buffers.push_back(make_dbuffer_at(loc, mems[i], touch.dtype));
                }
...
vtensor_t<dbuffer_t> partition_buffer(partition_t const& partition, dbuffer_t const& inn)
{
...
dbuffer_t unpartition_buffer(partition_t const& partition, vtensor_t<dbuffer_t> const& inn)
{
    dtype_t dtype;
    {
...
dbuffer_t reference_einsummable(einsummable_t const& einsummable, vector<dbuffer_t> const& inputs)
{
...
void reference_einsummable_inplace(einsummable_t const&     einsummable,
                                   dbuffer_t                ret,
...
dbuffer_t reference_select(select_t const& select, vector<dbuffer_t> const& inns)
{
...
template <typename T>
std::function<void(T&, T const&)> reference_touch_update(optional<castable_t> const& c)
{
    if (c) {
        ...
        if (op == castable_t::add) {
            ...
            return [](T& v, T a) { v = std::min(v, a); };
        } else if (op == castable_t::max) {
            return [](T& v, T a) { v = std::max(v, a); };
        } else {
            throw std::runtime_error("ref touch update not implemented");
        }
    } else {
        return [](T& out, T const& inn) { out = inn; };
    }
}
...
template <typename T>
void _reference_touch(touch_t const& touch, T* out, T const* inn)
{
    vector<uint64_t> shape = vector_from_each_member(touch.selection, uint64_t, size);
    ...
    vector<uint64_t> offset_inn = vector_from_each_member(touch.selection, uint64_t, offset_inn);
    vector<uint64_t> offset_out = vector_from_each_member(touch.selection, uint64_t, offset_out);
...
void reference_touch(touch_t const& touch, dbuffer_t out, dbuffer_t inn)
{
...
vtensor_t<dbuffer_t> get_partitioned_buffer(map<int, dbuffer_t> items, vtensor_t<int> whiches)
{
...
map<int, dbuffer_t> init_buffer_map(vtensor_t<int> keys, vtensor_t<dbuffer_t> values)
{
...
void fill_buffer_map(map<int, dbuffer_t>& items, vtensor_t<int> keys, vtensor_t<dbuffer_t> values)
{
...
map<int, dbuffer_t> to_typed_buffer_map(map<int, buffer_t> const& bs, map<int, dtype_t> to_dtypes)
{
    map<int, dbuffer_t> ret;
...
map<int, buffer_t> to_untyped_buffer_map(map<int, dbuffer_t> const& dbs)
{
...
map<int, dtype_t> typed_task_ids(graph_t const& graph, map<int, vtensor_t<int>> const& gid_to_tids)
{
    map<int, dtype_t> ret;
...
map<int, dbuffer_t>
typed_reference_compute_taskgraph_from_graph_info(taskgraph_t const&              taskgraph,
                                                  map<int, dbuffer_t> const&      inputs,
                                                  graph_t const&                  graph,
                                                  map<int, vtensor_t<int>> const& save_gid_to_tids)
{
    auto untyped_ret = reference_compute_taskgraph(taskgraph, to_untyped_buffer_map(inputs));
    return to_typed_buffer_map(untyped_ret, typed_task_ids(graph, save_gid_to_tids));
}
