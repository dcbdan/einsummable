...
struct allocator_t {
    ...
    optional<uint64_t> allocate_without_deps(uint64_t size);

    // Return whehter or not the allocate was successful
    [[nodiscard]] bool allocate_at_without_deps(uint64_t offset, uint64_t size);

    void set_strategy(allocator_strat_t s)
    {
        strat = s;
    };
    allocator_strat_t get_strategy() const
    {
        return strat;
    }
    ...
    optional<tuple<uint64_t, uint64_t>> get_allocated_region(uint64_t offset) const;
...
private:
    ...
    optional<tuple<iter_t, iter_t, uint64_t>> find_lowest_dependency_available(uint64_t size);

    optional<tuple<uint64_t, set<int>>> allocate_impl(uint64_t size_without_rem, bool no_deps);

    optional<tuple<iter_t, iter_t, uint64_t>> find_first_available(uint64_t size);
