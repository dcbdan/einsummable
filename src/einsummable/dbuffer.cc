...
dbuffer_t::dbuffer_t() : dtype(default_dtype()), data(nullptr) {}

dbuffer_t::dbuffer_t(dtype_t d, buffer_t b) : dtype(d), data(b)
{
...
template <typename T>
void _uniform_random_fill(T lower, T upper, T* data, uint64_t size)
{
...
template <>
void _uniform_random_fill(float16_t lower, float16_t upper, float16_t* data, uint64_t size)
{
...
{
    if (dtype == dtype_t::f16) {
        return scalar_t(std::accumulate(f16(), f16() + nelem(), float16_t(0.0)));
    } else if (dtype == dtype_t::f32) {
        return scalar_t(std::accumulate(f32(), f32() + nelem(), float(0.0)));
    } else if (dtype == dtype_t::f64) {
        return scalar_t(std::accumulate(f64(), f64() + nelem(), double(0.0)));
    } else if (dtype == dtype_t::c64) {
        return scalar_t(std::accumulate(c64(), c64() + nelem(), std::complex<float>(0.0, 0.0)));
    } else {
        throw std::runtime_error("should not reach");
    }
}
...
void dbuffer_t::agg_into(uint64_t which_elem, castable_t castable, scalar_t const& val)
{
    ...
    set(which_elem, op.eval({get(which_elem), val}));
}
...
void const* dbuffer_t::ptr() const
{
    return data->data;
}
...
{
    if (dtype != dtype_t::f16) {
        throw std::runtime_error("incroect dtype");
    }
...
bool is_close(dbuffer_t const& ll, dbuffer_t const& rr, float eps)
{
    ...
    if (dtype == dtype_t::f16) {
        ...
        auto rhs = rr.c64();
        for (int i = 0; i != n; ++i) {
            if (!is_close(lhs[i].real(), rhs[i].real(), eps)) {
                return false;
            }
            if (!is_close(lhs[i].imag(), rhs[i].imag(), eps)) {
                return false;
            }
        }
    } else {
