...
struct graph_writer_t {
    struct idx_t {
        ...
        bool                      is_squeeze() const
        {
            return std::holds_alternative<idx>(op);
        }
    ...
    struct tensor_t {
        ...
        full_shape_t const& get_shape() const
        {
            return shape;
        }
        int rank() const
    ...
    private:
        ...
        tensor_t(full_shape_t const& shape, int id, graph_writer_t* self);

        tensor_t(full_shape_t const& shape, vector<int> const& modes, int id, graph_writer_t* self);
    ...
    struct to_einsummable_info_t {
        ...
        einsummable_t build_einsummable(scalarop_t           scalarop,
    ...
    tensor_t input(vector<uint64_t> shape, dtype_t dtype = default_dtype());
    tensor_t input(full_shape_t shape, dtype_t dtype = default_dtype());
    tensor_t input(vector<vector<uint64_t>> const& shape, dtype_t dtype = default_dtype());

    tensor_t constant(scalar_t value, vector<uint64_t> shape, dtype_t dtype = default_dtype());
    tensor_t constant(scalar_t value, full_shape_t shape, dtype_t dtype = default_dtype());
    tensor_t constant(scalar_t                        value,
    ...
    tensor_t contraction(string str, tensor_t const& lhs, tensor_t const& rhs);

    tensor_t reduction(string str, castable_t castable, tensor_t const& inn);

    tensor_t ew(scalarop_t op, tensor_t const& inn);
    tensor_t ew( // ew = elementwise
        string          str,
        scalarop_t      op,
        tensor_t const& inn);
    tensor_t ew(string str, scalarop_t op, tensor_t const& lhs, tensor_t const& rhs);
    tensor_t ew(string str, scalarop_t op, vector<tensor_t> const& inns);

    tensor_t concat(int dim, vector<tensor_t> const& inns);

    tensor_t subset(vector<tuple<uint64_t, uint64_t>> const& hrect, tensor_t const& inn);
    ...
    tensor_t add(tensor_t const& lhs, tensor_t const& rhs);
    tensor_t mul(tensor_t const& lhs, tensor_t const& rhs);
    tensor_t straight_bew(scalarop_t op, tensor_t const& lhs, tensor_t const& rhs);

    // straight elementwise scale
    tensor_t scale(string val, tensor_t const& inn);
    tensor_t scale(scalar_t val, tensor_t const& inn);

    // supports ij,jk->ik and
    //         ...ij,...jk->...ik and
    //         ...ij,jk->ik       and
    //         ij,...jk->ik
    tensor_t matmul(tensor_t const& lhs, tensor_t const& rhs);

    tensor_t exp(tensor_t const& inn);

    // take the softmax over the last dimension
    tensor_t softmax_v1(tensor_t const& inn);
    tensor_t softmax_v2(tensor_t const& inn);
    tensor_t softmax_v3(tensor_t const& inn);
    tensor_t softmax_v3_scale(scalar_t alpha, tensor_t const& inn);

    // convert j->ij
    tensor_t broadcast(full_dim_t size, tensor_t const& inn);
    tensor_t broadcast(uint64_t sz, tensor_t const& inn);

    // 1ijk->1
    tensor_t sum_to_unit(tensor_t const& inn);

private:
    graph_t graph;

    int _insert_elementwise(string str, scalarop_t op, int id);

    optional<to_einsummable_info_t> make_einsummable_info(string str, vector<tensor_t> const& inns);
...
std::ostream& operator<<(std::ostream&, graph_writer_t::full_shape_t const&);
