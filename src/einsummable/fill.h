...
struct fill_t {
    ...
    explicit fill_t(lowertri_t const& l) : op(build_from_lowertri(l)) {}
    ...
    bool is_lowertri() const
    {
        return std::holds_alternative<lowertri_t>(op);
    }
    ...
    lowertri_t const& get_lowertri() const
    {
        return std::get<lowertri_t>(op);
    }

    uint64_t size() const
    {
        return nelem() * dtype_size(dtype());
    }
