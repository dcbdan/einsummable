...
int _compute_join_rank(vector<vector<int>> const& inns, int out_rank)
{
    ...
    for (auto const& inn : inns) {
        ...
        almost_join_rank = std::max(almost_join_rank, *std::max_element(inn.begin(), inn.end()));
    }
    almost_join_rank++;
...
einsummable_t::einsummable_t(vector<uint64_t>     join_shape,
                             vector<vector<int>>  inns,
                             int                  out_rank,
                             scalarop_t           join,
                             optional<castable_t> castable)
    : join_shape(join_shape), inns(inns), out_rank(out_rank), join(join), castable(castable)
{
...
int _count_adjacent(vector<int>::const_iterator iter, vector<int>::const_iterator end)
{
    int cnt = 0;
    int v = *iter;
...
int update_count_adjacent(int score, int r, vector<int> const& is)
{
...
vector<tuple<int, int>> build_adjacents(vector<vector<int>> const& inns)
{
    int                     r = 0;
...
einsummable_t einsummable_t::replace_scalar_variables(map<string, scalar_t> const& vars) const
{
    ...
    return einsummable_t(join_shape, inns, out_rank, new_join, castable);
}
...
inline einsummable_t
_einsummable_matmul_helper(uint64_t di, uint64_t dj, uint64_t dk, dtype_t dtype, string str)
{
    auto [inns, _] = einsummable_t::parse_str(str);

    return einsummable_t({di, dk, dj}, inns, 2, scalarop_t::make_mul(dtype), castable_t::add);
}

einsummable_t einsummable_t::from_matmul(uint64_t di, uint64_t dj, uint64_t dk, dtype_t dtype)
{
    return from_matmul_ss(di, dj, dk, dtype);
}

einsummable_t einsummable_t::from_matmul_ss(uint64_t di, uint64_t dj, uint64_t dk, dtype_t dtype)
{
    return _einsummable_matmul_helper(di, dj, dk, dtype, "ij,jk->ik");
}

einsummable_t einsummable_t::from_matmul_ts(uint64_t di, uint64_t dj, uint64_t dk, dtype_t dtype)
{
    return _einsummable_matmul_helper(di, dj, dk, dtype, "ji,jk->ik");
}

einsummable_t einsummable_t::from_matmul_st(uint64_t di, uint64_t dj, uint64_t dk, dtype_t dtype)
{
    return _einsummable_matmul_helper(di, dj, dk, dtype, "ij,kj->ik");
}

einsummable_t einsummable_t::from_matmul_tt(uint64_t di, uint64_t dj, uint64_t dk, dtype_t dtype)
{
    return _einsummable_matmul_helper(di, dj, dk, dtype, "ji,kj->ik");
}

einsummable_t einsummable_t::aggregate(vector<uint64_t> const& inn_shape,
                                       vector<int> const&      inn,
                                       int                     out_rank,
                                       dtype_t                 dtype,
                                       castable_t              castable)
{
    vector<uint64_t> join_shape = einsummable_t::construct_join_shape({inn}, {inn_shape}).value();
    return einsummable_t(join_shape, {inn}, out_rank, scalarop_t::make_identity(dtype), castable);
}

einsummable_t einsummable_t::with_new_shape(einsummable_t const&    e,
...
tuple<vector<vector<int>>, int> einsummable_t::parse_str(string str)
{
    auto iter = str.begin();
...
tuple<string, vector<string>> einsummable_t::make_str_terms(vector<vector<int>> const& inns,
...
string einsummable_t::make_str(vector<vector<int>> const& inns, vector<int> const& out)
{
...
string einsummable_t::make_str(vector<vector<int>> const& inns, int out_rank)
{
...
//   auto const& permuted_inns = {find_permutation(other_shape, argument_shape),
//   find_permutation(identity, argument_shape)}; return make_str(permuted_inns,
//   argument_shape.size());
// }
//
...
string einsummable_t::normalize_str(string const& str)
{
    auto [inns, out_rank] = parse_str(str);
    return make_str(inns, out_rank);
}

bool einsummable_t::valid_inns_out(vector<vector<int>> const& inns, int out_rank)
{
...
einsummable_t::construct_join_shape(vector<vector<int>> const&      inns,
...
vector<uint64_t> einsummable_t::construct_join_shape(vector<uint64_t> const&         out,
...
dtype_t einsummable_t::inn_dtype(int which_inn) const
{
    ...
    if (!maybe_dtype) {
        throw std::runtime_error("einsummable_t::inn_dtype: maybe arg not used?");
    }
...
vector<vector<int>> einsummable_t::input_idxs(vector<int> const& join_idx) const
{
...
{
    return inns.size() == 1 && out_rank == join_shape.size() && join.is_identity() &&
...
{
    return inns.size() == 1 && !has_aggregation() && !has_broadcast() && join.is_identity();
}
...
{
    return inns.size() == 2 && has_aggregation() && !has_broadcast() &&
           castable.value() == castable_t::add && join.is_mul();
}
...
{
    return inns.size() == 1 && has_broadcast() && !has_aggregation() && join.is_identity();
}
...
einsummable_t einsummable_t::remove_broadcast() const
{
    ...
    vector<uint64_t> new_join_shape = construct_join_shape(new_inns, inn_shapes()).value();
