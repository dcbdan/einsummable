...
fill_t::build_from_lowertri(fill_t::lowertri_t const& l)
{
    ...
    if (l.lower == l.upper) {
        ...
        return constant_t{.value = l.lower, .shape = vector<uint64_t>{l.nrow, l.ncol}};
    } else if (l.start <= 1 - int64_t(l.ncol)) {
        return constant_t{.value = l.lower, .shape = vector<uint64_t>{l.nrow, l.ncol}};
    } else if (l.start >= int64_t(l.nrow)) {
        return constant_t{.value = l.upper, .shape = vector<uint64_t>{l.nrow, l.ncol}};
    } else {
        return l;
    }
}
...
fill_t fill_t::make_square_lowertri(dtype_t d, uint64_t n)
{
    return fill_t(lowertri_t{
        .lower = scalar_t::one(d), .upper = scalar_t::zero(d), .nrow = n, .ncol = n, .start = 0});
}

fill_t::lowertri_t fill_t::lowertri_t::select(hrect_t const& hrect) const
{
    if (hrect.size() != 2) {
...
fill_t fill_t::select(hrect_t const& hrect) const
{
    ...
    if (is_constant()) {
        ...
        return fill_t(constant_t{.value = value, .shape = hrect_shape(hrect)});
    } else if (is_lowertri()) {
...
fill_t fill_t::from_proto(es_proto::Fill const& p)
{
    if (p.has_constant()) {
        ...
        auto        ds = c.shape();
        return fill_t(constant_t{.value = parse_with_ss<scalar_t>(c.value()),
                                 .shape = vector<uint64_t>(ds.begin(), ds.end())});
    } else if (p.has_lowertri()) {
        auto const& l = p.lowertri();
        return fill_t(lowertri_t{.lower = parse_with_ss<scalar_t>(l.lower()),
