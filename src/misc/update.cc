...
static int graph_insert_einsummable_ew(graph_t& graph, scalarop_t join, vector<int> const& inns)
{
    vector<uint64_t> shape = graph.out_shape(inns[0]);
    int              rank = shape.size();

    einsummable_t e(shape, vector<vector<int>>(inns.size(), vector_iota<int>(rank)), rank, join);

    return graph.insert_einsummable(e, inns);
}

static vector<tuple<int, fill_t>> vanilla_update_weights(dtype_t                  dtype,
                                                         graph_t&                 graph,
                                                         ...
                                                         vector<int> const&       grad_ids)
{
    scalarop_t grad_update = scalarop_t::combine(
        scalarop_t::make_sub(),
        {scalarop_t::make_identity(dtype), scalarop_t::make_scale("learning_rate", dtype)});
...
static vector<tuple<int, fill_t>> momentum_update_weights(dtype_t                  dtype,
                                                          graph_t&                 graph,
                                                          vector<tuple<int, int>>& ret,
                                                          vector<int> const&       weight_ids,
                                                          vector<int> const&       grad_ids)
{
    scalarop_t update_v = scalarop_t::combine(
        ...
        {scalarop_t::make_scale("eta", dtype), scalarop_t::make_scale("learning_rate", dtype)});
...
static void vanilla_update_vars(dtype_t dtype, map<string, scalar_t>& vars)
{
    // put a default learning rate if none is already in vars
    vars.insert({"learning_rate", scalar_t(dtype, "1e-3")});
}

static void momentum_update_vars(dtype_t dtype, map<string, scalar_t>& vars)
{
    // put a default learning rate if none is already in vars
    vars.insert({"learning_rate", scalar_t(dtype, "1e-3")});
...
static void
adamw_update_vars(dtype_t dtype, dtype_t min_precision, int iter, map<string, scalar_t>& vars)
{
    if (vars.count("beta1") == 0 || vars.count("beta2") == 0 || vars.count("eta") == 0) {
    ...
    vars.insert_or_assign("beta1tt",
                          one_mp / (one_mp - power.eval(vars["beta1"].convert(min_precision))));
    vars.insert_or_assign("beta2tt",
                          one_mp / (one_mp - power.eval(vars["beta2"].convert(min_precision))));
...
static vector<tuple<int, fill_t>> adamw_update_weights(dtype_t                  dtype,
                                                       ...
                                                       vector<int> const&       grad_ids)
{
    ...
    auto up_prec_scale = [&](string var) {
        return scalarop_t::combine(scalarop_t::make_scale(var, min_precision), {up_prec});
    };

    scalarop_t grad_update =
        scalarop_t::combine(scalarop_t::make_sub(min_precision),
                            {up_prec, scalarop_t::make_scale("eta", min_precision)});
    grad_update = scalarop_t::combine(down_prec, {grad_update});

    scalarop_t beta1_portion =
        scalarop_t::combine(scalarop_t::make_add(dtype),
                            {scalarop_t::make_scale("beta1", dtype),
                             scalarop_t::make_scale("beta1_complement", dtype)});

    scalarop_t scale_square = scalarop_t::combine(scalarop_t::make_scale("beta2_complement", dtype),
                                                  {scalarop_t::make_square(dtype)});

    scalarop_t beta2_portion = scalarop_t::combine(
        scalarop_t::make_add(dtype), {scalarop_t::make_scale("beta2", dtype), scale_square});

    scalarop_t sqrt_plus_eps = scalarop_t::combine(
        scalarop_t::make_add(min_precision),
        {scalarop_t::make_sqrt(min_precision), scalarop_t::make_variable("eps", min_precision)});
...
vector<tuple<int, fill_t>> update_weights(updater_desc_t const&    t,
                                          graph_t&                 graph,
                                          ...
                                          vector<int> const&       grad_ids)
{
    if (t.is_vanilla()) {
        ...
        return adamw_update_weights(
            t.dtype, a.min_precision, graph, old_news, weight_ids, grad_ids);
...
void update_vars(updater_desc_t const& t, int iter, map<string, scalar_t>& vars)
{
    if (t.is_vanilla()) {
