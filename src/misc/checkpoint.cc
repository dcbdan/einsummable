...
optional<int> graph_id_manager_t::get_sid_from_sid(
  int which_src, int src_sid, int which_dst) const
{
...
checkpoint_graphs_t::checkpoint_graphs_t(
  graph_t const& full_graph,
  vector<int> const& checkpoints,
  set<int> const& forward_ids)
{
  ...
  for(int which = 0; which != gs.graphs.size(); ++which) {
    ...
    auto make_rels = [&](map<int, vtensor_t<int>> const& mtids) {
      ...
      for(auto const& [sid, tids]: mtids) {
        ret.insert({sid, relation_t {
          .dtype = graph.out_dtype(sid),
          .placement = pls[sid],
          .tids = tids
        }});
      }
