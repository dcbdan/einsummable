...
cluster_t make_cluster(cluster_settings_t settings)
{
    ...
    vector<device_t> devices(num_nodes, device_t(num_threads_per_node, f_cost));

    vector<connection_t> connections;
    for (int i = 0; i != num_nodes; ++i) {
        for (int j = 0; j != num_nodes; ++j) {
            if (i != j) {
                connections.push_back(connection_t{.bandwidth = bandwidth, .src = i, .dst = j});
            }
        }
    }
...
taskgraph_t solve(graph_t const&       graph,
    ...
    for (auto const& beta : autoplace_settings.betas) {
        if (autoplace_settings.do_balanced) {
            threads.emplace_back([&]() {
                runner(forwardsim_mcmc_t::init_balanced(cluster, graph, beta),
                       autoplace_settings.num_steps);
            });
        }
        if (autoplace_settings.do_singleloc) {
            threads.emplace_back([&]() {
                runner(forwardsim_mcmc_t::init_with_single_loc(cluster, graph, beta),
                       autoplace_settings.num_steps);
            });
        }
    }
...
int main(int argc, char** argv)
{
    ...
    execute_taskgraph_settings_t execute_settings{
        .num_apply_runner = num_threads_per_node, .num_send_runner = 1, .num_recv_runner = 1};
    ...
    if (mpi.this_rank == 0) {
        ...
        cluster_settings_t cluster_settings{.num_nodes = num_nodes,
                                            .num_threads_per_node = num_threads_per_node,
                                            .compute_per_thread = 5 * giga,
                                            .bandwidth = 1 * giga};

        autoplace_settings_t autoplace_settings{
            .num_steps = 1000, .betas = {10000.0}, .do_balanced = true, .do_singleloc = false};
