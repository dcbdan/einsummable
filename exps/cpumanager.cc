...
void test_server(mpi_t&                     mpi,
                 full_settings_t const&     settings,
                 graph_t const&             graph,
                 vector<placement_t> const& placements,
                 map<int, dbuffer_t>        gid_inn_data)
{
    auto [inn_gid_to_tids, save_gid_to_tids, taskgraph] = taskgraph_t::make(graph, placements);
    ...
    auto process = [&](manager_base_t& manager) {
        ...
        for (auto [gid, dbuffer] : gid_inn_data) {
            relation_t rel{
                .dtype = dbuffer.dtype, .placement = placements[gid], .tids = inn_gid_to_tids[gid]};
        ...
        for (int gid = 0; gid != graph.nodes.size(); ++gid) {
            ...
            if (node.op.is_save()) {
                relation_t rel{.dtype = node.op.out_dtype(),
...
tuple<graph_t, vector<placement_t>, map<int, dbuffer_t>> mm(int world_size, int argc, char** argv)
{
    set_seed(0);
    ...
    int lhs = g.insert_input(partition_t({partdim_t::split(ni, li), partdim_t::split(nj, lj)}));
    int rhs = g.insert_input(partition_t({partdim_t::split(nj, rj), partdim_t::split(nk, rk)}));
    ...
    int out =
        g.insert_formation(partition_t({partdim_t::split(ni, oi), partdim_t::split(nk, ok)}), join);
...
int main(int argc, char** argv)
{
    ...
    full_settings_t settings{
        .exec_tg = execute_taskgraph_settings_t{.num_apply_runner = num_threads,
                                                .num_send_runner = 2,
                                                .num_recv_runner = 2},
        .exec_mg = execute_memgraph_settings_t{.num_apply_runner = num_threads,
    ...
    if (mpi.this_rank == 0) {
        ...
        test_server(mpi, settings, graph, placements, input_data);
    } else {
