...
void execute_memgraph_cpu(memgraph_t const& memgraph, buffer_t buffer, cpu_storage_t& storage);
...
int main(int argc, char** argv)
{
    ...
    auto [inn_to_memstoloc, out_to_memstoloc, memgraph] =
        memgraph_t::make(taskgraph, vector<int>{}, vector<uint64_t>{mem_size});
    ...
    for (auto const& [inn, memstoloc] : inn_to_memstoloc) {
        ...
        if (memstoloc.is_memloc()) {
            ...
            std::copy(tensor.data->data, tensor.data->data + size, buffer->data + offset);
        } else {
...
void execute_memgraph_cpu(memgraph_t const& memgraph, buffer_t buffer, cpu_storage_t& storage)
{
    ...
    exec_graph_t graph = exec_graph_t::make_cpu_exec_graph(memgraph, 0, executor);

    threadpool_t threadpool(12); // TODO: hardcoded number of threads

    rm_ptr_t resource_manager(
        new resource_manager_t(vector<rm_ptr_t>{rm_ptr_t(new cpu_workspace_manager_t()),
