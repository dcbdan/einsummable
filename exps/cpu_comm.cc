...
relation_t make_dst_rel(int num_split, uint64_t nrow, uint64_t ncol)
{
    partition_t partition(
        vector<partdim_t>{partdim_t::split(nrow, num_split), partdim_t::split(ncol, 1)});
    ...
    return relation_t{.dtype = dtype_t::f32, .placement = placement, .tids = tids};
}
...
void main07(int argc, char** argv)
{
    ...
    remap.insert(dst_relation.as_singleton(99), dst_relation);
