...
#define ROUT(x)                                                                                    \
    if (mpi.this_rank == 0) {                                                                      \
        std::cout << "(in cpu exec) " << x << std::endl;                                           \
    }
...
int main(int argc, char** argv)
{
    ...
    if (with_taskgraph) {
        execute_taskgraph_settings_t execute_settings{.num_apply_runner = num_threads_per_node,
    ...
    if (with_memgraph) {
        execute_memgraph_settings_t execute_settings{.num_apply_runner = num_threads_per_node,
