...
struct runner_state_t {
    ...
    void register_einsummable(einsummable_t const& e)
    {
        ...
        for (int i = 0; i != inn_shapes.size(); ++i) {
            inn_ids.push_back(make_data(e.inn_dtype(i), inn_shapes[i]));
        }

        ops.push_back(op_t{.e = e, .out = out_id, .inns = inn_ids});
    }
    ...
    void runner(int runner_id)
    {
        ...
        while (true) {
            ...
            double wall_time;
            {
                ...
                wall_time = std::chrono::duration<double, std::milli>(end - start).count();
            }
            ...
            datum_t datum{.inn_elem = inn_total,
    ...
    void print_csv(std::ostream& out) const
    {
        ...
        for (auto const& d : stats) {
            out << d.inn_elem << "," << d.out_elem << "," << d.compute << "," << d.time
                << std::endl;
        }
    }
...
void test_matmuls()
{
    ...
    set_seed(0);
    auto random_size = [&] { return possible_sizes[runif(possible_sizes.size())]; };
...
void test_ewbs(int num_threads)
{
    ...
    for (int _ = 0; _ != num_ops; ++_) {
        uint64_t i = random_size();
        uint64_t j = random_size();
        ss.register_einsummable(einsummable_t({i, j}, {{0, 1}, {0, 1}}, 2, scalarop_t::make_mul()));
    }
