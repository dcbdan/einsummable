    ...
    for (int idx = 0; idx != m.nodes.size(); ++idx) {
        ...
        for (auto input_idx : node.inns) {
            ...
            if (m.nodes[input_idx].outs.find(idx) == m.nodes[input_idx].outs.end()) {
                ...
                std::printf(
                    "Error: Node %d has node % d as its input but node %d "
        ...
        for (auto output_idx : node.outs) {
            ...
            if (m.nodes[output_idx].inns.find(idx) == m.nodes[output_idx].inns.end()) {
                ...
                std::printf(
                    "Error: Node %d has node % d as its output but node %d "
...

    {
        tuple<map<int, mem_t>, // input -> mem
            ...
            _info1 = memgraph_t::make_without_evict(
                taskgraph, mem_sizes, {allocator_strat_t::lowest_dependency, 4});
        ...
        for (int i = 0; i < np; ++i) {
            ...
            // std::cout << "memgraph size on gpu " << i << ": " << memgraph.mem_sizes()[i] <<
            // std::endl;
...
template <typename T>
void slow_mm_(uint64_t ni, uint64_t nj, uint64_t nk, T* out, T const* lhs, T const* rhs)
{
    std::fill(out, out + ni * nk, 0);
    for (uint64_t i = 0; i != ni; ++i) {
        for (uint64_t j = 0; j != nj; ++j) {
            for (uint64_t k = 0; k != nk; ++k) {
                out[i * nk + k] += lhs[i * nj + j] * rhs[j * nk + k];
            }
        }
    }
}
...
void mm_test()
{
    ...
    vector<tuple<uint64_t, uint64_t, uint64_t>> szs = {{1024, 1024, 1024},
                                                       {2048, 1024, 1024},
                                                       {1024, 2048, 1024},
                                                       {1024, 1024, 2048},
                                                       {2048, 2048, 2048},
...
void server_3d_mamtmul(int argc, char** argv)
{
    ...
    auto init_duration =
        std::chrono::duration_cast<std::chrono::microseconds>(data_init_time - start);
    ...
    auto execution_duration =
        std::chrono::duration_cast<std::chrono::microseconds>(execution_time - data_init_time);
...
void ffnn_graph(int argc, char** argv)
{
    ...
    DOUT("input " << num_input_msgs << "#, " << to_mb(num_input_bytes) << "MB, " << num_core_msgs
                  << "#, " << to_mb(num_core_bytes) << "MB");
...
void mm_test2()
{
    ...
    if (cudaMemcpy(c, output.data->data, output.data->size, cudaMemcpyHostToDevice) !=
        cudaSuccess) {
...
scalarop_t scalarop_km()
{
    ...
    // build the scalarop: +[*[constant{f32|0.999},hole|f32@0],*[constant{f32|0.000999987},
    // *[hole|f32@1, [hole|f32@1]]
    scalarop_t ret2_new =
        scalarop_t::combine(mul, {constant2, scalarop_t::replace_arguments(mul, {arg, arg})});
...
scalarop_t scalarop_km2()
{
    ...
    scalarop_t new_ret =
        scalarop_t::replace_arguments(div, {one, scalarop_t::replace_arguments(mul, {arg2, arg2})});
