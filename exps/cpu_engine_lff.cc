...
make_graph_ff(
uint64_t batch,  int nparts_batch,
uint64_t hidden, int nparts_hidden,
uint64_t dim,    int nparts_dims);

double execute_direct_ff(
  uint64_t batch,
  uint64_t hidden,
  uint64_t dim);
  ...
  uint64_t dim,    int nparts_dim)
{
  ...
  auto maybe_parts = autopart_from_inputs(
    graph,
    map<int, partition_t>{
      { x.get_id(),  partition_t({ pd_batch, pd_dim}) },
