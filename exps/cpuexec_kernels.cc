...
void test_subset_bmm(dtype_t   dtype,
                     bool      trans_lhs,
                     bool      batch_out,
                     partdim_t pd_b,
                     partdim_t pd_i,
                     ...
                     uint64_t  nk)
{
    ...
    vector<uint64_t> join_shape =
        einsummable_t::construct_join_shape(out_shape, inns, {lhs_shape, rhs_shape});
    einsummable_t contraction(
        join_shape, inns, out_rank, scalarop_t::make_mul(dtype), castable_t::add);
...
void test_bmm(dtype_t dtype)
{
    ...
    einsummable_t matmul = einsummable_t(
        {b, i, k, j}, {{0, 1, 3}, {3, 2}}, 3, scalarop_t::make_mul(dtype), castable_t::add);
...
void test_unary(scalarop_t scalarop)
{
    ...
    einsummable_t e = einsummable_t({a, b}, {{0, 1}}, 2, scalarop);
...
void test_straight_binary(scalarop_t scalarop)
{
    ...
    einsummable_t e = einsummable_t({a, b, c}, {{0, 1, 2}, {0, 1, 2}}, 3, scalarop);
...
void test_ab_a_binary(scalarop_t scalarop)
{
    ...
    einsummable_t e = einsummable_t({a, b, c}, {{0, 1, 2}, {0, 1}}, 3, scalarop);
...
void test_ab_b_binary(scalarop_t scalarop)
{
    ...
    einsummable_t e = einsummable_t({a, b, c}, {{0, 1, 2}, {2}}, 3, scalarop);
...
void test_reduction_ab_a(dtype_t dtype, castable_t castable)
{
    ...
    einsummable_t e =
        einsummable_t({a, b}, {{0, 1}}, 1, scalarop_t::make_identity(dtype), castable);
...
void test_a_contraction()
{
    ...
    optional<tuple<void*, uint64_t>> workspace =
        tuple<void*, uint64_t>{workspace_buffer->raw(), workspace_size};
