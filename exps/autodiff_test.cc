...
void exp07()
{
    ...
    int x = g.insert_input(partition_t({partdim_t::from_sizes({11, 9})}), dtype_t::f32);
    int y = g.insert_to_complex(partition_t({partdim_t::from_sizes({5, 5})}), x);
...
{
    select_t select =
        make_concat(0, default_dtype(), vector<vector<uint64_t>>{{20, 30}, {10, 30}, {40, 30}});
    {
        hrect_t out_hrect;
