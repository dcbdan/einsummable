...
taskgraph_t test_make_taskgraph(graph_t const&             graph,
                                vector<placement_t> const& placements,
                                map<int, dbuffer_t>        full_inns)
{
    tuple<map<int, vtensor_t<int>>, map<int, vtensor_t<int>>, taskgraph_t> _info =
    ...
    for (auto [gid, full_buffer] : full_inns) {
        vtensor_t<dbuffer_t> pbuffer = partition_buffer(placements.at(gid).partition, full_buffer);
    ...
    map<int, dbuffer_t> task_outs = typed_reference_compute_taskgraph_from_graph_info(
...
taskgraph_t test_make_taskgraph(graph_constructor_t const& graph, map<int, dbuffer_t> full_inns)
{
    return test_make_taskgraph(graph.graph, graph.get_placements(), full_inns);
}

void test_make_memgraph_with_evict(graph_t const&             graph,
                                   vector<placement_t> const& placements,
                                   map<int, dbuffer_t>        full_inns,
                                   ...
                                   vector<uint64_t>           mem_sizes)
{
    ...
    tuple<map<int, vtensor_t<int>>, map<int, vtensor_t<int>>, taskgraph_t> _info0 =
        taskgraph_t::make(graph, placements);
    ...
    // copy into the location wise buffers
    {
        ...
        for (auto const& [tid, dbuffer] : task_inns) {
            ...
            if (inn_memstoloc.is_stoloc()) {
                ...
                std::copy(buffer->data, buffer->data + size, loc_buffers[loc]->data + offset);
            }
        }
    }
        ...
        for (int i = 0; i != vec.size(); ++i) {
            ...
            if (out_memstoloc.is_memloc()) {
                ...
                from_memgraph = dbuffer_t(
                    from_graph.dtype, make_buffer_reference(loc_buffers[loc]->data + offset, size));
            } else {
                auto const& [sto_loc, sto_id] = out_memstoloc.get_stoloc();
                from_memgraph =
                    dbuffer_t(from_graph.dtype,
                              make_buffer_reference(sto_buffers[sto_loc].at(sto_id)->data,
                                                    sto_buffers[sto_loc].at(sto_id)->size));
            }
...
void test_make_memgraph_without_evict(graph_t const&             graph,
                                      vector<placement_t> const& placements,
                                      map<int, dbuffer_t>        full_inns)
{
    // graph.print();
...
void test_make_memgraph_without_evict(graph_constructor_t const& graph,
                                      map<int, dbuffer_t>        full_inns)
{
    return test_make_memgraph_without_evict(graph.graph, graph.get_placements(), full_inns);
}
...
void test_obvious_matmul(int pi, int pj, int pk)
{
    ...
    int id_save = graph.insert_formation(partition_t({pdi, pdk}), id_join, true);
...
void test_obvious_same_input_matmul(int pi, int pj, int pk)
{
    ...
    int id_join = graph.insert_einsummable(partition_t({pdi, pdk, pdj}), matmul, {id_inn, id_inn});
...
void test_obvious_random_loc_matmul(int pi, int pj, int pk, int nloc)
{
    ...
    int id_join = graph.insert_einsummable(
        placement_t::random(partition_t({pdi, pdk, pdj}), nloc), matmul, {id_lhs, id_rhs});

    int id_save =
        graph.insert_formation(placement_t::random(partition_t({pdi, pdk}), nloc), id_join, true);
...
void test_obvious_matmul_with_evict(int      pi,
                                    int      pj,
                                    int      pk,
                                    uint64_t ni = 20,
                                    uint64_t nj = 20,
                                    ...
                                    uint64_t mem_size = 2000)
{
    ...
    test_make_memgraph_with_evict(
        graph.graph, graph.get_placements(), inns, which_storage, mem_sizes);
}
...
void test_random_matmul()
{
    ...
    int id_join =
        graph.insert_einsummable(random_placement({ni, nk, nj}), matmul, {id_lhs, id_rhs});

    int id_save = graph.insert_formation(random_placement({ni, nk}), id_join, true);
...
void test_random_matmul_then_unary_ew(scalarop_t unary_scalar_op)
{
    ...
    einsummable_t unary_es({ni, nk}, {{0, 1}}, 2, unary_scalar_op);

    auto const& pds = graph.placements.at(id_join).partition;
    partition_t part_unary(vector<partdim_t>(pds.partdims.begin(), pds.partdims.begin() + 2));

    int id_unary =
        graph.insert_einsummable(placement_t::random(part_unary, nloc), unary_es, {id_join});

    int id_save = graph.insert_formation(random_placement({ni, nk}), id_unary, true);
...
void test_3d_matmul(int pi, int pj, int pk, int nloc)
{
    ...
    graph_constructor_t graph =
        three_dimensional_matrix_multiplication(pi, pj, pk, di, dj, dk, nloc);
...
taskgraph_t test_random_concat(int               dim,
                               ...
                               int               maxpart = 10)
{
    ...
    for (int i = 0; i != n_inn; ++i) {
        ...
        inn_tensors.insert({inns.back().get_id(), make_dbuffer(dtype, product(shape))});
    }
...
void main_pass_through_partials_stuff()
{
...

    {
        ...
        tg.add_to_partial(
            x,
            a1,
            touch_t{.selection = {{50, 100, 0, 0, 50}}, .castable = std::nullopt, .dtype = dtype},
            false);
        tg.add_to_partial(
            x,
            a2,
            touch_t{.selection = {{50, 100, 0, 50, 50}}, .castable = std::nullopt, .dtype = dtype},
            false);
...
void main_touch_compose()
{
    auto make_1d_touch = [](vector<uint64_t> v) {
        return touch_t{.selection = {touchdim_t{v[0], v[1], v[2], v[3], v[4]}},
...
void main_subset(int which)
{
    ...
    if (which == 0) {
        ...
        scalar_t scalar = dtype_is_complex(dtype) ? scalar_t(std::complex<float>(0.01, 0.01))
                                                  : scalar_t(dtype, "0.01");
    ...
    test_make_taskgraph(graph, pls, {{x.get_id(), x_data}});
}
...
void mm_test()
{
    ...
    auto [_0, _1, maybe_mg, exec_mg] = memgraph_t::make_(taskgraph,
                                                         {},
                                                         vector<uint64_t>(np, mem_size),
                                                         {},
