  ...
  uint64_t n_heads)
{
  ...
  scalarop_t scale = scalarop_t::make_scale(
    scalar_t(default_dtype(), write_with_ss(
      1.0 / (std::sqrt(double(1.0) * head_dim))
    ))
  );
  ...
  uint64_t n_heads)
{
  ...
  auto es_a = make_e(
    {batch_size, unit, n_heads, head_dim, n_heads, head_dim},
    "abef,cdef->abcd");
  auto a0 = make_data(es_a, 0);
  auto a1 = make_data(es_a, 1);

  auto es_b = make_e(
    {batch_size, n_heads, unit, start_pos, head_dim},
    "acbe,adbe->abcd");
  auto b1 = make_data(es_b, 1);

  auto es_c = make_e(
    {batch_size, n_heads, unit, head_dim, start_pos},
    "abce,aebd->abcd");
  ...
  optional<tuple<void*, uint64_t>> wspace =
    tuple<void*, uint64_t>(workspace->raw(), wsz);
  ...
  uint64_t n_heads)
{
  ...
  auto es_a = make_e(
    {batch_size, n_heads, head_dim, n_heads, head_dim},
    "ade,bcde->abc");
  auto a0 = make_data(es_a, 0);
  auto a1 = make_data(es_a, 1);

  auto es_b = make_e(
    {batch_size, n_heads, start_pos, head_dim},
    "abd,acbd->abc");
  auto b1 = make_data(es_b, 1);

  auto es_c = make_e(
    {batch_size, n_heads, head_dim, start_pos},
    "abd,adbc->abc");
  auto c1 = make_data(es_c, 1);
...
struct tblis_t {
  tblis_t(vector<uint64_t> const& shape_, bool rnd = false)
    : stride(shape_.size())
  {
  ...
  tblis::tblis_tensor*       ptr()       { return &t; }
  tblis::tblis_tensor const* ptr() const { return &t; }
...
void tblis_exp01() {
  ...
  tblis_init_tensor_s(&A, 4, l0.data(),
    reinterpret_cast<float*>(ddd->raw()), s0.data());
  DLINE;
  ...
  tblis_init_tensor_scaled_s(&B, 0.0f, 3, l1.data(),
                             data_B, s1.data());
  DLINE;
