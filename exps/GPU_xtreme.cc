...
static string train_file =
    "/home/zhimin/einsummable_mg/einsummable/build/AmazonCat-13K.bow/train.txt";
static string validate_file =
    "/home/zhimin/einsummable_mg/einsummable/build/AmazonCat-13K.bow/test.txt";
...
void main_rank_zero(server_base_t* server, xtreme_dist_t& xtreme_dist, args_t& args);
                        ...
                        autoplace_config_t const& autoplace_config)
{
    ...
    auto validate =
        make_validate_info(batch_validate, state_shapes, model_config, autoplace_config);
...
struct xtreme_file_reader_t {
    xtreme_file_reader_t(string const& data_filename) : file(data_filename), iter(0)
    {
        string str;
...
pscores_t compute_scores(uint64_t num_labels, dbuffer_t predictions, vector<datum_t> const& data)
{
...
void main_rank_zero(server_base_t* server, xtreme_dist_t& xtreme_dist, args_t& args)
{
    ...
    autoplace_config_t autoplace_config =
        autoplace_config_t::make_default01(num_gpus, num_computes);
    ...
    for (int which_run = 0; which_run != num_runs; ++which_run) {
        ...
        auto scores = compute_scores(
            num_labels, server->get_tensor_from_gid(info.validate.predictions), data);
