...
void execute_memgraph_cpu(memgraph_t const& memgraph,
...
memgraph_t build_memgraph(vector<uint64_t> mem_sizes, int argc, char** argv)
{
    ...
    auto [inn_to_memstoloc, out_to_memstoloc, memgraph] =
        memgraph_t::make(taskgraph, vector<int>{}, mem_sizes);
                          ...
                          cpu_storage_t&    storage)
{
    ...
    exec_graph_t graph =
        exec_graph_t::make_cpu_exec_graph(memgraph, communicator.get_this_rank(), executor);
