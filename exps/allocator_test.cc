...
int main05() {
  ...
  auto [occupied_dep, deps] = alo.allocate_at(0, 75);
  DOUT("should fail here");
  std::cout << "occupied_dep: " << occupied_dep << std::endl;
  alo.print();
  alo.free(o2, 2);
  auto [occupied_dep1, deps1] = alo.allocate_at(0, 75);
  DOUT("should success");
  std::cout << "occupied_dep: " << occupied_dep1 <<  std::endl;
  alo.print();
  return 0;
}
