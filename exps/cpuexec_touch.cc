...
touchdim_t random_touchdim()
{
    ...
    touchdim_t ret{.d_inn = d1, .d_out = d2, .offset_inn = o1, .offset_out = o2, .size = sz};
...
touch_t random_touch(int sz)
{
    ...
    return touch_t{
        .selection = selection, .castable = random_castable(dtype != dtype_t::c64), .dtype = dtype};
}
...
int main()
{
    ...
    einsummable_t e({10, 20, 30, 40}, inns, out_rank, scalarop_t::make_identity());
    DOUT(e);
