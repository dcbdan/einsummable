    ...
    graph_constructor_t gc;
    {
        ...
        for (int i = 1; i != n + 1; ++i) {
            tensors[0] = gc.insert_formation(
                part_l, gc.insert_einsummable(part_j, einsummable, {tensors[0], tensors[i]}));
        }
    }
...
tuple<taskgraph_t, map<int, buffer_t>> make_tg_and_data(uint64_t sz, int n, int npart)
{
...
int test01(int argc, char** argv)
{
    ...
    for (int i = 1; i != n + 1; ++i) {
        ...
        matrix_multiply(
            default_dtype(), sz, sz, sz, false, false, out.raw(), data[0].raw(), data[i].raw());
        data[0] = out;
    }
...
int test02(int argc, char** argv)
{
    ...
    auto  status = mkl_jit_create_sgemm(
        &handle, MKL_ROW_MAJOR, MKL_NOTRANS, MKL_NOTRANS, ni, nk, nj, 1.0, nj, nk, 0.0, nk);
...
int main01(int argc, char** argv)
{
    ...
    execute_taskgraph_settings_t settings{
        .num_apply_runner = nrunner, .num_send_runner = 0, .num_recv_runner = 0};
