...
struct dataset_reader_t {
    ...
    //   (bsz x vocab_size)
    tuple<dbuffer_t, dbuffer_t> make_random_data(dtype_t dtype, uint64_t bsz, uint64_t max_seq_len)
    {
        ...
        auto set_x = [&](uint64_t b, uint64_t s, uint64_t v) {
            uint64_t iii =
                indexer_utils<uint64_t>::idxs_to_index({bsz, max_seq_len, vocab_size}, {b, s, v});
            x.set(iii, one);
        };
        auto set_y = [&](uint64_t b, uint64_t v) {
            uint64_t iii = indexer_utils<uint64_t>::idxs_to_index({bsz, vocab_size}, {b, v});
            y.set(iii, one);
        };
